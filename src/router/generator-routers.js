  ...
  return new Promise((resolve, reject) => {
    request({
      ...
      .then(res => {
        ...
        store.getters.userInfo.popscreen = null
        ...
        const getPath = (array) => {
          array.forEach(item => {
            if (item.children && item.children.length) {
              ...
              if (item.meta && item.meta.component === 'crm/Popscreen') {
                store.getters.userInfo.popscreen = item.path
              }
            }
          })
        }
  ...
  list.forEach(item => {
    ...
    if (item.parentId === parentId) {
      if (type === 'componentView') {
        viewList.push(item)
        listToTree(list, [], item.id, 'componentView', item.view)
      } else {
        const child = {
          ...item,
