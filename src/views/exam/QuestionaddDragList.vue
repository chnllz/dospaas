<template>
  <div>
    <draggable v-model="list" animation="200" handle=".handle" @start="drag = true" @end="drag = false">
      <div v-for="(value, key) in list" :key="key" class="list-item">
        <a-form-item :wrapper-col="{ span: 24 }">
          <a-row type="flex" align="middle">
            <a-col :span="1">
              <a-radio-group v-if="type === 'single'" v-model.trim="chiose[key]">
                <a-radio :value="key" @change="change($event, key)"></a-radio>
              </a-radio-group>
              <a-checkbox
                v-if="type === 'multiple'"
                :checked="multiple[key]"
                @change="change($event, key)"
              ></a-checkbox>
            </a-col>
            <a-col :span="1" style="text-align: right">{{ String.fromCharCode(65 + key) }}:</a-col>
            <a-col :span="2">
              <a-icon type="drag" class="handle" />
            </a-col>
            <a-col :span="16">
              <a-input v-model.trim="list[key]" :placeholder="'选项' + (key + 1)" />
            </a-col>
