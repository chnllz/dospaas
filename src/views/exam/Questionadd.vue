    ...
    <a-spin :spinning="false">
      <div style="padding-bottom: 100px">
        ...
        <a-form :form="form" :label-col="{ span: 4 }" v-bind="formItemLayoutWithOutLabel" @submit="handleSubmit">
          <a-form-item v-if="config.action === 'add'" label="题型" :wrapper-col="{ span: 20 }">
            <a-radio-group
              v-decorator="[
                'info[type]',
                { initialValue: questionstype, rules: [{ required: true, message: '请选择题型' }] }
              ]"
              button-style="solid"
              @change="handleChange"
            >
          ...
          <a-form-item v-if="questionstype !== 'fills'" label="题目" :wrapper-col="{ span: 20 }">
            <a-textarea
              v-decorator="[
                'info[title]',
                {
                  ...
                  rules: [
                    { required: allshow, message: '请输入题目' },
                    { max: 200, message: '题目长度不得大于200个字符' }
                  ]
                }
              ]"
          ...
          <div v-if="questionstype === 'fills'">
            ...
            <a-col :span="4"></a-col>
            <span>
              <a style="padding-right: 5px" @click="pushfills">插入填空项</a>
              <a-tooltip>
                <template slot="title">
              ...
              <a class="margin" @click="refreshfills">刷新</a>
            </span>
            <div v-for="(item, index) in optionsfill" :key="index">
              <a-form-item v-bind="index === 0 ? formItemLayout : {}" :required="true">
                ...
                <a-col :span="14">
                  <a-input
                    v-decorator="[
                      'optionsfill[' + index + ']',
                      { initialValue: optionsfill[index], rules: [{ required: allshow, message: '请输入填空项' }] }
                    ]"
            ...
            <a-col v-if="optionsfill.length > 0" :span="4"></a-col>
            <a-form-item v-if="optionsfill.length > 0" :wrapper-col="{ span: 20 }">
              <a-checkbox v-model="allow" @change="allowchange">允许答题与答案顺序不一致</a-checkbox>
            </a-form-item>
          </div>
          <div v-if="questionstype === 'single' || questionstype === 'multiple'">
            ...
            <a-form-item label="答案" :wrapper-col="{ span: 20 }">
              <a-input
                ...
                v-decorator="[
                  'answerShow',
                  { initialValue: singleanswer, rules: [{ required: true, message: '请选择正确答案' }] }
                ]"
                :read-only="true"
                placeholder="未设置"
              ...
              <a-input
                ...
                v-decorator="[
                  'answerShow',
                  { initialValue: muanswer.toString(), rules: [{ required: true, message: '请选择正确答案' }] }
                ]"
                :read-only="true"
                placeholder="未设置"
                style="border: none; box-shadow: 0px 0px 0px 0px"
          ...
          <a-form-item v-if="questionstype === 'answer'" :wrapper-col="{ span: 20 }">
            <span slot="label">
              得分关键词
              <a-tooltip placement="top">
                <template slot="title">
            ...
            <a-textarea
              v-decorator="[
                'setting[answer]',
                { rules: [{ required: true, message: '请输入得分关键词' }], initialValue: formdata.answer }
              ]"
              :auto-size="{ minRows: 5, maxRows: 8 }"
              @blur="validateBlurForKeyword"
            />
          </a-form-item>
          <a-form-item v-if="questionstype === 'judge'" label="答案" :wrapper-col="{ span: 20 }">
            <a-radio-group
              v-decorator="[
                'setting[answer]',
                { initialValue: formdata.answer, rules: [{ required: true, message: '请选择正确答案' }] }
              ]"
              button-style="solid"
            >
        ...
        <div class="bbar" style="position: absolute; bottom: 0; right: 0; width: 100%; background-color: white">
          <a-button
            ...
            type="primary"
            html-type="submit"
            @click="handleSubmit($event, 'next')"
          >
            添加并继续
          </a-button>
          <a-button type="primary" html-type="submit" @click="handleSubmit">
            {{ config.action === 'edit' ? '保存' : '添加并关闭' }}
          </a-button>
  ...
  methods: {
    ...
    validateBlur (e) {
      ...
      if (e.target.value && reg.test(e.target.value)) {
        ...
        this.form.setFields({ 'info[title]': { value: e.target.value, errors: arr } })
      }
    },
    validateBlurForKeyword (e) {
      const reg = /^\s*$/
      if (e.target.value && reg.test(e.target.value)) {
        const arr = [{
          message: '内容不能仅为空格!',
        ...
        this.form.setFields({ 'setting[answer]': { value: e.target.value, errors: arr } })
      }
    },
    // 数据提交
    handleSubmit (e, type) {
      ...
      this.form.validateFields((err, values) => {
        ...
        // todo 校验空字符串
        const reg = /^\s*$/
        if (reg.test(values.info.title)) {
          err = '标题不能仅为空格'
        }
        const arr = []
        if (values.setting.list) {
          for (const item in values.setting.list) {
            arr.push(item)
            if (!values.setting.list[item]) {
              err = '选项内容不能为空'
            }
          }
          if (arr.length < 2) {
        ...
        if (!err) {
          ...
          if (this.config.action === 'edit') {
            ...
            self.axios({
              ...
              if (this.questionstype === 'single' || this.questionstype === 'multiple') {
                if (this.$refs.Drag.chiose) {
                  this.$refs.Drag.chiose = []
                }
                if (this.$refs.Drag.multiple) {
                  this.$refs.Drag.multiple = []
                }
              }
              return res.result
