            ...
            <a-tabs v-model="activeKey" class="tabs-flex" default-active-key="chat" @change="changeTab">
              <a-tab-pane key="chat">
                    ...
                    <a-popover v-model="nowFilterVisible" trigger="click" placement="bottom" :title="$t('排序方案')">
                      <div slot="content" style="width: 240px">
                        ...
                        <div style="text-align: right; margin-top: 8px">
                          <a-space>
                            <a-button size="small" @click="nowFilterVisible = !nowFilterVisible">
                              {{ $t('关闭') }}
                            </a-button>
                            <a-button
                              type="primary"
                ...
                <a-list
                  ...
                  class="tabs-flex-list"
                >
                  <a-list-item
                    slot="renderItem"
                    slot-scope="item, index"
                    :class="item.id == currentVisitor ? 'chat-list-active' : !item.state ? '' : 'tag-check'"
                    class="chat-list-hover"
                ...
                <div class="tab-box">
                  ...
                  >
                    <a-list-item
                      ...
                      :class="item.visitorId == queueVisitor ? 'chat-list-active' : 'chat-list-hover'"
                      :style="{ borderBottom: '1px solid #f0f0f0' }"
                      ...
                      <a-list-item-meta>
                        <a-row slot="title" type="flex" align="middle">
                          <a-col :span="16" style="display: flex; align-items: center">
                            {{ $t('转接自：') }}
                            <a-badge>
                              <span class="visitor-name" style="font-size: 16px; color: #26262; font-weight: 500">
                                {{ item.transferFrom }}
                              </span>
                            </a-badge>
                            <span style="padding-left: 8px">
                ...
                <div class="tab-box">
                  ...
                  >
                    <a-list-item
                      ...
                      style="padding: 8px 16px"
                      :class="item.conversationId == summarizedVisitor ? 'chat-list-active' : 'chat-list-hover'"
                      :style="{ borderBottom: '1px solid #f0f0f0' }"
                ...
                <div class="tab-box">
                    ...
                    <a-col>
                      ...
                      >
                        <div slot="content" style="width: 240px">
                          ...
                          <div style="text-align: right; margin-top: 8px">
                            <a-space>
                              <a-button size="small" @click="historyFilterVisible = !historyFilterVisible">
                                关闭
                              </a-button>
                              <a-button
                                type="primary"
                  ...
                  >
                    <a-list-item
                      ...
                      style="padding: 8px 16px"
                      :style="{ borderBottom: '1px solid #f0f0f0' }"
                      :class="item.conversationId == historyVisitor ? 'chat-list-active' : 'chat-list-hover'"
                ...
                <div class="tab-box">
                  ...
                  >
                    <a-list-item
                      ...
                      style="padding: 8px 16px"
                      :style="{ borderBottom: '1px solid #f0f0f0' }"
                      :class="item.visitorId == monitorVisitor ? 'chat-list-active' : 'chat-list-hover'"
          ...
          <a-col :span="12" style="border-left: 1px solid #f0f0f0; border-right: 1px solid #f0f0f0">
            <div class="chat-head">
              <a-space style="flex-flow: row wrap">
                ...
                <span class="current-name">{{ chatContent.visitorName }}</span>
                  ...
                  <a-dropdown>
                    <a-menu slot="overlay" style="width: 120px" @click="editServiceStatus">
                      ...
                      <a-sub-menu v-if="settingControl.busyEnable" :key="2">
                        ...
                        <template>
                          <a-menu-item v-for="busyItem in furtherStates" v-show="busyItem.check" :key="busyItem.value">
                            <a-icon type="pause-circle" theme="twoTone" two-tone-color="#FACC14" />
                            {{ busyItem.label }}
                          </a-menu-item>
                        </template>
                    ...
                    <span v-else style="cursor: pointer">
                      <a-icon type="pause-circle" theme="twoTone" two-tone-color="#FACC14" />
                      {{
                        settingControl.busyEnable && furtherStates.find((item) => item.value === serviceStatus)
                          ? furtherStates.find((item) => item.value === serviceStatus).label
                          : $t('示忙')
                      }}
                    </span>
                  </a-dropdown>
            ...
            <a-spin :spinning="loadingContent">
              <div class="chat-content-list-box">
                <div ref="chats" class="chat-content-list" @scroll="popupScroll">
                  <a-spin
                    v-if="
                  ...
                  <div v-if="noGet && !notGetChat" class="no-more" style="padding-top: 10px">
                    {{ $t('--没有更多数据--') }}
                  </div>
                  <div v-for="(item, i) in contentList" :key="i">
                    <!-- 历史消息分割线 -->
                    <div v-if="['1046'].includes(item.code) && !notGetChat" class="system-reminder">
                      <a-divider dashed style="margin: 4px 0px; font-size: 12px; color: #8c8c8c">
                        {{ $t('以上为历史消息') }}
                      </a-divider>
                    </div>
                    <!-- <div v-else-if="['1046'].includes(item.code) && notGetChat" class="system-reminder">
                    ...
                    >
                      ...
                      <div v-else style="display: flex; align-items: center">
                        ...
                        <div class="reply-box" @contextmenu.prevent="rightClick($event, item)">
                          <a-spin
                            :spinning="
                      ...
                      <div class="chat-box">
                        <template
                          v-if="
                            ...
                            item.content.includes('<a target')
                          "
                        >
                          <!-- eslint-disable-next-line vue/no-v-html -->
                          <span v-viewer class="chat-content" v-html="item.content"></span>
                        </template>
                        <span v-else v-viewer v-dompurify-html="item.content" class="chat-content"></span>
                        <a-popover
                ...
                <div v-if="newsTipsShow" class="tips">
                  <div class="tips-box" @click="scrollToBottom">
                    <div class="tips-icon"><a-icon type="double-right" /></div>
                    <span>您有新消息</span>
                  </div>
                </div>
              </div>
                ...
                >
                  <a-spin :spinning="replyLoading">
                    ...
                    >
                      ...
                      <a-list-item v-if="replyContentList.length == replyContentTotal && noReplyMore" class="no-more">
                        <div class="no-more" style="width: 100%">{{ $t('--没有更多数据--') }}</div>
                      </a-list-item>
                    </a-list>
                    <div v-if="replyContentList.length === 0" class="no-more">{{ $t('--没有更多数据--') }}</div>
                  </a-spin>
                </div>
              ...
              <a-tab-pane v-for="tabsItem in tabsList" :key="tabsItem.key" :tab="tabsItem.name">
                <div style="height: 100%">
                  <template v-if="tabsItem.key === '1'">
                    <a-spin :spinning="!chatContent.id && !chatContent.conversationId" style="height: 100%">
                        ...
                        >
                          <a-form :labelCol="{ span: 8 }" :wrapperCol="{ span: 16 }">
                            ...
                            <a-form-item :label="$t('服务订单号')">
                              {{ userMessage.serviceOrderNumber || '--' }}
                            </a-form-item>
                    ...
                    <div style="margin-top: 8px">
                      ...
                      <a-list :data-source="knowledgeBase" class="container" :loading="knowledgeLoading">
                        <a-list-item v-if="!noFind" slot="renderItem" slot-scope="item" style="padding-bottom: 0">
                          <a-row>
                            <a-col type="flex">
                              <span style="color: #333333; font-weight: 600" class="two-line">
                                <a-icon type="file-text" style="color: #1fc1cc; font-size: 14px; padding-right: 4px" />
                                {{ item.title }}
                              </span>
                            </a-col>
                            <a-col style="color: #808080; padding-top: 6px" class="two-line">{{ item.content }}</a-col>
          ...
          <a-spin :spinning="false">
            <a-form :form="transferForm">
              ...
              <a-row type="flex" :gutter="8" style="margin-bottom: 8px">
                <a-col flex="1">
                  <a-input v-model="queryServiceParam.name" type="text" :placeholder="$t('请输入账号搜索')" />
                </a-col>
                <a-col>
...
export default {
  ...
  methods: {
    ...
    // 时间戳转化
    timestampFormat (date, initTime) {
      const m2 = initTime ? moment(initTime) : moment()
    ...
    // 时间戳转化
    timestampFormatM (date, initTime) {
      const m2 = moment(initTime)
      const m1 = moment(date)
      const du = moment.duration(m2 - m1, 'ms')
      ...
      if (days > 0) {
        ...
        return '<span style="color: ' + color + ';">' + sec + this.$t('秒') + '</span>'
      } else {
        return ''
      }
    },
      ...
      this.axios({
        ...
        if (res.code === 0) {
          if (res.result.isService) {
            ...
            this.serviceStatus = res.result.status === 3 ? this.$store.state.app.onlineStatus : res.result.status
            this.settingControl = res.result.info
            this.furtherStates = JSON.parse(this.settingControl.furtherStates)
            this.furtherStates.forEach(item => { item.label = this.$t(item.label) })
            this.getChannal()
    ...
    editServiceStatus (menu) {
      ...
      this.$store.dispatch('SetOnlineStatus', menu.key)
      this.$stompClient.send('/chat/sendToUser', JSON.stringify(data))
    },
    // 取消撤回框
    clickTable () {
    ...
    reCall () {
      const data = {
        ...
        data: {
          ...
          messageId: this.reCallItem.messageId
        }
      }
      this.$stompClient.send('/chat/sendToUser', JSON.stringify(data))
    },
    // 初始webstocket
    initWebSocket () {
      const indexChatMsg = (msg, stompClient) => {
        this.$stompClient = stompClient
        this.infoData = JSON.parse(msg.body)
        // 订阅回调信息不执行
        if (this.infoData.code === '2004') return false
        if (this.infoData.from || (this.infoData.data && this.infoData.data.data && this.infoData.data.data.userId)) {
        ...
        if (this.infoData.code === '1010') {
          if (this.chatContent.visitorId === this.infoData.from.userId) {
            ...
            this.$stompClient.send('/chat/sendToUser', JSON.stringify(msg))

            // 显示有新消息提示
            const { scrollHeight, scrollTop, offsetHeight } = this.$refs.chats
            if (scrollHeight - scrollTop - offsetHeight > 80) {
              this.newsTipsShow = true
            } else {
              this.scrollBottomUpdate = true
            }
          }
          // 访客发送消息，提示声音
          if (this.settingControl.soundReminder && this.settingControl.receiveMessage &&
            this.settingControl.receiveMessageFile && this.settingControl.receiveMessageFile.length > 0) {
            const file = this.settingControl.receiveMessageFile[0] || {}
            this.audioPlay(file)
          }
        } else if (['1011', '1021', '1019'].includes(this.infoData.code)) {
        ...
        // 记录与当前访客的聊天内容
        if (this.groupData && this.groupData.length > 0 && ['1010', '1011', '1021', '1023', '1027', '1019', '1030', '1044', '1002', '1031', '1049', '1050', '1051', '1054', '1055', '1056', '1057', '1059', '1061', '1003'].includes(this.infoData.code)) {
          if (!this.chatContent.state && this.visitorInfo && this.chatContent.visitorId === this.visitorInfo.userId) {
            ...
            if (this.infoData.code === '1011') {
              if (index !== -1) {
                this.contentList[index].loading = null
                this.contentList[index].id = this.infoData.data.messageId
              }
            } else if (this.infoData.code === '1021') {
              this.contentList[index].loading = null
              this.contentList[index].isMark = true
              this.contentList[index].markTips = this.infoData.data.content
              this.contentList[index].id = this.infoData.data.messageId
          ...
          this.groupData.forEach(item => {
            if (!item.state && item.visitorId === this.visitorInfo.userId) {
              ...
              // 客服未读数（角标）
              if ((this.chatContent.id !== item.id || (this.chatContent.id === item.visitorId && this.chatContent.state)) && this.infoData.code === '1010') {
                item.unreadMessage = item.unreadMessage + 1
              }
              // 客服未回复访客时间
              if (this.infoData.code === '1010' && this.infoData.from.userId === item.visitorId && !item.serviceUnansweredTime) {
                item.serviceUnansweredTime = this.infoData.time
              }
        ...
        // 访客接入时播放提示声音
        if (this.infoData.code === '1080') {
          if (this.settingControl.guestAccessFile && this.settingControl.guestAccessFile.length > 0) {
            const file = this.settingControl.guestAccessFile[0] || {}
            this.audioPlay(file)
          }
        } else if (this.infoData.code === '1081') {
          // 收到转接请求时播放提示声音
          if (this.settingControl.transferRequestFile && this.settingControl.transferRequestFile.length > 0) {
            const file = this.settingControl.transferRequestFile[0] || {}
            this.audioPlay(file)
          }
        } else if (this.infoData.code === '1082') {
          // 访客进入排队时播放提示声音
          if (this.settingControl.enterQueueFile && this.settingControl.enterQueueFile.length > 0) {
            const file = this.settingControl.enterQueueFile[0] || {}
            this.audioPlay(file)
          }
        }
    ...
    audioPlay (file) {
      const baseUrl = process.env.NODE_ENV === 'production' ? this.$store.state.env.VUE_APP_API_BASE_URL : 'https://dev.java.doscs.com/fengchao/myapi/'
      const soundSrc = `${baseUrl}admin/api/download/?filePath=${file.filePath}`
      if (file.filePath) {
        if (this.audio !== null) {
          this.audio.pause()
          this.audio = null
    ...
    changeTime (initTime) {
      ...
      this.groupData = this.groupData.map(item => {
        item.formatTime = this.timestampFormat(Date.parse(item.lastTime), initTime)
        return item
      })
      if (this.groupData.length > 0) {
        this.changeTimeOut = setTimeout(() => {
          const newInitTime = initTime + 60000
          this.changeTime(newInitTime)
        }, 60000)
      }
    },
    getCheckTime (initTime) {
      clearTimeout(this.checkTimeOut)
      this.groupData = this.groupData.map(item => {
        const time = item.serviceUnansweredTime
        item.checkTime = time ? this.timestampFormatM(Date.parse(time), initTime) : ''
        return item
      })
      if (this.groupData.length > 0) {
        this.checkTimeOut = setTimeout(() => {
          const newInitTime = initTime + 1000
          this.getCheckTime(newInitTime)
        }, 1000)
      }
    },
    // 控制不能发送空白消息的气泡显示
    ...
    getServiceList (parameter) {
      ...
      return this.axios({
        url: '/chat/wsService/transferInit',
        data: data
      }).then(res => {
        return res.result
      })
      ...
      this.axios({
        ...
        if (res.code === 0) {
          ...
          this.groupData.forEach(item => {
            ...
            if (this.chatContent.visitorId === item.visitorId) {
              item.unreadMessage = 0
            }
          })
          this.groupTotal = res.result.totalCount
          this.groupLoading = false
          this.changeTime(res.timestamp)
          this.getCheckTime(res.timestamp)
          if (record && record.id) {
    ...
    grabTrasfer (item, status) {
      ...
      this.axios({
        url: '/chat/wsService/transferListConfirm',
        data: data
      }).then(res => {
        if (res.code === 0) {
    ...
    eventClaim (record) {
      ...
      this.$stompClient.send('/chat/sendToUser', JSON.stringify(data))
      // this.axios({
      //   url: '/chat/wsService/grabVisitor',
      //   data: data
      // }).then(res => {
      //   if (res.code !== 0) {
      ...
      this.$confirm({
        ...
        onOk: () => {
          if (type === 'end') {
            ...
            that.axios({
              url: '/chat/wsService/enforceTransfer',
              data: data
            }).then(res => {
              if (res.code === 0) {
      ...
      this.transferRemarkForm.validateFields((errors, values) => {
        if (!errors) {
          ...
          if (that.settingControl.conversationTransferConfirm) {
            that.axios({
              url: '/chat/wsService/serviceTransfer',
              data: {
    ...
    getChatSummarized (record, type) {
      ...
      this.axios({
        url: '/chat/wsService/chatRecords',
        data: params
      }).then(res => {
    ...
    getChatHistory (record) {
      ...
      this.sending = false
      this.visitorName = record.visitorName
      this.historyVisitor = record.conversationId
      this.pageContent.pageNo = 1
      this.disableChat = true
      this.noMore = false
    ...
    getChat (record, index) {
      ...
      const url = '/chat/wsService/chatRecords'
      this.axios({
        url: url,
        data: params
      }).then(res => {
    ...
    popupScroll (e) {
      ...
      if (e) {
        ...
        } else if (this.newsTipsShow) {
          const { offsetHeight } = e.target
          if (height - scrollTop - offsetHeight < 80) {
            this.newsTipsShow = false
          }
        }
      }
    ...
    popupReplyScroll (e) {
      if (e) {
        if (e.target.scrollTop === (e.target.scrollHeight - 200) && this.scrollReply) {
          ...
          const content = document.getElementsByClassName('send-textarea')[0].value
      ...
      if (this.modal === 'blacklist') {
        ...
        validateFields((errors, values) => {
          if (!errors) {
            ...
            this.axios({
              url: '/chat/wsService/addBlacklist',
              data: data
            }).then(res => {
              if (res.code === 0) {
                this.$message.success(res.message)
    ...
    sendNews (content, code, image, toVisitor, data, file) {
      ...
      if (!string.replace(/(\r\n)|\n|\s|&nbsp;|<br\/>/g, '') && code === '1011') {
        ...
        this.content = ''
        this.scrollBottomUpdate = true
        this.$stompClient.send('/chat/sendToUser', JSON.stringify(title))
        // this.axios({
    ...
    wsSend () {
      ...
      this.$stompClient.send('/chat/sendToUser', JSON.stringify(msgWs))
    },
    // 监听数据框的输入值
    ...
    // 表情转化
    emojiInsert (word) {
      const myField = document.getElementsByClassName('send-textarea')[0]
    ...
    checkBlacklist () {
      this.axios({
        url: '/chat/wsService/getBlackRemark',
    ...
    showConfirm () {
      ...
      this.$confirm({
        ...
        onOk () {
          that.axios({
            url: '/chat/wsService/sendEvaluation',
    ...
    showTips (item) {
      this.$warning({
        title: '提示',
        content: item.markTips || '您发送的信息中包含违禁词，无法发送'
      })
    }
  }
}
