      ...
      <a-space direction="vertical" style="width: 100%">
        <a-card size="small" :title="$t('基础设置')">
          ...
          <a-form-item :wrapperCol="{ span: 2 }">
            ...
            <a-select v-decorator="['viewFontSize', { initialValue: data.viewFontSize || '15' }]">
              <a-select-option value="13">{{ $t('小') }}</a-select-option>
              <a-select-option value="15">{{ $t('正常') }}</a-select-option>
              <a-select-option value="17">{{ $t('大') }}</a-select-option>
              <a-select-option value="19">{{ $t('超大') }}</a-select-option>
            </a-select>
          </a-form-item>
        </a-card>
        ...
        <a-form :form="modalform" :labelCol="{ span: 4 }" :wrapperCol="{ span: 20 }">
          ...
          <a-form-item :label="$t('表单视图')">
            <a-select
              ...
              :filter-option="filterOption"
            >
              <a-select-option v-for="view in viewList" :key="view.templateId">{{ view.name }}</a-select-option>
            </a-select>
          </a-form-item>
...
export default {
  ...
  data () {
    return {
      ...
      summaryColumns: [{
        ...
        customRender: (text, record) => {
          const view = this.viewList.find(item => item.templateId === text)
  ...
  created () {
    this.axios({
      url: '/admin/general/tableTplviews',
      data: {
  ...
  methods: {
    handlePriv () {
      const privArr = this.data.allTabsPriv ? JSON.parse(this.data.allTabsPriv) : []
