<template>
  <div>
    <a-form :form="form">
      ...
      <a-form-item :label="$t('地址搜索')" :labelCol="labelCol" :wrapperCol="wrapperCol">
        <a-switch
          v-decorator="[
            'setting[form][coordinateShow]',
            { initialValue: setting.form.coordinateShow || false, valuePropName: 'checked' }
          ]"
          :un-checked-children="$t('关')"
          :checked-children="$t('开')"
      ...
      <a-form-item v-if="coordinateShow" :labelCol="{ span: 8 }" :wrapperCol="{ span: 16 }">
        <span slot="label">
          {{ $t('地址坐标回写字段') }}
          <a-tooltip :title="$t('只能回写到单行文本。回写目的字段不要设置到表单视图中')">
            <a-icon type="question-circle" />
        ...
        <a-select
          v-decorator="[
            'setting[form][coordinate]',
            {
              initialValue: setting.form.coordinate || false,
              rules: [{ required: false, message: $t('请选择地址坐标回写字段') }]
            }
          ]"
          allowClear
        >
  ...
  watch: {
    setting: {
      handler (newVal) {
        ...
        this.coordinateChange(this.data.form.coordinateShow)
