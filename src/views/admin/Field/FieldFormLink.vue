<template>
  ...
  >
    <a-spin :spinning="loading">
      <a-form :form="form">
        <a-form-item :label="$t('联动数据表')" :wrapperCol="wrapperCol">
          <a-cascader
        ...
        <a-form-item validate-status="success" :label="$t('数据联动')" :wrapperCol="wrapperCol">
          <a-select
            v-decorator="[
              'link[currentValue]',
              {
                initialValue: link.currentValue || undefined,
                rules: [{ required: true, message: $t('请选择当前表字段') }]
              }
            ]"
          ...
          <a-select
            ...
            style="width: 200px; margin-right: 10px"
          >
            <a-select-option value="key">{{ $t('当前子表单名称') }}</a-select-option>
          </a-select>
          {{ $t('联动显示') }}
          <a-select
            v-decorator="[
