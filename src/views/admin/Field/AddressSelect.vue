    ...
    <a-spin :spinning="loading">
      <a-form :form="form">
        <a-popover v-model="selectShow" :trigger="fieldRule === 'readonly' ? '' : 'click'" placement="bottomLeft">
          ...
          <div
            v-show="!showInput"
            style="display: flex; align-items: center; margin-right: 8px; margin-top: 2px"
            @click.stop="changeStatus"
          >
            <span>{{ cityName }}</span>
            <a-icon v-show="!selectShow" type="down" />
            <a-icon v-show="selectShow" type="up" />
          </div>
...
export default {
  ...
  methods: {
    ...
    popupScroll (e) {
      ...
      if (scrollBottom < 1 && this.page.pageNo * this.page.pageSize < this.totalCount && this.searchValue) {
        this.page.pageNo++
