                            ...
                            fieldArr.forEach((item1) => {
                              if (item1.alias === value) {
                                ...
                                if (simpleItem.src && item1.formType === 'treeselect') {
                                  ...
                                  getTreeOption(simpleItem, item1)
                                } else if (item1.formType === 'switch') {
                                  simpleItem.option = [
                                    { label: item1.settings.form.word.value[0], value: 0 },
                                    { label: item1.settings.form.word.value[1], value: 1 }
                                  ]
                                } else {
                                  simpleItem.option = []
                                }
                                ...
                                } else if (['switch'].indexOf(item1.formType) !== -1) {
                                  simpleItem.conditionArr = [
                                    { enName: 'eq', cnName: '等于' },
                                    { enName: 'ne', cnName: '不等于' }
                                  ]
                        ...
                        >
                          ...
                          <a-col v-if="simpleItem.fieldUser === 'field'" :span="14">
                            <template v-if="!includeItem.type">
                              ...
                              <a-select
                                v-else-if="
                                  ['combobox', 'switch'].includes(simpleItem.formType) &&
                                  simpleItem.option &&
                              ...
                              <div v-else-if="simpleItem.formType === 'cascader'">
                                <tabs-select
                                  ...
                                  :value="
                                    simpleItem.option
                                      ? simpleItem.option.filter((item) => item.dictDataNumber === includeItem.value)
                                      : []
                                  "
                                  @send="
                              ...
                              <a-input
                                ...
                                v-model="includeItem.value"
                                size="small"
                                :disabled="includeItem.include === 'em'"
                              />
                            </template>
                            <template v-else>
...
export default {
          ...
          this.fieldArr.forEach((fieldItem, fieldIndex) => {
            if (simpleItem.field === fieldItem.alias) {
              ...
              if (simpleItem.src && fieldItem.formType === 'treeselect') {
                ...
                this.getTreeOption(simpleItem, fieldItem)
              } else if (fieldItem.formType === 'switch') {
                simpleItem.option = [
                  { label: fieldItem.settings.form.word.value[0], value: 0 },
                  { label: fieldItem.settings.form.word.value[1], value: 1 }
                ]
              } else {
                simpleItem.option = []
              }
  ...
  methods: {
    ...
    getConditionArr (fieldItem, simpleItem) {
      if (['text', 'textarea', 'serialnumber', 'associated'].indexOf(fieldItem.formType) !== -1) {
        simpleItem.conditionArr = [
          { enName: 'eq', cnName: '等于' },
          { enName: 'ne', cnName: '不等于' },
          { enName: 'cn', cnName: '包含' },
        ...
        simpleItem.conditionArr = [
          ...
          { enName: 'em', cnName: '为空' },
          { enName: 'nem', cnName: '不为空' }]
      } else if (['switch'].indexOf(fieldItem.formType) !== -1) {
        simpleItem.conditionArr = [
          { enName: 'eq', cnName: '等于' },
          { enName: 'ne', cnName: '不等于' }
        ]
      ...
      if (this.myCondition.flowData && this.myCondition.currentSelect && this.myCondition.currentSelect.type === 'link') {
        ...
        if (currentSelect.source.type === 'user_transition') {
          this.axios({
            ...
            if (!res.code) {
              const setting = res.result.nodeSetting
