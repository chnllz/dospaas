  ...
  methods: {
    showAdd (config) {
      ...
      this.axios({
        ...
        this.data = {
          parentDictDataNumber: config.record && config.record.fullDictDataNumber ? config.record.fullDictDataNumber.split('/') : null,
    ...
    changeName (e) {
      if (!this.editable || this.config.action === 'add') {
        const val = this.fixVal(e.target.value)
        if (val) {
          this.setField(val)
        }
      }
      ...
      validateFields((errors, values) => {
        if (!errors) {
          ...
          if (this.userInfo.superAdmin) {
            data.accessLevel = values.accessLevel
          }
