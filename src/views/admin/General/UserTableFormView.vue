<template>
  ...
  <a-spin :spinning="pageLoading">
          ...
          <a-textarea
            v-decorator="[
              'handleRemarks',
              {
                ...
                rules: [{ required: params.remarksRule === 'show_allow', message: $t('办理备注不能为空') }]
              }
            ]"
            :autoSize="{ minRows: params.remarksMinRows || 4, maxRows: params.remarksMaxRows || params.remarksMinRows }"
          />
        </a-form-item>
      </a-row>
    </a-form>
    ...
    <!-- 快递流程查看 -->
    <ExpressCheck ref="expressCheck"></ExpressCheck>
  </a-spin>
</template>
<script>
...
export default {
  ...
  components: {
    ...
    ExpressCheck: () => import('../ExpressCheck'),
    UserTableComponents
  },
  props: {
    params: {
  ...
  data () {
    return {
      ...
      data: [],
      addressValue: {},
      cascadeAllvalue: {}, // 级联选择allValue存储
  ...
  watch: {
    params: {
      handler (newValue) {
        const val = newValue
        this.action = val.parentParams.action
        ...
        const getTemplate = (array) => {
          array.forEach((item, index) => {
            if (item.columns) {
              ...
              if (item.field && item.field.formType === 'address' && item.field) {
        ...
        this.handleFormRules = val.handleFormRules
        this.defaultWay = ''
        this.handleRemarks = ''
        if (val.handleWayData && val.handleFormRules) {
  ...
  mounted () {
    ...
    window._AMapSecurityConfig = {
      securityJsCode: '5cb15b4cc01c4bdbd37e61ccfa305ae0'
    }
    VueAMap.initAMapApiLoader({
      key: '97108600249b089ad8952ece0b1d6291', // 高德开发者key
      plugin: ['ToolBar', 'Autocomplete', 'AMap.Geocoder'], // 'AMap.Autocomplete', 'AMap.Geocoder', 'AMap.Geolocation', 'ToolBar', 'MapType'
      v: '1.4.4'
    })
    Vue.use(BaiduMap, {
      ak: '75w6UjsySnMwziTzMBLySL9GMKa3a5H3'
    })
  },
  methods: {
    ...
    giveData (obj, addressValue, alias) {
      if (addressValue) {
        ...
        const setObj = {}
        for (const key in oldData.record) {
    ...
    getRule (array, eventItem) {
      array.forEach((item, index) => {
        if (item.columns) {
          this.getRule(item.columns, eventItem)
        } else if (item.trs) {
          this.getRule(item.trs, eventItem)
        } else if (item.tds) {
          this.getRule(item.tds, eventItem)
        } else if (item.list) {
          this.getRule(item.list, eventItem)
        } else {
          if (item.field && item.field.alias === eventItem.field) {
            if (eventItem.status === 'changeValue') {
              const obj = {}
              const name = 'record' + '[' + eventItem.field + ']'
              obj[name] = eventItem.changeValue
              if (eventItem.formType === 'address' || eventItem.formType === 'cascader' || eventItem.formType === 'treeselect') {
                const getArray = (array) => {
                  array.forEach((item, index) => {
                    if (item.columns) {
                      ...
                      if (item.field && item.field.alias === eventItem.field) {
                        if (eventItem.formType === 'address') {
                          this.form.setFieldsValue(obj)
                          this.$set(item.field, 'value', eventItem.changeValue)
                        } else if (eventItem.formType === 'treeselect') {
                          ...
                          this.form.setFieldsValue(obj)
                          this.$set(item.field, 'value', { value: eventItem.changeValue })
                        }
                      }
                    }
                  })
                ...
                this.form.setFieldsValue(obj)
                this.changeFieldValue(this.template, eventItem.field, eventItem.changeValue)
              }
            } else if (eventItem.status === 'optionalRange') {
              // 单选，下拉，多选，使用
              if (['radio', 'checkbox', 'combobox'].includes(item.field.formType)) {
                item.field.option = item.field.option.map(im => {
                  let choiceOption = eventItem.choiceOption || []
                  choiceOption = eventItem.choiceOption.map(choiceItem => choiceItem.value)
                  im.disabled = !choiceOption.includes(im.value)
                  return im
                })
              } else {
                this.$set(item.field, 'optionalRange', eventItem.choiceOption)
              }
            } else if (eventItem.status === 'notOptionalRange') {
              if (['radio', 'checkbox', 'combobox'].includes(item.field.formType)) {
                item.field.option = item.field.option.filter(im => {
                  let choiceOption = eventItem.choiceOption || []
                  choiceOption = eventItem.choiceOption.map(choiceItem => choiceItem.value)
                  im.disabled = choiceOption.includes(im.value)
                  return im
                ...
                this.$set(item.field, 'notOptionalRange', eventItem.choiceOption)
              }
            } else {
              eventItem.mandatory = eventItem.status === 'hidden' ? false : eventItem.mandatory
              item.field.setting.attribute.required = eventItem.mandatory === 'true'
              if (eventItem.titleField) {
                item.field.name = eventItem.changeTitle
              }
              item.fieldRule = eventItem.status
            }
          }
        }
      })
    ...
    getRecovery (array, tem) {
      array.forEach((item, index) => {
        if (item.columns) {
          ...
          if (item.field && tem.length > 0 && tem.indexOf(item.field.alias) === -1) {
    ...
    // 子表change事件触发
    sendSubTableChange (record, val, subThis, changeType) {
      const attribute = record.attribute
      if (attribute) {
        const str = 'return ' + attribute
        // eslint-disable-next-line no-new-func
        const func = new Function(str)
        func()(this, val, subThis, changeType)
      }
    },
    // 事件触发实现
    ...
    handleWayRule () {
      const that = this
      this.handleFormRules.forEach((dataItem, dataIndex) => {
    ...
    handleWayProRule (value) {
      ...
      this.handleWayData.forEach(item => {
        if (item.id === value && this.handleFormRules) {
          this.handleFormRules.forEach(wayItem => {
            const simpleFlagArr = []
    ...
    handleSimple (include, value, inputValue, allValue) {
      ...
      return action && action(value, inputValue, allValue)
    },
    // 数组对象排序
    compare (property) {
      return (obj1, obj2) => {
    ...
    filterSubForm (array) {
      array.forEach((item) => {
        if (item.columns && item.columns.length) {
          if (item.columns.every(chiItem => ['formmark', 'web_sub_data_window'].includes(chiItem.type) || chiItem.filterStatus)) {
            item.filterStatus = true
          }
          this.filterSubForm(item.columns)
        } else if (item.trs && item.trs.length) {
          if (item.trs.every(chiItem => ['formmark', 'web_sub_data_window'].includes(chiItem.type) || chiItem.filterStatus)) {
            item.filterStatus = true
          }
          this.filterSubForm(item.trs)
        } else if (item.tds && item.tds.length) {
          if (item.tds.every(chiItem => ['formmark', 'web_sub_data_window'].includes(chiItem.type) || chiItem.filterStatus)) {
            item.filterStatus = true
          }
          this.filterSubForm(item.tds)
        } else if (item.list && item.list.length) {
          if (item.list.every(chiItem => ['formmark', 'web_sub_data_window'].includes(chiItem.type) || chiItem.filterStatus)) {
            item.filterStatus = true
          }
    ...
    handleSubmit (data = {}, page, jointable, type) {
      let result
      if (['onlySave', 'onlySaveClose'].includes(type)) {
        result = this.getOnlySaveValue(data, page, jointable)
      } else {
        result = this.getValueData(data, page, jointable)
      }
    ...
    valueTrim (data, objKey) {
      for (const key in data) {
        if (typeof data[key] === 'string') {
          ...
          if (objKey === 'record' && this.templateObj[key] && ['text', 'textarea'].includes(this.templateObj[key].formType)) {
            const obj = {}
      ...
      this.form.validateFields((errors, values) => {
        if (!errors) {
          ...
          const getTemplate = (array) => {
            ...
            array.forEach((item, index) => {
              if (item.columns) {
                ...
                if (item.field && (item.field.formType === 'file' || item.field.formType === 'image') && item.fieldRule !== 'disabled') {
                ...
                } else if (item.field && item.field.formType === 'datetime' && item.field.fieldType === 'TIME' && item.fieldRule !== 'disabled') {
                  if (item.field.beforeAlias) {
                ...
                } else if (item.field && item.field.formType === 'datetime' && item.fieldRule !== 'disabled') {
                  if (item.field.beforeAlias) {
                ...
                } else if (item.field && item.field.formType === 'switch' && item.fieldRule !== 'disabled') {
                  if (item.field.beforeAlias) {
                ...
                } else if (item.field && item.field.formType === 'autocomplete' && item.fieldRule !== 'disabled') {
                  if (item.field.beforeAlias) {
                ...
                } else if (item.field && item.field.formType === 'address' && item.fieldRule !== 'disabled') {
                  if (item.field.beforeAlias) {
                    ...
                    const address = 'record' + '[product_' + item.aliasKey + ']' + '[' + item.field.beforeAlias + ']'
                    values['record']['product_' + item.aliasKey][item.field.beforeAlias] = this.addressValue[address] || ''
                  } else {
                    if (!values['record'][item.field.alias]) {
                      values['record'][item.field.alias] = {}
                    }
                    const address = 'record' + '[' + item.field.alias + ']'
                    values['record'][item.field.alias] = this.addressValue[address] || ''
                  }
                } else if (item.field && item.field.formType === 'treeselect' && values['record'][item.field.alias] && item.fieldRule !== 'disabled') {
                  if (item.field.beforeAlias) {
                ...
                } else if (item.field && item.field.formType === 'json' && item.fieldRule !== 'disabled') {
                  if (values.record[item.field.alias]) {
                    try {
                      const str = 'return ' + values.record[item.field.alias]
                      // eslint-disable-next-line no-new-func
                      const func = new Function(str)
                      values.record[item.field.alias] = func()
                    } catch (err) {
                      console.error(err)
                      bool = true
                    }
                ...
                if (item.type === 'web_sub_form_view') {
                  const getSubForm = (array) => {
                    array.forEach(subItem => {
                      if (subItem.currentRecord && subItem.currentRecord.key === item.key) {
                        const value = subItem.handleSubmit()
                        if (value === false) {
                          this.$message.warning(this.$t('请按要求填写子表单'))
                          throw new Error('请按要求填写子表单')
                        }
                        values.subformRecords.push(value)
          ...
          if (page !== 'web_sub_data_window') {
            ...
            // 对工作流的action进行处理 start
            // 流程创建 action 为 add
            if (me.parentParams.viewType === 'create') {
              action = 'add'
            } else if (this.action === 'get') { // 弹出流程的表单视图时，params.parentParams.action 赋值为get，具体见 WorkflowHandleForm.vue的requestData方法
              action = 'edit' // 非流程创建 action 为 edit
            }
            // 对工作流的action进行处理 end
            ...
            if (me.parentParams.templateId) {
              valueData.templateId = me.parentParams.templateId
            }
            ...
            if (me.parentParams.workitemId) {
              valueData.workitemId = me.parentParams.workitemId
            }
            ...
            if (me.formThis && me.formThis.listThis && me.formThis.listThis.interactiveType === 'tree') {
    ...
    getOnlySaveValue (data = {}, page, jointable) {
          ...
          if (item.columns) {
            ...
            if (item.type === 'web_sub_form_view') {
              const getSubForm = (array) => {
                array.forEach(subItem => {
                  if (subItem.currentRecord && subItem.currentRecord.key === item.key) {
                    const value = subItem.handleOnlySave()
                    if (values) {
                      values.subformRecords.push(value)
                    }
                  } else if (subItem.$refs && subItem.$refs.nestedComponents) {
      ...
      if (page !== 'web_sub_data_window') {
        ...
        // 对工作流的action进行处理 start
        // 流程创建 action 为 add
        if (this.parentParams.viewType === 'create') {
          action = 'add'
        } else if (this.action === 'get') { // 弹出流程的表单视图时，params.parentParams.action 赋值为get，具体见 WorkflowHandleForm.vue的requestData方法
          action = 'edit' // 非流程创建 action 为 edit
        }
        ...
        const valueData = this.$deepmerge(values, {
          action,
          id: this.parentParams.record ? this.parentParams.record.id : 0
        })
        if (data.extFormData) {
          for (const i in data.extFormData) {
            if (i === 'record') {
        ...
        if (this.parentParams.templateId) {
          valueData.templateId = this.parentParams.templateId
        }
        if (this.parentParams.tableId) {
          valueData.tableId = this.parentParams.tableId
        }
        if (this.parentParams.workflowId) {
          valueData.workflowId = this.parentParams.workflowId
        }
        if (this.parentParams.caseId) {
          valueData.caseId = this.parentParams.caseId
        }
        if (this.parentParams.workitemId) {
          valueData.workitemId = this.parentParams.workitemId
        }
        if (this.parentParams.operation) {
          valueData.operation = this.parentParams.operation
        }
        if (this.parentParams.flowCondition) {
          valueData.flowCondition = this.parentParams.flowCondition
        }
        valueData.onlySave = true
        if (this.formThis && this.formThis.listThis && this.formThis.listThis.interactiveType === 'tree') {
          const obj = this.formThis.listThis
          Object.assign(valueData.record, { [obj.interactiveAlias]: obj.interactiveObj[obj.interactiveAlias] ? obj.interactiveObj[obj.interactiveAlias][0] : null })
        }
        result = { values, valueData }
        this.valueTrim(valueData)
