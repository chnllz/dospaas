  ...
  <div>
    <a-card size="small">
      ...
      <div class="table-operator">
        <a-button v-if="tbarAbb" icon="plus" type="primary" @click="getData">{{ $t('添加') }}</a-button>
        <a-button v-if="tbarDel" icon="delete" type="danger" :disabled="selectedRowKeys.length == 0" @click="onDelete">
          {{ $t('批量删除') }}
        </a-button>
        <!-- 下面是动态生成区域 -->
              ...
              <div :key="item.dataIndex">
                <a-form-item v-if="!loading">
                  ...
                  <a-input
                    ...
                    v-decorator="[
                      item.dataIndex + record.sort,
                      {
                        initialValue: text,
                        rules: [
                          {
                            required:
                              item.rule === 'required' ? true : templateObj[item.dataIndex].setting.attribute.required,
                            message: $t('{0}不能为空', { 0: item.title })
                          },
                          {
                  ...
                  <a-textarea
                    ...
                    :placeholder="templateObj[item.dataIndex].setting.attribute.emptyText"
                    :autoSize="{
                      minRows: templateObj[item.dataIndex].setting.attribute.minRows || 1,
                      maxRows:
                        templateObj[item.dataIndex].setting.attribute.maxRows ||
                        templateObj[item.dataIndex].setting.attribute.minRows
                    }"
                    @change="(e) => onChange(record, item.dataIndex, item.type, e)"
                  />
                  <!-- 数字 -->
                  <a-input-number
                          ...
                          },
                          {
                            validator: (rule, value, callback) => {
                              if (item.attribute) {
                                ...
                                if (!str) {
                                  if (
                                    templateObj[item.dataIndex].setting.attribute.minValue &&
                                    value < Number(templateObj[item.dataIndex].setting.attribute.minValue)
                                  ) {
                                    const string = $t('{0}不能小于{1}', {
                                      0: item.title,
                                      1: templateObj[item.dataIndex].setting.attribute.minValue
                                    })
                                    callback(string)
                                  } else if (
                                    templateObj[item.dataIndex].setting.attribute.maxValue &&
                                    value > Number(templateObj[item.dataIndex].setting.attribute.maxValue)
                                  ) {
                                    const string = $t('{0}不能大于{1}', {
                                      0: item.title,
                                      1: templateObj[item.dataIndex].setting.attribute.maxValue
                                    })
                                    callback(string)
                                  } else {
                                    callback()
                    ...
                    :formatter="
                      (value) =>
                        `${templateObj[item.dataIndex].setting.form.prefixVal || ''}${value}${
                          templateObj[item.dataIndex].setting.form.suffixVal || ''
                        }`
                    "
                    :parser="
                      (value) => {
                        let latter = templateObj[item.dataIndex].setting.form.suffixVal
                          ? templateObj[item.dataIndex].setting.form.suffixVal.split('')
                          : []
                        ...
                        value = value
                          .replace(lastLatter, '')
                          .replace(firstLatter, '')
                          .replace(templateObj[item.dataIndex].setting.form.prefixVal, '')
                        return value
                      }
                    "
                    size="small"
                    ...
                    @change="
                      (value) => {
                        onChange(record, item.dataIndex, item.type, value)
                      }
                    "
                  />
                  <!-- 日期 -->
                  <a-date-picker
                    v-else-if="
                      item.type === 'datetime' && templateObj[item.dataIndex].setting.attribute.format === 'Y-m-d H:i:s'
                    "
                    v-decorator="[
                      item.dataIndex + record.sort,
                      {
                    ...
                    @change="
                      (value1, value2) => {
                        onChange(record, item.dataIndex, item.type, value1, value2)
                      }
                    "
                  />
                  <a-date-picker
                    v-else-if="
                      item.type === 'datetime' && templateObj[item.dataIndex].setting.attribute.format === 'Y-m-d'
                    "
                    v-decorator="[
                      item.dataIndex + record.sort,
                      {
                  ...
                  <a-time-picker
                    v-else-if="
                      item.type === 'datetime' && templateObj[item.dataIndex].setting.attribute.format === 'H:i:s'
                    "
                    v-decorator="[
                      item.dataIndex + record.sort,
                      {
                          ...
                          },
                          {
                            validator: (rule, value, callback) => {
                              ...
                              if (info.form.minType === 'field') {
                                ...
                                minValue = form.getFieldValue('record' + '[' + alias + ']')
                              ...
                              if (info.form.maxType === 'field') {
                                ...
                                maxValue = form.getFieldValue('record' + '[' + alias + ']')
                  ...
                  <div v-if="item.type === 'address' && !loading">
                    <a-input
                      v-show="false"
                      v-decorator="[
                        item.dataIndex + record.sort + '[address]',
                        {
                          ...
                          rules: [
                            {
                              required:
                                item.rule === 'required'
                                  ? true
                                  : templateObj[item.dataIndex].setting.attribute.required,
                              message: $t('{0}不能为空', { 0: item.title })
                            }
                          ]
                        }
                    ...
                    <a-form-item v-show="false">
                      <a-input
                        ...
                        v-decorator="[
                          item.dataIndex + record.sort + 'reqSeries',
                          {
                            initialValue: templateObj[item.dataIndex].setting.form.reqSeries || ''
                          }
                        ]"
                      />
                    </a-form-item>
                    <a-input-group compact>
                      <address-select
                        ...
                        :series="templateObj[item.dataIndex].setting.form.showSeries"
                        ...
                        @send="getAddress"
                      />
                      <!-- :style="{ width:templateObj[item.dataIndex].setting.form.detailsShow === '1' ? '25%' : '100%'}"
                       <a-form-item v-if="templateObj[item.dataIndex].setting.form.detailsShow === '1'" :style="{width: templateObj[item.dataIndex].setting.form.detailsShow === '1' ? '75%' : '0%' }">
                        <a-input
                  ...
                  <template v-if="item.type === 'treeselect' && !loading">
                    <a-tree-select
                      ...
                      v-decorator="[
                        ...
                        {
                          initialValue:
                            record[item.dataIndex] && record[item.dataIndex].length > 0
                      ...
                      :placeholder="templateObj[item.dataIndex].setting.attribute.emptyText || $t('请选择')"
                      ...
                      :load-data="
                        (data) => {
                          return onLoadData(data, templateObj[item.dataIndex])
                        }
                      "
                      :show-checked-strategy="
                        templateObj[item.dataIndex].setting.attribute.inherit === '1' ? SHOW_PARENT : SHOW_CHILD
                      "
                      @change="
                        (value) => {
                          onChange(record, item.dataIndex, item.type, value)
                    ...
                    >
                      <a-icon
                        v-if="templateObj[item.dataIndex].setting.form.suffixIcon === 'custom'"
                        slot="suffixIcon"
                        :type="templateObj[item.dataIndex].setting.form.suffixVal.type"
                      />
                      <span
                        v-else-if="templateObj[item.dataIndex].setting.form.suffixIcon === 'string'"
                        slot="suffixIcon"
                      >
                        {{ templateObj[item.dataIndex].setting.form.suffixVal.type }}
                      </span>
                    </a-tree-select>
                    <a-tree-select
                      v-else
                  ...
                  <div v-if="item.type === 'cascader' && !loading">
                    ...
                    <tabs-select
                      ...
                      @send="
                        (val, alias) => {
                          getcascaderValue(val, alias, record.sort)
                        }
                      "
                    />
                  </div>
                  ...
                  <a-select
                    v-if="
                      item.type === 'combobox' &&
                      templateObj[item.dataIndex].setting.attribute.dataSource !== 'dictionaryBack'
                    "
                    v-decorator="[
                      item.dataIndex + record.sort,
                      {
                    ...
                    size="small"
                    :disabled="item.rule === 'readonly'"
                    :placeholder="templateObj[item.dataIndex].setting.attribute.emptyText || $t('请选择下拉数据')"
                  ...
                  >
                    <a-select-option
                      v-for="(select, selectIndex) in templateObj[item.dataIndex].option"
                      :key="selectIndex"
                      :value="select.value"
                    >
                      {{ select.label }}
                  ...
                  <template v-else-if="item.type === 'checkbox' && !loading">
                    <div
                      v-if="templateObj[item.dataIndex].setting.attribute.checkAll === '1'"
                      :style="{ borderBottom: '1px solid #E9E9E9', marginBottom: '2px' }"
                    >
                      <a-checkbox
                        :disabled="item.rule === 'readonly'"
                        :indeterminate="
                          templateObj[item.dataIndex][record.sort]
                            ? templateObj[item.dataIndex][record.sort].indeterminate
                            : false
                        "
                        :checked="
                          templateObj[item.dataIndex][record.sort]
                            ? templateObj[item.dataIndex][record.sort].checkAll
                            : false
                        "
                        @change="
                          (e) => {
                            onCheckAll(e, templateObj[item.dataIndex], record, item.dataIndex)
                          }
                        "
                      >
                        {{ $t('全选') }}
                    ...
                    <a-checkbox-group
                      ...
                      @change="
                        (value) => {
                          onCheck(value, templateObj[item.dataIndex], record, item.dataIndex)
                        }
                      "
                    >
                      <a-checkbox
                    ...
                    <div style="position: relative; width: 100%">
                      <template v-if="templateObj[item.dataIndex].setting.form.mode === 'combobox'">
                        <a-input
                          ...
                          @click="
                            handleLink(
                              templateObj[item.dataIndex],
                              templateObj[item.dataIndex].setting.form.mode,
                              record
                            )
                          "
                        />
                        <a-icon
                          v-show="!templateObj[item.dataIndex].iconFlag"
                          type="down"
                          style="
                            position: absolute;
                            top: 8px;
                      ...
                      <a-input
                        ...
                        size="small"
                        @click="
                          handleLink(templateObj[item.dataIndex], templateObj[item.dataIndex].setting.form.mode, record)
                        "
                      >
                        <a-icon
                          ...
                          slot="addonAfter"
                          type="search"
                          @click="handleWinlist(templateObj[item.dataIndex], record)"
                        ></a-icon>
                      </a-input>
                      <div
                        v-if="templateObj[item.dataIndex].iconFlag"
                        style="position: absolute; left: 0; top: 32px; width: 100%; z-index: 111"
                      >
                        <a-select
                          ref="select"
                          show-search
                          ...
                          @search="
                            (val) => {
                              handleSearch(val, templateObj[item.dataIndex], record)
                            }
                          "
    ...
    <!-- 导入表单 -->
    <general-export ref="generalexport"></general-export>
...
export default {
      ...
      compareActions: new Map([[
        'eq', (value, ele, record) => {
          ...
          if (alias) {
            if (value === Number(record[alias[0]])) {
              ...
              return this.$t('字段不能不等于') + this.templateObj[alias[0]].name
            }
          } else if (number) {
            if (value !== number[0]) {
              return this.$t('字段不能不等于') + number[0]
            }
          }
        }
      ], [
        'ne', (value, ele, record) => {
          ...
          if (alias) {
            if (value !== Number(record[alias[0]])) {
              ...
              return this.$t('字段不能等于') + this.templateObj[alias[0]].name
            }
          } else if (number) {
            if (value === number[0]) {
              return this.$t('字段不能等于') + number[0]
            }
          }
        }
      ], [
          ...
          if (alias) {
            if (String(record[alias[1]]) !== 'undefined') {
              if (value <= Number(record[alias[0]]) || value <= Number(record[alias[1]])) {
                const message = this.$t('字段不能小于等于{0}或{1}', { 0: this.templateObj[alias[0]].name, 1: this.templateObj[alias[1]].name })
                return message
              } else {
                return undefined
              }
              ...
              if (value <= Number(record[alias[0]])) {
                return this.$t('字段不能小于等于') + this.templateObj[alias[0]].name
              } else {
                return undefined
              }
            }
            ...
            if (value <= number[0]) {
              return this.$t('字段不能小于等于') + number[0]
            }
          }
        }
      ], [
        'ge', (value, ele, record) => {
          ...
          if (alias) {
            if (String(record[alias[1]]) !== 'undefined') {
              if (value < Number(record[alias[0]]) || value < Number(record[alias[1]])) {
                const message = this.$t('字段不能小于{0}或{1}', { 0: this.templateObj[alias[0]].name, 1: this.templateObj[alias[1]].name })
                return message
              } else {
                return undefined
              }
              ...
              if (value < Number(record[alias[0]])) {
                return this.$t('字段不能小于') + this.templateObj[alias[0]].name
              } else {
                return undefined
              }
            }
        ...
        'lt', (value, ele, record) => {
          ...
          if (alias) {
            if (String(record[alias[1]]) !== 'undefined') {
              if (value >= Number(record[alias[0]]) || value >= Number(record[alias[1]])) {
                const message = this.$t('字段不能大于等于{0}或{1}', { 0: this.templateObj[alias[0]].name, 1: this.templateObj[alias[1]].name })
                return message
              } else {
                return undefined
              }
              ...
              if (value >= Number(record[alias[0]])) {
                return this.$t('字段不能大于等于') + this.templateObj[alias[0]].name
              } else {
                return undefined
              }
            }
            ...
            if (value >= number[0]) {
              return this.$t('字段不能大于等于') + number[0]
            }
          }
        }
      ], [
        'le', (value, ele, record) => {
          ...
          if (alias) {
            if (String(record[alias[1]]) !== 'undefined') {
              if (value > Number(record[alias[0]]) || value > Number(record[alias[1]])) {
                const message = this.$t('字段不能大于{0}或{1}', { 0: this.templateObj[alias[0]].name, 1: this.templateObj[alias[1]].name })
                return message
              } else {
                return undefined
              }
              ...
              if (value > Number(record[alias[0]])) {
                return this.$t('字段不能大于') + this.templateObj[alias[0]].name
              } else {
                return undefined
              }
            }
        ...
        'st', (value, ele, record) => {
          ...
          if (alias) {
            if (Number(record[alias[0]]) < value && value < Number(record[alias[1]])) {
              ...
              const message = this.$t('字段不能小于{0}或大于{1}', { 0: this.templateObj[alias[0]].name, 1: this.templateObj[alias[1]].name })
              return message
            }
          } else if (number) {
            if (value < number[0] || value > number[1]) {
              const message = this.$t('字段不能小于{0}或大于{1}', { 0: number[0], 1: number[1] })
              return message
            }
          }
        }
        ...
        'sta', (value, ele, record) => {
          ...
          if (Number(record[alias]) < value && value < number[0]) {
            ...
            const message = this.$t('字段不得小于{0}或大于{1}', { 0: this.templateObj[alias].name, 1: number[0] })
            return message
          }
        }
      ], [
        ...
        'stn', (value, ele, record) => {
          ...
          if (number[0] < value && value < Number(record[alias])) {
            ...
            const message = this.$t('字段不得小于{0}或大于{1}', { 0: number[0], 1: this.templateObj[alias].name })
            return message
          }
        }
      ]]),
    ...
    const p = new Promise((resolve) => {
      this.axios({
        ...
        for (let i = 0; i < this.toolButtons.length; i++) {
          if (this.toolButtons[i].name === '批量删除') {
            this.tbarDel = true
          } else if (this.toolButtons[i].name === '添加') {
            this.tbarAbb = true
          }
        }
        this.toolButtons = res.result.toolButtons.filter(item => item.name !== '批量删除' && item.name !== '添加')
        if (!this.tbarDel) {
  ...
  methods: {
    ...
    dataTotalChange (record) {
      ...
      const dataIndex = this.dataTotal.findIndex(dataItem => dataItem.sort === record.sort)
      for (const i in saveData) {
        if (i.substr(0, 1) === '_' && i.substr(-1) === '_') {
          delete saveData[i]
        }
    ...
    onSubform (parameter, data) {
      if (parameter.barData.importTemplate) {
        let importTemplate = parameter.barData.importTemplate
        ...
        this.$message.error(this.$t('开窗工具条缺少属性'))
      }
    },
    // 获取开窗选择数据
    getVal (val, alias, info, record) {
      if (alias) {
        ...
        val.forEach((valItem, valIndex) => {
          ...
          const obj1 = {}
          const obj2 = {}
          obj2.sort = this.dataTotal.length + valIndex
          obj2.__action__ = 'add'
          const index = ''
          for (const i in this.templateObj) {
          ...
          this.importTemplate.fieldList.forEach(item => {
        ...
        let key = this.subformItem.field.setting.form.sonFieldKey
        if (key && key.length) {
      ...
      if (info.form.mode !== 'dialog') {
        ...
        this.timeout = setTimeout(() => {
          this.axios({
            ...
            data: Object.assign(parameter, { tableId: info.form.linkList[1], fieldId: item.fieldId }, { advanceSearch })
          }).then(res => {
            const selectList = []
    ...
    handleChoice (item1, item, record) {
      this.axios({
        url: '/admin/general/getTableRecord',
        data: { tableId: item.setting.form.linkList[1], id: item1.value.id }
      }).then(res => {
