  ...
  methods: {
    show (parameter, page) {
      ...
      this.axios({
        ...
        this.templateId = this.params.tplviewidtpl ? this.params.tplviewidtpl : templateId
        if (this.templateId) {
    ...
    requestData () {
      ...
      this.axios({
        ...
        data: {
          action: nowParams.action,
          id: this.params.record ? this.params.record.id : 0,
          listTemplateId: this.params.tpl || this.$parent.params.templateId,
        ...
        const getComponent = (array) => {
          array.forEach((item, index) => {
            if (item.columns) {
              ...
              if (item.type === 'component') {
                const funcStr = `return (_this, record) => {  return ${item.attribute} }`
                // eslint-disable-next-line no-new-func
                const func = new Function(funcStr)
                const obj = func()(_this, _this.params.record)
                if (obj.template) {
                  obj.template = obj.template.replace(/[\r\n]/g, '')
                }
                item.component = obj
