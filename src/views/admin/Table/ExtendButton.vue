<template>
  <div>
    ...
    <a-table ref="table" rowKey="id" :columns="columns" :dataSource="formViewButtons" size="small" :pagination="false">
      ...
      <span slot="visible" slot-scope="text">
        <template v-if="text">{{ $t('是') }}</template>
    ...
    <!-- 访问权限 -->
    <priv-visit-form ref="privVisitForm" :params="{ formView: formViewButtons }" @func="getPrivExtendBarMenus" />
...
export default {
  ...
  watch: {
    extendBarMenuData (newValue) {
      this.formViewButtons = newValue || []
    }
  },
  created () {
    this.formViewButtons = this.extendBarMenuData || []
  },
  methods: {
    handleAdd () {
      this.$refs.extendButtonForm.show({
        ...
        record: {
          id: (new Date()).valueOf(),
          listOrder: this.formViewButtons.length * 10 + 10,
          name: '',
          buttonType: 0,
          visible: false,
          usePermissions: '',
    ...
    darwerOpen () {
      this.$refs.toolButtonChoose.show({
        ...
        unCheckBar: this.listArray.filter(item => this.formViewButtons.findIndex(barItem => barItem.usage === item.usage) === -1)
      })
    },
    getButtonData (data) {
    ...
    copyPrivShow (record) {
      this.$refs.privCopy.show({
        ...
        dataIndex: 'name',
        record: record,
        data: this.formViewButtons.filter(item => item.id !== record.id)
      })
    },
    ...
    getSort (data) {
      this.formViewButtons = data
    },
    getPrivExtendBarMenus (e, index) {
      this.formViewButtons.forEach((item, idx) => {
        if (index === idx) {
          item.usePermissions = e
        }
      })
    },
    getExtendBarMenus (action, value, index) {
      if (action !== 'edit') {
        this.formViewButtons.push(value)
      } else {
        this.formViewButtons.splice(index, 1, value)
      }
    },
