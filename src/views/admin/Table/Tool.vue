<template>
  <div>
    <div class="table-operator">
      ...
      <a-button v-if="config.variable !== 'webSubformDataWindow'" @click="addFlowButton">
        {{ $t('一键切换成流程按钮') }}
      </a-button>
    ...
    >
      ...
      <span slot="appVisible" slot-scope="text, record">
        <a-switch
          :disabled="record.disabled == 1"
          :defaultChecked="text"
          @change="
            (value) => {
              record.appVisible = value
            }
          "
        />
      </span>
      ...
      <div slot="action" slot-scope="text, record, index">
        ...
        <a-divider v-if="record.usage === 'sys_bulksearch'" type="vertical" />
        <a v-if="record.usage === 'sys_bulksearch'" @click="handleBatchSearch(record, index)">{{ $t('配置') }}</a>
        <a-divider type="vertical" />
...
<script>
import { listButton, cardButton, subformWebButton, workflowButton } from './DefaultButton'
export default {
  ...
  computed: {
    listArray () {
      if (this.config.type === 'tableCardWindow') {
        return cardButton
      } else if (this.config.type === 'webSubformDataWindow') {
        return subformWebButton
      } else if (this.config.type === 'workflow_list') {
  ...
  mounted () {
    ...
    // Tool 组件在 WorkflowForm（流程管理） TplviewDataAppFormEdit(app数据窗口) TplviewDataFormEdit（web数据窗口） 等中使用， table的cloumns根据不同组件显示不同表头
    if (this.entranceType === 'WorkflowForm') {
      const addColumns = [{
        title: this.$t('是否显示(pc端)'),
        dataIndex: 'visible',
        width: 140,
        scopedSlots: { customRender: 'visible' }
      }, {
        title: this.$t('是否显示(移动端)'),
        dataIndex: 'appVisible',
        width: 140,
        scopedSlots: { customRender: 'appVisible' }
      }]
      this.columns.splice(5, 1, ...addColumns)
    }
  },
  methods: {
    handleAdd () {
