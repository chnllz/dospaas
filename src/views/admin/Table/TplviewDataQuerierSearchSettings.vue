    ...
    <a-modal :title="config.title" :visible="visible" :width="600" :destroyOnClose="true" @cancel="visible = !visible">
      <a-spin :spinning="loading">
        <a-form :form="form" :labelCol="labelCol" :wrapperCol="wrapperCol">
          ...
          <a-row>
            <a-col :span="12">
              <a-form-item :label="$t('搜索区默认状态')" :labelCol="{ span: 12 }" :wrapperCol="{ span: 12 }">
                <a-radio-group
                  v-decorator="[
                    'info[defaultStatus]',
                    {
                  ...
                  @change="(e) => (data.defaultStatus = e.target.value)"
                >
            ...
            <a-col v-if="data.defaultStatus === '0'" :span="12">
              <a-form-item :label="$t('默认展开行数')" :labelCol="{ span: 12 }" :wrapperCol="{ span: 12 }">
                <a-input-number
                  v-decorator="[
                    'info[defaultRows]',
                    {
                      initialValue: data.defaultRows || 1,
                      rules: [{ required: true, message: '请输入默认展开行数' }]
                    }
                  ]"
                  :min="0"
                ></a-input-number>
