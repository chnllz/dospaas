...
export default {
          ...
          this.axios({
            ...
            if (res.code === 0) {
              ...
              this.tpl.forEach(item => {
                if (item.templateId === this.data.dateView) {
  ...
  created () {
    this.axios({
      url: '/admin/field/getModuleTableFieldOptions'
    }).then(res => {
  ...
  methods: {
    ...
    getDataForm (value, id) {
      const templateId = value.templateId ? value.templateId : id
      this.axios({
        url: '/admin/table/getFieldsAndTemplates',
        ...
        if (res.code === 0) {
          ...
          if (templateId) {
            const { setFieldsValue } = this.form
            setFieldsValue({ 'dateView': templateId })
    ...
    handleWinField (val, item) {
      ...
      if (val) {
        ...
        item.currentFields = this.currentFields.filter(item => {
          if (formType === 'text' || formType === 'serialnumber') {
            return item.formType === 'text' || item.formType === 'serialnumber'
          }
    ...
    handleCurrentField (val, item) {
      ...
      if (val) {
        ...
        item.linkFields = this.linkFields.filter(item => {
          if (formType === 'text' || formType === 'serialnumber') {
            return item.formType === 'text' || item.formType === 'serialnumber'
          }
          return formType === item.formType
    ...
    handleSubmit () {
      ...
      validateFields((errors, values) => {
        ...
        if (!errors) {
          values.fieldList = this.fieldList.map(item => {
            const { currentField, winField } = item
            return {
              currentField,
              winField
            }
