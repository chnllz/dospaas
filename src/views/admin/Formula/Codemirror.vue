          ...
          >
            ...
            <span slot="action" slot-scope="text, record">
              <a
                ...
                @click="
                  () => {
                    addText(`${toUnderline(record.alias)}`, `{${record.name}}`, 'cm-field')
                  }
                "
              >
                {{ $t('引用') }}
              ...
              <a
                ...
                @click="
                  () => {
                    addText(`$getData('${record.alias}')`, '${' + record.name + '}', 'cm-field', 'getValue')
                  }
                "
              >
                {{ $t('取值') }}
          ...
          >
            ...
            <span slot="action" slot-scope="text, record">
              <a
                ...
                @click="
                  () => {
                    addText(`${record.module + '_' + toUnderline(record.alias)}`, `#${record.name}#`, 'cm-table')
                  }
                "
              >
                {{ $t('引用') }}
              ...
              >
                ...
                <span slot="dictCategoryName" slot-scope="text, record">
                  <a
                    ...
                    "
                  >
                    {{ text ? text + (record.childCount > 0 ? '(' + record.childCount + ')' : '') : '' }}
                  </a>
                </span>
                <span slot="action" slot-scope="text, record">
                  <a
...
export default {
  ...
  methods: {
    ...
    // 默认赋值
    setValue () {
      const value = this.params.data.value ? this.params.data.value : ''
      this.$refs.mycode.codemirror.setValue(value)
    },
    // 加载本表字段数据
    fieldSelfData (parameter) {
      return this.axios({
        ...
        this.tableParam.module = res.result.module || 'admin'
        return res.result
      })
    },
    // 字段数据
