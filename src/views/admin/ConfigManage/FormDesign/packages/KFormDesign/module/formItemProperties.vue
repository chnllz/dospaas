<template>
  <div class="properties-centent kk-checkbox">
    <div class="head-title">
      ...
      <span v-else-if="selectItem.formType === 'web_sub_data_window'">{{ $t('子表') }}</span>
      ...
      <span v-else-if="selectItem.type === 'web_sub_data_window'">{{ $t('子表DW') }}</span>
    ...
    <div class="properties-body">
      <a-form v-show="selectItem.key !== ''">
        <a-form-item :label="$t('名称')">
          <a-input v-if="selectItem.type === 'field' || selectItem.alias" v-model="selectItem.name" :disabled="true" />
          <a-input v-if="selectItem.type === 'placeholder'" v-model="selectItem.label" :disabled="true" />
          <a-input
            v-else-if="selectItem.type === 'divider' && selectItem.dividerText"
            v-model="selectItem.dividerText"
            :disabled="true"
          />
          <a-input v-else-if="selectItem.type !== 'field'" v-model="selectItem.label" :placeholder="$t('请输入')" />
        </a-form-item>
        ...
        <div v-if="tableType">
          <a-form-item v-if="selectItem.alias && selectItem.formType !== 'image'">
            <a-space>
              <a-button @click="styleChange('0')">{{ $t('默认样式') }}</a-button>
              <a-button @click="styleChange('1')">{{ $t('标签样式') }}</a-button>
            </a-space>
          </a-form-item>
          <a-form-item v-if="selectItem.alias" :label="$t('是否显示label')">
            <a-radio-group v-model="selectItem.labelShow" defaultValue="1">
              <a-radio value="1">{{ $t('是') }}</a-radio>
              <a-radio value="0">{{ $t('否') }}</a-radio>
            </a-radio-group>
          </a-form-item>
          <a-form-item v-if="selectItem.alias" :label="$t('label颜色')">
            <a-radio-group v-model="selectItem.labelColor" defaultValue="">
              <a-radio v-for="itemColor in colorType" :key="itemColor.value" :value="itemColor.value">
                <span :style="{ background: itemColor.color, color: itemColor.color }">{{ itemColor.type }}</span>
              </a-radio>
            </a-radio-group>
          </a-form-item>
          <a-form-item v-if="selectItem.alias && selectItem.formType !== 'image'" :label="$t('字体大小')">
            <a-radio-group v-model="selectItem.fontSize" defaultValue="0">
              <a-radio value="0">{{ $t('普通文本') }}</a-radio>
          ...
          <a-form-item v-if="selectItem.alias && selectItem.formType !== 'image'" :label="$t('正文颜色')">
            <a-radio-group v-model="selectItem.color" defaultValue="">
              <a-radio value="#fafafa">{{ $t('白色') }}</a-radio>
              <a-radio v-for="itemColor in colorType" :key="itemColor.value" :value="itemColor.value">
                <span :style="{ background: itemColor.color, color: itemColor.color }">{{ itemColor.type }}</span>
              </a-radio>
            </a-radio-group>
          </a-form-item>
          <a-form-item v-if="selectItem.alias && selectItem.formType !== 'image'" :label="$t('正文背景颜色')">
            <a-radio-group v-model="selectItem.backGroundColor" defaultValue="">
              <a-radio value="">{{ $t('无色') }}</a-radio>
              <a-radio
                v-for="itemColor in colorType"
                v-show="itemColor.color !== '#000'"
                :key="itemColor.value"
          ...
          <a-form-item v-if="selectItem.alias && selectItem.formType === 'image'" :label="$t('图片展示效果')">
            <a-radio-group v-model="selectItem.borderRadius" defaultValue="0">
              <a-radio value="0">{{ $t('方形') }}</a-radio>
              <a-radio value="1">{{ $t('圆形') }}</a-radio>
            </a-radio-group>
          </a-form-item>
          <a-form-item v-if="selectItem.alias" :label="$t('内边距')">
            <div>
              {{ $t('上 :') }}
              <a-input-number v-model="selectItem.topPadding" :min="0" />
              px
            </div>
            <div>
              {{ $t('下 :') }}
              <a-input-number v-model="selectItem.bottomPadding" :min="0" />
              px
            </div>
            <div>
              {{ $t('左 :') }}
              <a-input-number v-model="selectItem.leftPadding" :min="0" />
              px
            </div>
            <div>
              {{ $t('右 :') }}
              <a-input-number v-model="selectItem.rightPadding" :min="0" />
              px
            </div>
          </a-form-item>
          <a-form-item v-if="selectItem.alias" :label="$t('外边距')">
            <div>
              {{ $t('上 :') }}
              <a-input-number v-model="selectItem.topMargin" :min="0" />
              px
            </div>
            <div>
              {{ $t('下 :') }}
              <a-input-number v-model="selectItem.bottomMargin" :min="0" />
              px
            </div>
            <div>
              {{ $t('左 :') }}
              <a-input-number v-model="selectItem.leftMargin" :min="0" />
              px
            </div>
            <div>
              {{ $t('右 :') }}
              <a-input-number v-model="selectItem.rightMargin" :min="0" />
              px
            </div>
          </a-form-item>
        </div>
        <div v-else>
          <!-- 子表设置start -->
          <a-form-item v-if="selectItem.type === 'web_sub_data_window'" :label="$t('子表设置')">
            <a-button @click="subFormShow">
              <a-badge v-if="selectItem.associatedList.tableidSon" status="success" />
            ...
            <SubtableSetForm
              ref="subtableSetForm"
              :params="{
                associatedList: selectItem.associatedList,
                fieldColumns: params.fieldColumns,
                tableList: params.tableList
              }"
              @func="
                (data) => {
                  $set(selectItem, 'associatedList', data)
                }
              "
            />
          </a-form-item>
          <a-form-item v-if="selectItem.type === 'web_sub_data_window'" :label="$t('是否启用')">
            <a-radio-group v-model="selectItem.associatedList.enable">
              <a-radio value="1">{{ $t('是') }}</a-radio>
              <a-radio value="0">{{ $t('否') }}</a-radio>
            </a-radio-group>
          </a-form-item>
          ...
          <a-form-item v-if="selectItem.type === 'web_sub_data_window'" :label="$t('访问权限')">
            <a-button @click="handleFieldPriv">
              <a-badge v-if="selectItem.associatedList.priv" status="success" />
              <a-badge v-else status="default" />
              {{ $t('访问权限') }}
            </a-button>
            <priv-visit-form
              ref="privVisitForm"
              :params="{ formView: selectItem.associatedList }"
              @func="
                (e) => {
                  $set(selectItem, 'associatedList', e)
                }
              "
            />
          </a-form-item>
          <!-- 子表设置end -->
          <!-- 客户画像设置start -->
          <a-form-item v-if="selectItem.type === 'portrait'" :label="$t('客户画像设置')">
            <a-button @click="portraitSet">{{ $t('客户画像设置') }}</a-button>
            <portrait-set
              ref="portraitSet"
              :params="{ fieldColumns: params.fieldColumns, tableList: params.tableList }"
              @ok="(data) => (selectItem.portraitData = data)"
            />
          </a-form-item>
          <!-- 流程办理方式start -->
          ...
          <!-- 流程办理方式end -->
          <!-- 组织结构start -->
          <a-form-item v-if="selectItem.formType === 'organization'" :label="$t('自定义数据源')">
            <a-input v-model="selectItem.dataSourceUrl" />
          </a-form-item>
          <!-- 组织结构end -->
          <!-- 字段附加属性start -->
          <a-form-item v-if="selectItem.type === 'field'">
            <span slot="label">
              ...
              <a-tooltip>
                <template slot="title">
                  {{ $t('实例说明连接：') }}
                  <a href="https://doscs.yuque.com/dos-rnd/chyfv7/agsxsx" target="_blank">
                    https://doscs.yuque.com/dos-rnd/chyfv7/agsxsx
                  </a>
                </template>
          ...
          <!-- 字段附加属性end -->
          <!-- 自定义组件附加属性start -->
          <a-form-item v-if="selectItem.type === 'component'" :label="$t('自定义文件路径')">
          ...
          :label="$t('数据字段')"
        >
          <a-input v-model="selectItem.model" :placeholder="$t('请输入')" />
        </a-form-item> -->
          <!-- input type start -->
          <!-- <a-form-item v-if="selectItem.formType === 'text'" :label="$t('输入框type')">
          <a-input v-model="options.type" :placeholder="$t('请输入')" />
        </a-form-item> -->
          <!-- input type end -->
          <!-- <a-form-item
          v-if="
          ...
          :label="$t('占位内容')"
        >
          <a-input :placeholder="$t('请输入')" v-model="options.rangePlaceholder[0]" />
          <a-input :placeholder="$t('请输入')" v-model="options.rangePlaceholder[1]" />
        </a-form-item>
        <a-form-item
          v-else-if="typeof options.placeholder !== 'undefined'"
          :label="$t('占位内容')"
        >
          <a-input :placeholder="$t('请输入')" v-model="options.placeholder" />
        </a-form-item> -->
          <!-- <a-form-item
          v-if="selectItem.formType === 'textarea'"
          :label="$t('自适应内容高度')"
        >
          <a-input-number
            style="width:100%"
          ...
          <!-- <a-form-item v-if="typeof options.width !== 'undefined'" :label="$t('宽度')">
          <a-input :placeholder="$t('请输入')" v-model="options.width" />
        </a-form-item>
        <a-form-item v-if="typeof options.height !== 'undefined'" :label="$t('高度')">
          <a-input-number v-model="options.height" />
        </a-form-item>
        <a-form-item v-if="typeof options.step !== 'undefined'" :label="$t('步长')">
          <a-input-number v-model="options.step" :placeholder="$t('请输入')" />
        </a-form-item>
        <a-form-item v-if="typeof options.min !== 'undefined'" :label="$t('最小值')">
          <a-input-number v-model="options.min" :placeholder="$t('请输入')" />
        </a-form-item>
        <a-form-item v-if="typeof options.max !== 'undefined'" :label="$t('最大值')">
          <a-input-number v-model="options.max" :placeholder="$t('请输入')" />
        </a-form-item> -->
          <!-- <a-form-item
        ...
        >
          <a-radio-group buttonStyle="solid" v-model="options.dynamic">
            <a-radio-button :value="false">{{ $t('静态数据') }}</a-radio-button>
            <a-radio-button :value="true">{{ $t('动态数据') }}</a-radio-button>
          </a-radio-group>

          <a-input
            v-show="options.dynamic"
            v-model="options.dynamicKey"
            :placeholder="$t('动态数据变量名')"
          ></a-input>
          ...
          <!-- 选项配置及动态数据配置 end -->
          <!-- 日期选择器默认值 start -->
          <!-- <a-form-item v-if="selectItem.type === 'date'" :label="$t('默认值')">
          <a-input
            v-if="!options.range"
        ...
        >
          <a-input
            v-model="options.defaultValue"
            :placeholder="
              typeof options.format === 'undefined' ? $t('请输入') : options.format
            "
          />
        </a-form-item> -->
          <!-- 修改html -->
          <a-form-item v-if="selectItem.type === 'html'" :label="$t('默认值')">
        ...
        >
          <a-input
            ...
            :placeholder="$t('时间格式如：YYYY-MM-DD HH:mm:ss')"
          />
        </a-form-item> -->
          <!-- 下载方式 start -->
          <!-- <a-form-item
        ...
        >
          <a-radio-group buttonStyle="solid" v-model="options.downloadWay">
            <a-radio-button value="a">{{ $t('a标签') }}</a-radio-button>
            <a-radio-button value="ajax">ajax</a-radio-button>
            <a-radio-button value="dynamic">{{ $t('动态函数') }}</a-radio-button>
          </a-radio-group>
          <a-input
          ...
          <!-- 下载方式 end -->
          <a-form-item v-if="selectItem.type === 'button'" :label="$t('按钮操作')">
            <a-radio-group v-model="options.handle" buttonStyle="solid">
              <a-radio-button value="submit">{{ $t('提交') }}</a-radio-button>
              <a-radio-button value="reset">{{ $t('重置') }}</a-radio-button>
              <a-radio-button value="dynamic">{{ $t('动态函数') }}</a-radio-button>
            </a-radio-group>
            <a-button @click="buttoCodeEditor">{{ $t('附加属性') }}</a-button>
            <!-- <a-input
          ...
          <a-form-item v-if="selectItem.type === 'alert'" :label="$t('辅助描述')">
            <a-textarea v-model="options.description" :auto-size="{ minRows: 5, maxRows: 10 }" />
          </a-form-item>
          <a-form-item v-if="selectItem.type === 'alert'" :label="$t('类型')">
          ...
          <a-form-item v-if="selectItem.type === 'alert'" :label="$t('操作属性')">
            <kCheckbox v-model="options.showIcon" style="display: block" :label="$t('显示图标')" />
            <kCheckbox v-model="options.banner" style="display: block" :label="$t('无边框')" />
            <kCheckbox v-model="options.closable" style="display: block" :label="$t('可关闭')" />
          </a-form-item>
          <!-- 上传图片 -->
          <!-- <a-form-item v-if="selectItem.type === 'uploadImg'" :label="$t('样式')">
        ...
        >
          ...
          :label="$t('上传地址')"
        >
          <a-input v-model="options.action" :placeholder="$t('请输入')"></a-input>
        </a-form-item> -->
          ...
          :label="$t('文件name')"
        >
          <a-input v-model="options.fileName" :placeholder="$t('请输入')"></a-input>
        </a-form-item> -->
          <!-- 上传额外参数 -->
          <!-- <a-form-item
          ...
          :label="$t('额外参数（JSON格式）')"
        >
          <a-input v-model="options.data" :placeholder="$t('严格JSON格式')"></a-input>
        </a-form-item> -->
          <!-- 文字对齐方式 -->
          ...
          <!-- <a-form-item v-if="selectItem.type === 'text'" :label="$t('操作属性')">
          <kCheckbox v-model="options.showRequiredMark" :label="$t('显示必选标记')" />
        </a-form-item> -->
          <a-form-item
          ...
          :label="$t('校验')"
        >
          <kCheckbox v-model="selectItem.rules[0].required" :label="$t('必填')" />
          <a-input
            v-model="selectItem.rules[0].message"
            :placeholder="$t('必填校验提示信息')"
          />
          <KChangeOption v-model="selectItem.rules" type="rules" />
        </a-form-item> -->

          <!-- 表格选项 -->
          <a-form-item v-if="selectItem.type === 'table'" :label="$t('表格样式CSS')">
            <a-input v-model="selectItem.options.customStyle" />
          </a-form-item>
          <a-form-item v-if="selectItem.type === 'table'" :label="$t('属性')">
            <kCheckbox v-model="selectItem.options.bordered" :label="$t('显示边框')" />
            <kCheckbox v-model="selectItem.options.bright" :label="$t('鼠标经过点亮')" />
            <kCheckbox v-model="selectItem.options.small" :label="$t('紧凑型')" />
          </a-form-item>

          <a-form-item v-if="selectItem.type === 'table'" :label="$t('提示')">
            <p style="line-height: 26px">{{ $t('请点击右键增加行列，或者合并单元格') }}</p>
          </a-form-item>
        </div>
        <!-- 卡片设置 -->
        <a-form-item v-if="selectItem.type === 'card'" :label="$t('边框设置')">
          <a-radio-group v-model="selectItem.bordered" buttonStyle="solid">
            <a-radio value="1">{{ $t('显示') }}</a-radio>
            <a-radio value="0">{{ $t('隐藏') }}</a-radio>
          </a-radio-group>
        </a-form-item>
        <a-form-item v-if="selectItem.type === 'card'" :label="$t('添加按钮设置')">
          <a-radio-group v-model="selectItem.addShow" buttonStyle="solid" defaultValue="0">
            <a-radio value="1">{{ $t('显示') }}</a-radio>
            <a-radio value="0">{{ $t('隐藏') }}</a-radio>
          </a-radio-group>
        </a-form-item>
        <!-- tabs配置 start -->
        <a-form-item v-if="options.tabBarGutter" :label="$t('页签间距')">
          <a-input-number v-model="options.tabBarGutter" :placeholder="$t('请输入')" />
        </a-form-item>
        ...
        <a-form-item v-if="selectItem.type === 'tabs'" :label="$t('页签高度')">
          <a-input-number v-model="selectItem.height" :placeholder="$t('请输入')" />
        </a-form-item>
        <!-- tabs配置 end -->
        <a-form-item v-if="selectItem.type === 'grid'" :label="$t('栅格间距')">
        ...
        <a-form-item v-if="selectItem.type === 'switch'" :label="$t('默认值')">
          <a-switch v-model="options.defaultValue" />
        </a-form-item>
        <a-form-item v-if="['number', 'slider'].indexOf(selectItem.type) >= 0" :label="$t('默认值')">
          <a-input-number
            v-model="options.defaultValue"
            :step="options.step"
        ...
        <a-form-item v-if="selectItem.type === 'rate'" :label="$t('默认值')">
          <a-rate v-model="options.defaultValue" :allowHalf="options.allowHalf" :count="options.max" />
        </a-form-item>
        <a-form-item v-if="selectItem.type === 'select'" :label="$t('默认值')">
          <a-select v-model="options.defaultValue" :options="options.options" />
        </a-form-item>
        <a-form-item v-if="selectItem.type === 'radio'" :label="$t('默认值')">
          <a-radio-group v-model="options.defaultValue" :options="options.options" />
        </a-form-item>
        <a-form-item v-if="selectItem.type === 'checkbox'" :label="$t('默认值')">
          <a-checkbox-group v-model="options.defaultValue" :options="options.options" />
        </a-form-item>
        <a-form-item v-if="String(options.orientation) !== 'undefined'" :label="$t('标签位置')">
          <a-radio-group v-model="options.orientation" buttonStyle="solid">
            <a-radio-button value="left">{{ $t('左') }}</a-radio-button>
            <a-radio-button value="">{{ $t('居中') }}</a-radio-button>
            <a-radio-button value="right">{{ $t('右') }}</a-radio-button>
          </a-radio-group>
        </a-form-item>
        <!-- 页签位置 start -->
        <a-form-item v-if="selectItem.type === 'tabs'" :label="$t('页签位置')">
        ...
        <!-- 页签位置 end -->
        <!-- 页签类型 start -->
        <a-form-item v-if="String(options.type) !== 'undefined' && selectItem.type === 'tabs'" :label="$t('页签类型')">
          <a-radio-group v-model="options.type">
        ...
        <!-- 页签类型 end -->
        <!-- 页签大小 start -->
        <a-form-item v-if="String(options.size) !== 'undefined'" :label="$t('页签大小')">
        ...
        <!-- 页签大小 end -->
        <a-form-item v-if="selectItem.type === 'button'" :label="$t('类型')">
          <a-radio-group v-model="options.type">
        ...
        <a-form-item
          v-if="selectItem.type === 'card' || selectItem.type === 'tabs' || selectItem.type === 'table'"
          :label="$t('外边距设置')"
        >
          <a-row>
            <a-col :span="3">{{ $t('上 :') }}</a-col>
            <a-col :span="21">
              <a-input-number v-model="selectItem.topMargin" />
              px
            </a-col>
            <a-col :span="3">{{ $t('下 :') }}</a-col>
            <a-col :span="21">
              <a-input-number v-model="selectItem.downMargin" />
              px
            </a-col>
            <a-col :span="3">{{ $t('左 :') }}</a-col>
            <a-col :span="21">
              <a-input-number v-model="selectItem.leftMargin" />
              px
            </a-col>
            <a-col :span="3">{{ $t('右 :') }}</a-col>
            <a-col :span="21">
              <a-input-number v-model="selectItem.rightMargin" />
              px
            </a-col>
          </a-row>
        </a-form-item>
...
export default {
  ...
  methods: {
    ...
    subFormShow () {
      if (!this.selectItem.associatedList.tableidSon) {
        ...
        this.$refs.subtableSetForm.show({
          action: 'edit',
          title: `${this.$t('编辑')}: ${this.selectItem.label}`,
          record: this.selectItem.associatedList
        })
      }
    },
    portraitSet () {
    ...
    handleFieldPriv () {
      if (this.selectItem.associatedList.tableidSon) {
        this.$refs.privVisitForm.show({
          action: 'edit',
          title: `${this.$t('访问权限')}: ${this.selectItem.label}`,
          record: this.selectItem.associatedList,
          key: 'priv',
          selectType: 'radio',
          privArr: {
    ...
    componentChange (e) {
      ...
      } else if (e === 'web_sub_data_window') {
        this.selectItem.associatedList = {
          enable: '1'
        }
      } else if (e === 'html') {
        this.selectItem.options = {
