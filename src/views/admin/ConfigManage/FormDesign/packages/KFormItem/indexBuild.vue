  ...
  <div>
    <div v-if="!formConfig.tableType">
      ...
      >
        ...
        <a-input
          v-else-if="record.formType === 'textarea' || record.formType === 'json'"
          :rows="2"
          :disabled="record.fieldRule === 'readonly'"
          :placeholder="record.formType === 'textarea' ? $t('多行文本') : $t('json数据格式')"
        >
          <a-icon slot="suffix" type="align-left" />
        </a-input>
        <a-date-picker
        ...
        <!-- 子表 -->
        <a-input
          v-else-if="record.formType === 'web_sub_data_window'"
          :disabled="record.fieldRule === 'readonly'"
          :placeholder="$t('子表')"
        >
        ...
        <div v-else-if="record.type === 'formmark'">
          <a-timeline>
            <a-timeline-item>
              <h3>{{ moment(new Date()).format('YYYY-MM-DD hh:mm:ss') }} {{ $t('张三(项目经理)') }}</h3>
              <a-table
                size="small"
                :pagination="false"
                rowKey="id"
