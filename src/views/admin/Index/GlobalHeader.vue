<template>
  <transition name="showHeader">
    <a-layout-header v-if="visible" ref="header" class="header" style="height: 64px">
      <div class="logo">
        <img
          v-if="setting.logoType && setting.logoType !== 'text' && setting.logoUrl && setting.logoUrl.filePath"
          :src="`${$store.state.env.VUE_APP_API_BASE_URL}admin/api/download/?filePath=${setting.logoUrl.filePath}`"
        />
        <h1
      ...
      <div style="flex: 1; display: flex; align-items: center">
        <div id="mypanel" style="margin-left: 64px; width: 820px; line-height: normal"></div>
      </div>
      <!-- <template>
            ...
            <span v-else style="cursor: pointer">
              ...
              {{
                setting.userBusyEnable && setting.userFurtherStates.find((item) => item.value === setting.userStatus)
                  ? $t(setting.userFurtherStates.find((item) => item.value === setting.userStatus).label)
                  : $t('示忙')
              }}
            </span>
          </div>
      ...
      <!-- 新建工单 -->
      <a-tooltip v-if="userInfo.popscreen">
        <template slot="title">{{ $t('新建工单') }}</template>
        <div
          class="message"
          @click="
      ...
      <a-dropdown>
        ...
        <a-menu slot="overlay" class="user-dropdown-menu-wrapper">
          ...
          <a-sub-menu v-if="defaultSettings.language.length > 1" key="4">
            <a slot="title" href="javascript:;" style="display: inline-block">
              <a-icon type="global" style="padding-right: 8px" />
              <span>{{ $t('语言') }}</span>
            </a>
            <a-menu-item v-for="item of defaultSettings.language" :key="item.value" @click="handleSetLang(item.value)">
              <a-badge v-if="lang === item.value" status="processing" />
      ...
      >
        <a-row style="line-height: 30px">
          <a-col :span="6">{{ $t('产品名称：') }}</a-col>
          <a-col :span="18">{{ $t(setting.title) }}</a-col>
          <a-col :span="6">{{ $t('版权所有：') }}</a-col>
          <a-col :span="18">{{ $t(setting.copyright) }}</a-col>
          <a-col :span="6">{{ $t('版本号：') }}</a-col>
          <a-col :span="18">v{{ setting.releaseVersion }}</a-col>
          <a-col :span="6">{{ $t('前端发行时间：') }}</a-col>
          <a-col :span="18">{{ $t(release) }}</a-col>
          <a-col :span="6">{{ $t('后端发行时间：') }}</a-col>
          <a-col :span="18">{{ moment(setting.releaseTime).format('YYYY-MM-DD HH:mm:ss') }}</a-col>
        </a-row>
        <div slot="footer">
...
export default {
  ...
  mounted () {
    ...
    if (this.$store.getters.userInfo.extension) {
      ...
      const urlStr = this.$store.getters.setting.callcenterControlPanelUrl || ''
      ...
      element.src = `${interfaceurl}sdk/index/callcenter/?debug=0&extension=${this.$store.getters.userInfo.extension}&popscreen_callback=mypopscreen&control_container=mypanel&lang=${storage.get('lang')}`
  ...
  methods: {
    async initWebSocket () {
      ...
      if (typeof (WebSocket) === 'undefined') {
        ...
        // 监听socket连接
        let path
        if (this.setting.websocket) {
          path = `${this.setting.websocket}?role=user&token=${this.userInfo.accessToken}`
        } else {
          path = `${this.$store.state.env.VUE_APP_API_BASE_URL}admin/websocket/client?role=user&token=${this.userInfo.accessToken}`
        }
        const sendMsg = {
          code: '1001',
        ...
        const connectCallback = () => {
          ...
          this.status = 'online'
          stompClient.subscribe('/chat/sendToUser', (msg) => {
            // to do
          })
          // 系统消息通知通道
          stompClient.subscribe('/topic/all')
          // 全局个人通道
          stompClient.subscribe('/user/topic/message', (msg) => {
            const obj = JSON.parse(msg.body)
            this.$store.state.app.setting.webSocketMsg && this.$store.state.app.setting.webSocketMsg.indexChatMsg(msg, stompClient)
            if (obj) {
              const msgData = obj
              if (msgData.code === '2004') {
                stompClient.send('/admin/sendToUser', JSON.stringify(sendMsg))
              } else if (msgData.code === '2002') {
                window.location.reload()
              } else if (msgData.code === '1001') {
                this.$setSetting({ clientId: msgData.data.clientId })
              } else if (msgData.code === '1072') {
                // 更新在线客服状态
                this.$store.dispatch('SetOnlineStatus', msgData.data.status)
              } else if (msgData.code === '2001') {
        ...
        this.connect(path, connectCallback)
      }
    },
    connect (path, connectCallback) { // 定义连接函数
      if (this.stompClient === null || !this.stompClient.connected) {
        const socket = new SockJS(path)
        this.stompClient = Stomp.over(socket)
        this.stompClient.heartbeat.outgoing = 60000
        this.stompClient.heartbeat.incoming = 60000
        ...
        this.stompClient.connect({}, connectCallback, () => {
          ...
          this.timeId = setTimeout(() => {
            console.log('websocket连接失败，正在尝试重新连接')
    ...
    editServiceStatus (menu) {
      ...
      if (this.status === 'online') {
        this.stompClient.send('/admin/sendToUser', JSON.stringify(obj))
      } else {
    ...
    handleUserinfo () {
      this.$router.push(this.userInfo.userInfoMenu)
    },
    // 弹屏提交
    popFormSubmit () {
      ...
      validateFields((errors, values) => {
        if (!errors) {
          const url = `${process.env.VUE_APP_BASE_URL}CustomPage/?view=` + this.setting.popurl + '&Event=Popscreen&Poptime=Ring' + '&Callerid=' + values.contactPhone + '&Calleeid=' + this.userInfo.extension + '&CalleeIDName=&Recordfile=&Uniqueid=' + Date.now() + '&DateTime=' + this.moment().format('YYYY-MM-DD HH:mm:ss') + '&Server=localhost&Method=DialIn'
          window.open(url)
