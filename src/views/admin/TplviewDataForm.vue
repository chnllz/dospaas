    ...
    <a-spin :spinning="loading">
      <a-form :form="form">
        <a-tabs>
          ...
          <a-tab-pane v-if="config.type !== 'webSubformDataWindow'" key="3" :tab="$t('搜索器')">
  ...
  methods: {
    show (config) {
      ...
      this.axios({
        ...
        data: Object.assign({ action: 'get', tableId: config.tableId ? config.tableId : 0, type: this.config.type, id: config.record ? config.record.id : 0 })
      }).then((res) => {
        this.loading = false
      ...
      validateFields((errors, values) => {
        if (!errors) {
          ...
          this.axios({
            ...
            data: { id: this.config.action === 'edit' ? this.data.id : 0, name: values.info.name, type: this.config.type, tableId: this.config.tableId }
          }).then((res) => {
            if (res.code) {
