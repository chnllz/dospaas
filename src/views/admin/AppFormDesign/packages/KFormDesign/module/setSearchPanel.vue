              ...
              <a-row :gutter="10">
                <a-col span="12">
                  <a-row :gutter="10" style="margin-bottom: 10px">
                    <a-col span="8">
                      <a-select
                        v-model="fieldCategorySearch"
                        :placeholder="$t('请选择分类')"
                        allowClear
                        @change="searchFiled"
                      >
                        <a-select-option v-for="item in fieldCategory" :key="item.value" :value="item.value">
                          {{ item.value }}
                        </a-select-option>
                      </a-select>
                    </a-col>
  ...
  methods: {
    show (config) {
      ...
      this.dataSource = this.fieldColumns.filter(item => {
        return this.conditionSet.searchFields.findIndex(selectedItem => selectedItem.alias === item.alias) === -1 && this.searchFields.includes(item.formType)
      })
    ...
    searchFiled () {
      let dataSource = this.filterDataSource.filter(item => item.name.includes(this.fieldSearch) && this.searchFields.includes(item.formType))
      if (this.fieldCategorySearch) {
        dataSource = dataSource.filter(item => item.category === this.fieldCategorySearch)
      }
    ...
    */
    sortField (orignList, sortList) {
      ...
      return orignList.filter(item => {
        return aliasList.includes(item.alias) && this.searchFields.includes(item.formType)
      })
    },
    /**
     *  过滤出非选字段
    ...
    filterNoSelectFiled () {
      this.filterDataSource = this.fieldColumns.filter(item => {
        return this.conditionSet.searchFields.findIndex(selectedItem => selectedItem.alias === item.alias && this.searchFields.includes(item.formType)) === -1
      })
    },
