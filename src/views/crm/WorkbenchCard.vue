                ...
                <a-tab-pane key="log" tab="Overview">
                  <a-timeline v-if="logList.length">
                    <a-timeline-item v-for="(logItem, logIndex) in logList" :key="logIndex">
                      ...
                      <div>{{ $t('办理方式：') }}{{ logItem.handleWay }}</div>
                      <div>{{ $t('办理备注：') }}{{ logItem.handleRemarks || '--' }}</div>
                    </a-timeline-item>
                  </a-timeline>
                  <a-empty v-else />
                </a-tab-pane>
  ...
  methods: {
    show (config) {
      ...
      if (item.ddh) {
        this.axios({
          ...
          data: {
            orderNo: item.ddh || '',
            workOrderNumber: item.caseNumber || ''
          }
        }).then(res => {
      ...
      if (['all_process'].includes(item.type) || ['my_process'].includes(item.type) && item.process_current_user && item.process_current_user.includes(this.userInfo.username)) {
        ...
        dataValue.data = {
          caseId: item.caseId,
          flowCondition: config.parent.flowCondition,
          templateId: 'd2b4017dc8044c528b503369580fd21a',
          viewType: 'view'
        }
      }
      this.visible = true
        ...
        this.axios({
          ...
          if (res.code) {
            ...
            if (this.orderData.data) {
              this.parentParams.caseId = this.orderData.data.caseId
              ...
              this.axios({
                ...
                data: {
                  ...
                  sortField: 'id',
                  sortOrder: 'descend',
                  caseId: this.orderData.data.caseId
                }
              }).then(res => {
              ...
              if (this.orderData.data.gdlx === 'zanting_gd') {
                this.axios({
                  ...
                  data: {
                    workOrderNumber: this.orderData.data.caseNumber
                  }
                }).then(res => {
            ...
            if (val.template) {
              // 表单初始化loader
              if (val.templateScript && val.templateScript.afterInit) {
                var initAttribute = val.templateScript.afterInit
                var initTemplate = {
                  type: 'component',
            ...
            this.handleWayData = val.handleMethods
            this.templateOther = val.template
            this.remarksMinRows = val.remarksMinRows
            this.remarksMaxRows = val.remarksMaxRows
    ...
    handleEmail (type, mailItem, replyAgain) {
      if (type === 'create') {
        ...
        this.$refs.mailInfo.show({
          data: Object.assign(mailItem, { caseNumber: this.orderData.data.caseNumber || '' }),
          title: this.$t('详情'),
          type: 'workbench'
        })
      }
    ...
    getAssList () {
      if (!this.associatedList.length) {
        this.axios({
          ...
          data: {
            curWorkOrderNumber: this.orderData.data.caseNumber,
    ...
    getMailInfo () {
      this.axios({
        ...
        data: { workOrderNumber: this.orderData.data.caseNumber || '' }
      }).then(res => {
          ...
          new Promise((resolve, reject) => {
            this.axios({
              ...
              data: {
                ...
                workOrderNumber: formData.caseNumber
              }
            }).then(res => {
              if (res.code) {
                this.modalLoading = false
            ...
            this.axios({
              ...
              data: {
                ...
                conditionFieldValue: this.orderData.data.caseNumber
              }
            }).then(res => {
              this.modalLoading = false
              if (res.code) {
