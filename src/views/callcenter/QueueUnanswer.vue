  ...
  methods: {
    init () {
      if (this.changeFlag) {
        ...
        this.axios({
          url: '/callcenter/queuestat/unanswered',
          data: {
            type: 'income_detail',
          ...
          if (res.code === 0) {
            ...
            for (const key in keyObj) {
              ...
              if (keyObj[key]) {
                ...
                obj.name = [keyObj[key]]
                if (key === 'division') {
                  obj.value = data.totalInboundTimes + '/' + data.totalConnectTimes
                } else if (key === 'allPercent') {
                  let num = data.totalInboundTimes ? data.totalConnectTimes / data.totalInboundTimes * 100 : 0
                  num = num.toFixed(2)
                  obj.value = num + '%'
                } else {
                  obj.value = data[key]
    ...
    handleExport (record, key) {
      ...
      const arr = ['queueWaitLevelVO', 'queueThroughTheStatisticalVO', 'un_agent']
      const arrSub = ['queueWaitLevel', 'queueUnConnectStatistics', 'queueCallMinute']
      const index = arr.findIndex(item => this.tbKey === item)
      if (key) {
        this.$refs.generalExport.show({
          title: this.$t('导出'),
          className: 'ExportQueueUnConnectTask',
          setMenuName: true,
          parameter: {
            condition: {
              req: data,
        ...
        this.$refs.generalExport.show({
          ...
          parameter: {
            condition: {
              req: data,
              condition: this.tbKey === 'queueWaitLevelVO' ? record.grade : record.qname,
    ...
    loadData (parameter) {
      return this.axios({
        ...
        url: '/callcenter/queuestat/getundata'
      }).then((res) => {
        return res.result
      })
    },
