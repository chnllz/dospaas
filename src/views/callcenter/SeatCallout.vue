...
export default {
  ...
  data () {
    return {
      ...
      infoColumns: [
        ...
        { title: this.$t('总呼出数'), dataIndex: 'totalCalls' },
        ...
        { title: this.$t('总呼出时长'), dataIndex: 'totalBillPrint' },
      ...
      childColumns: [
        ...
        { title: '呼叫时间', dataIndex: 'callTime' },
        { title: '主叫', dataIndex: 'source' },
        { title: '被叫', dataIndex: 'destination' },
        { title: '计费时长', dataIndex: 'billableSeconds' },
  ...
  methods: {
    ...
    onExpand (expanded, record) {
      if (expanded === true) {
        this.queryParam = { exten: record.extension }
    ...
    initCharts () {
      ...
      const option = {
        ...
        color: ['#67DC00', '#C52518'],
        title: { text: this.$t('呼出接听数与未接数对比图') },
        toolbox: { show: false },
        tooltip: {
          trigger: 'axis',
      ...
      const optionBottom = {
        color: ['#3398DB'],
        title: { text: this.$t('坐席呼出的总数统计图（单位：次）') },
        tooltip: {
          trigger: 'axis',
          axisPointer: { // 坐标轴指示器，坐标轴触发有效
            type: 'shadow' // 默认为直线，可选为：'line' | 'shadow'
