      ...
      <a-form :form="form">
        <a-alert type="info" style="margin-bottom: 8px">
          <div slot="message">
            <div>
              1.{{
                $t(
                  '模板CODE和模板内容直接从短信平台系统中复制过来，在使用文件导入的方式进行批量发送短信时，模板文件中的自定义字段1、自定义字段2、自定义字段3等会按照顺序依次替换模板中的变量${xxxx}'
                )
              }}
            </div>
            <div>2.{{ $t('当模板CODE为空时，发送短信内容，将为模板内容') }}</div>
            <div>
              3.{{
                $t(
                  '例如：您的注册验证码为：${code}，请于${minute}分钟内完成注册。如非本人操作，请忽略。模板文件中的自定义字段1和自定义字段2会分别替换变量${code}和${minute}'
                )
              }}
            </div>
          </div>
        </a-alert>
        <a-form-item :label="$t('模板编号')" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input-number
            v-decorator="[
              'info[number]',
              { rules: [{ required: true, message: $t('请输入模板编号') }], initialValue: data.number }
            ]"
            :min="1000"
            :max="100000000"
            style="width: 100%"
        ...
        <a-form-item :label="$t('模板名称')" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input
            v-decorator="[
              'info[name]',
              { rules: [{ required: true, message: $t('请输入模板名称') }], initialValue: data.name }
            ]"
          />
        </a-form-item>
        ...
        <a-form-item :label="$t('模板内容')" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-textarea
            v-decorator="[
              'info[content]',
              { rules: [{ required: true, message: $t('请输入模板内容') }], initialValue: data.content }
            ]"
    ...
    handleSubmit () {
      ...
      validateFields((errors, values) => {
        if (!errors) {
          ...
          this.axios({
            url: this.config.url,
            data: Object.assign({ action: 'submit', id: this.data.id }, values)
          }).then((res) => {
            this.loading = false
            if (res.code) {
