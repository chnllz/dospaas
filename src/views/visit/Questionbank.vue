<template>
  <div class="page">
        ...
        <a-row class="form" :class="advanced ? 'advanced' : 'normal'">
          <a-col :span="6">
            <a-form-item :label="$t('题型')">
              <a-select v-model="queryParam.questionstype" :allowClear="true" :showSearch="true">
                <a-select-option v-for="(value, keys) in questionTypeShow" :key="keys + 1" :value="value.type">
                  {{ value.type }}
                </a-select-option>
              </a-select>
            </a-form-item>
    ...
    <a-space>
      <a-button v-action:add icon="plus" type="primary" @click="addPage('单选题')">{{ $t('添加') }}</a-button>
    </a-space>
    <s-table
      ref="table"
      class="table-fill"
    ...
    >
      <div slot="action" slot-scope="text, record">
        ...
        <a @click="bankDelete(record)">{{ $t('删除') }}</a>
      </div>
    </s-table>
