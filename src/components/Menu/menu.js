...
export default {
  ...
  methods: {
    ...
    updateMenu () {
      ...
      const curRouteList = matched[matched.length - 1].path.split('/')
      const routes = []
      const setRoutes = function (item, index) {
        if (!index) {
          return {
            path: item
          }
        ...
        return {
          path: routes[index - 1].path + '/' + item
        }
      }
      curRouteList.forEach((item, index) => {
        routes.push(setRoutes(item, index))
      })
    ...
    renderIcon (icon) {
      ...
      if (icon.indexOf('fa-') !== -1) {
        ...
        props.icon = icon
        return (
          <font-awesome-icon {... { props, style: { 'margin-right': this.collapsed ? '40px' : '10px' }, class: 'font-awesome-icon' }} />
        )
      }
      return (
        <Icon {... { props }} />
