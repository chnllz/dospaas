      ...
      <a-layout>
        ...
        <a-layout-content :style="{ paddingLeft: contentPaddingLeft, display: 'flex', 'flex-direction': 'column' }">
          ...
          >
            ...
            <div v-show="$route.meta.viewConfig.query.url" key="2" style="height: 100%; overflow: hidden">
              <iframe
                v-for="(item, i) in iframeList"
                v-show="$route.meta.viewConfig.query.url === item"
...
export default {
  ...
  watch: {
    ...
    '$route': {
      handler (to, from) {
        ...
        if (this.$route.meta.viewConfig.query.url && !this.iframeList.includes(this.$route.meta.viewConfig.query.url)) {
          this.iframeList.push(this.$route.meta.viewConfig.query.url)
        }
      },
      immediate: true
    }
  ...
  methods: {
    ...
    onRefresh () {
      // iframe刷新
      const index = this.iframeList.indexOf(this.$route.meta.viewConfig.query.url)
      if (index !== -1) {
