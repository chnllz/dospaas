  ...
  <a-form :form="form">
    ...
    <a-form-item :label="$t('后置图标')" :labelCol="labelCol" :wrapperCol="wrapperCol">
      <a-checkbox-group :defaultValue="setting.form.suffixIcon || []" @change="handleLatter">
        ...
        <a-checkbox value="custom"></a-checkbox>
      </a-checkbox-group>
      <a-button size="small" @click="codeEditor('custom')">
        <a-badge v-if="attributeFlag" status="success" :text="$t('自定义')" />
        <a-badge v-else status="default" :text="$t('自定义')" />
      </a-button>
    </a-form-item>
...
export default {
  ...
  methods: {
    ...
    changeText (e) {
      this.$nextTick(() => {
        const str = this.form.getFieldValue('setting[form][prefixVal]').trim()
        if (!str) {
          this.form.setFieldsValue({
            'setting[form][prefixVal]': ''
          })
