    ...
    handleSubmit (type) {
      ...
      validateFields((errors, values) => {
        if (!errors) {
          ...
          input.value = `${chatNetworkUrl}#/?routeNumber=${this.config.record.routeNumber || ''}&visitorId=${values.info.visitorId || ''}&visitorName=${values.info.visitorName || ''}` + (this.visitorAvatar ? `&visitorAvatar=${this.visitorAvatar}` : '')
          input.select()
          try {
            if (document.execCommand('copy', false)) {
              this.$message.success(this.$t('访客链接已经生成并复制到剪贴板，请在浏览器中打开访问'))
            } else {
