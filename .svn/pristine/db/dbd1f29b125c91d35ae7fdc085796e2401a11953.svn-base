<template>
  <a-layout class="components-layout-demo-custom-trigger">
      ...
      <a-layout-content style="padding: 20px">
        <div>
          <a-input-search
            ...
            :placeholder="$t('请输入问题标题进行搜索')"
            allowClear
            @search="getDataList(1, 0)"
          />
          <div style="padding-top: 10px; margin-bottom: 10px">
            <a-button
              v-for="(value, index) in sortRule"
              :key="index"
              style="margin-right: 10px"
              :type="searchIndex === index ? 'primary' : ''"
              @click="getDataList(1, index, value.rules)"
            >
              {{ $t(value.name) }}
            </a-button>
          </div>
              ...
              <a-list :data-source="dataList">
                <a-list-item slot="renderItem" slot-scope="item">
                  <a-list-item-meta>
                    ...
                    <div slot="description" style="margin-top: 10px; user-select: none; color: rgba(0, 0, 0, 0.45)">
                      <span>
                      ...
                      <span style="float: right">{{ $t('提问时间') }}: {{ item.inputTime }}</span>
                    </div>
                  </a-list-item-meta>
                </a-list-item>
              </a-list>
    ...
    >
      ...
      <a-button style="width: 100%" type="primary" @click="questionAdd">{{ $t('我要提问') }}</a-button>

      <a-card
        size="small"
        :bordered="false"
        style="margin-top: 20px; height: 200px; padding: 40px 0; background-color: #f5f5f5"
      >
        <a-row type="flex">
          ...
          <a-col :span="1" style="text-align: center">
            <a-divider type="vertical" style="height: 100%; background-color: #d9d9d9" />
          </a-col>
      ...
      <a-divider />
      <div
        :style="{ 'font-weight': 'bold', 'font-size': '22px', color: 'rgba(0, 0, 0, 0.92)', 'margin-bottom': '20px' }"
      >
        {{ $t('热门问题分类') }}
      </div>
      <a-row>
        <a-button
          ...
          :key="value.number"
          style="margin-right: 10px; font-size: 14px; padding: 0px 20px; margin-bottom: 10px"
          @click="getDataList(1, searchIndex, sortRule[searchIndex].rules, value.number)"
        >
          {{ value.name }}
        </a-button>
      </a-row>
