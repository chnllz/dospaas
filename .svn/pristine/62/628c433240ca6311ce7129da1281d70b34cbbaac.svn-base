<template>
  <div class="page">
    ...
    <a-drawer :title="$t(config.title)" width="100%" :visible="visiblescore" @close="visiblescore = !visiblescore">
      <a-spin :spinning="loading">
                        ...
                        >
                          ...
                          <div v-if="['1013'].includes(item.chats_code)" style="max-width: 300px; width: 100vw">
                            <a-card v-if="item.contentObj.status === '取件'" size="small" style="margin: 0">
                              ...
                              <div style="padding: 8px">
                                <div style="margin-bottom: 8px">
                                  <span style="color: rgba(0, 0, 0, 0.35)">{{ $t('保管服务费') }}：</span>
                                  <span>￥{{ item.contentObj.custodyMoney }}</span>
                                </div>
                                <div style="margin-bottom: 8px">
                                  <span style="color: rgba(0, 0, 0, 0.35)">{{ $t('收派员') }}：</span>
                                  <span>{{ item.contentObj.staffMobile || '--' }}</span>
                                </div>
                                <div style="margin-bottom: 8px">
                                  <span style="color: rgba(0, 0, 0, 0.35)">{{ $t('柜机地址') }}：</span>
                                  <span>{{ item.contentObj.edAdress || '--' }}</span>
                                </div>
                                <div style="margin-bottom: 8px">
                                  <span style="color: rgba(0, 0, 0, 0.35)">{{ $t('入柜时间') }}：</span>
                                  <span>{{ item.contentObj.sendTm || '--' }}</span>
                                </div>
                                <div>
                                  <span style="color: rgba(0, 0, 0, 0.35)">{{ $t('取件时间') }}：</span>
                            ...
                            <a-card v-else-if="item.contentObj.status === '存包'" size="small" style="margin: 0">
                              ...
                              <div style="padding: 8px">
                                <div style="margin-bottom: 8px">
                                  <span style="color: rgba(0, 0, 0, 0.35)">{{ $t('订单编号') }}：</span>
                                  <span>{{ item.contentObj.orderId }}</span>
                                </div>
                                <div style="margin-bottom: 8px">
                                  <span style="color: rgba(0, 0, 0, 0.35)">{{ $t('到期时间') }}：</span>
                                  <span>{{ item.contentObj.expiredTime || '--' }}</span>
                                </div>
                                <div style="margin-bottom: 8px">
                                  <span style="color: rgba(0, 0, 0, 0.35)">{{ $t('格口编号') }}：</span>
                            ...
                            >
                              <div style="padding: 8px">
                                ...
                                <div>
                                  <span style="color: rgba(0, 0, 0, 0.35)">{{ $t('寄出时间') }}：</span>
                      ...
                      <a-card size="small" :title="$t('会话信息')" style="margin-bottom: 8px">
                        <a-form :labelCol="{ span: 8 }" :wrapperCol="{ span: 16 }">
                          <a-form-item :label="$t('会话开始时间')">
                            <a-input v-model="safe.allInfo.startTime" />
                          </a-form-item>
                          <a-form-item :label="$t('客服接待时间')">
                            <a-input v-model="safe.allInfo.serviceReceptionTime" />
                          </a-form-item>
                          <a-form-item :label="$t('会话结束时间')">
                            <a-input v-model="safe.allInfo.endTime" />
                          </a-form-item>
                          <a-form-item :label="$t('渠道')">
                            <a-input v-model="safe.allInfo.channel" />
                          </a-form-item>
                          <a-form-item :label="$t('会话结束原因')">
                            <a-input v-model="safe.allInfo.endType" />
                          </a-form-item>
                          <a-form-item :label="$t('访客ID')">
                            <a-input v-model="safe.allInfo.visitorId" />
                          </a-form-item>
                          <a-form-item :label="$t('访客名称')">
                            <a-input v-model="safe.allInfo.visitorName" />
                          </a-form-item>
                          <a-form-item :label="$t('客服账号')">
                            <a-input v-model="safe.allInfo.serviceId" />
                          </a-form-item>
                          <a-form-item :label="$t('客服昵称')">
                            <a-input v-model="safe.allInfo.serviceName" />
                          </a-form-item>
                          <a-form-item :label="$t('是否有效咨询')">
                            <a-input v-model="safe.allInfo.isvalid" />
                          </a-form-item>
                        </a-form>
                      </a-card>
                      <template>
                      ...
                      <a-card size="small" :title="$t('满意度评价')">
                        <a-form :labelCol="{ span: 8 }" :wrapperCol="{ span: 16 }">
                          <a-form-item :label="$t('评价结果')">
                            <a-input v-model="safe.allInfo.comment" />
                          </a-form-item>
                          <a-form-item :label="$t('评价标签')">
                            <a-input v-model="safe.allInfo.commentTags" />
                          </a-form-item>
                          <a-form-item :label="$t('评价备注')">
                            <a-textarea v-model="safe.allInfo.commentRemarks" :auto-size="{ minRows: 3, maxRows: 5 }" />
                          </a-form-item>
                        </a-form>
                      </a-card>
                    </a-tab-pane>
                      ...
                      <a-card size="small" :title="$t('会话消息')">
                        <a-form :labelCol="{ span: 8 }" :wrapperCol="{ span: 16 }">
                          <a-form-item :label="$t('消息总数')">
                            <a-input v-model="safe.allInfo.messageAll" />
                          </a-form-item>
                          <a-form-item :label="$t('系统消息数')">
                            <a-input v-model="safe.allInfo.messageSystem" />
                          </a-form-item>
                          <a-form-item :label="$t('客服消息数')">
                            <a-input v-model="safe.allInfo.messageService" />
                          </a-form-item>
                          <a-form-item :label="$t('访客信息数')">
                            <a-input v-model="safe.allInfo.messageVisitor" />
                          </a-form-item>
                          <a-form-item :label="$t('交互轮次')">
                            <a-input v-model="safe.allInfo.questionAnswerCount" />
                          </a-form-item>
                          <a-form-item :label="$t('客服平均消息间隔')">
                            <a-input v-model="safe.allInfo.serviceAverageReplyInterval" />
                          </a-form-item>
                          <a-form-item :label="$t('访客平均消息间隔')">
                            <a-input v-model="safe.allInfo.visitorAverageReplyInterval" />
                          </a-form-item>
                          <a-form-item :label="$t('客服撤回消息数')">
                            <a-input v-model="safe.allInfo.serviceWithdraw" />
                          </a-form-item>
                        </a-form>
                      </a-card>
                      <a-card size="small" :title="$t('会话时间')" style="margin: 8px 0px">
                        <a-form :labelCol="{ span: 8 }" :wrapperCol="{ span: 16 }">
                          <a-form-item :label="$t('会话总时长')">
                            <a-input v-model="safe.allInfo.totalTime" />
                          </a-form-item>
                          <a-form-item :label="$t('客服接待时间')">
                            <a-input v-model="safe.allInfo.serviceReceptionTime" />
                          </a-form-item>
                          <a-form-item :label="$t('访客排队时长')">
                            <a-input v-model="safe.allInfo.visitorQueueTime" />
                          </a-form-item>
                          <a-form-item :label="$t('客服服务时长')">
                            <a-input v-model="safe.allInfo.serviceDuration" />
                          </a-form-item>
                        </a-form>
                      </a-card>
                      <a-card size="small" :title="$t('会话响应')">
                        <a-form :labelCol="{ span: 8 }" :wrapperCol="{ span: 16 }">
                          <a-form-item :label="$t('客服首次响应时长')">
                            <a-input v-model="safe.allInfo.serviceReplyFirst" />
                          </a-form-item>
                          <a-form-item :label="$t('客服最大响应时长')">
                            <a-input v-model="safe.allInfo.serviceMaxRespons" />
                          </a-form-item>
                          <a-form-item :label="$t('客服超时响应轮次')">
                            <a-input v-model="safe.allInfo.serviceReplyTimeoutCount" />
                          </a-form-item>
                          <a-form-item :label="$t('客服响应总次数')">
                            <a-input v-model="safe.allInfo.serviceReplyTotalCount" />
                          </a-form-item>
                          <a-form-item :label="$t('客服响应总时长')">
                            <a-input v-model="safe.allInfo.serviceReplyTotalTime" />
                          </a-form-item>
                          <a-form-item :label="$t('平均响应时长')">
                            <a-input v-model="safe.allInfo.serviceAverageReplyTime" />
                          </a-form-item>
                        </a-form>
                      </a-card>
                    </a-tab-pane>
            ...
            <a-col span="14">
              ...
              <a-form-item>
                <a-card :title="$t('综合评语')" size="small">
                  <a-row>
                    <a-col :span="9">
                      <a-form-item :label="$t('优秀案例')" :label-col="{ span: 8 }" :wrapper-col="{ span: 12 }">
                        <a-switch
                  ...
                  <a-form-item :label="$t('综合评语')" :label-col="{ span: 3 }" :wrapper-col="{ span: 16 }">
        ...
        <div class="bbar">
          ...
          <a-button @click="visiblescore = !visiblescore">{{ $t('关闭') }}</a-button>
        </div>
      </a-spin>
    </a-drawer>
...
export default {
  ...
  props: {
    url: {
      type: String,
      default: '/quality/awfulOnline/mockInit'
    }
  },
  data () {
    return {
