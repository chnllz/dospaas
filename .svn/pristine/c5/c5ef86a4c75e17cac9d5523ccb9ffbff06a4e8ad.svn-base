<template>
  <div>
    ...
    <a-card size="small" style="margin-bottom: 16px">
      ...
      <a-form-item :label="$t('帮助说明设置')" :labelCol="labelCol" :wrapperCol="wrapperCol">
        <a-button @click="getEditor">
          <a-badge v-if="setting.helpText" status="success" :text="$t('设置')" />
          <a-badge v-else status="default" :text="$t('设置')" />
        </a-button>
        <a-modal
          :title="$t('帮助说明设置')"
        ...
        >
          <a-spin :spinning="loading">
            <a-form v-if="!loading" :form="form" :labelCol="{ span: 4 }" :wrapperCol="{ span: 20 }">
      ...
      <a-form-item
        v-if="data.type !== 'webProcessCenterDataWindow'"
        :label="$t('流程功能')"
        :labelCol="labelCol"
      ...
      <a-form-item :label="$t('备注')" :labelCol="labelCol" :wrapperCol="wrapperCol">
        <a-textarea v-decorator="['remarks', { initialValue: data.remarks }]" :auto-size="{ minRows: 1, maxRows: 5 }" />
      </a-form-item>
    ...
    <a-card size="small">
      ...
      <a-form-item
        v-if="pageName !== 'webSubformDataWindow'"
        :label="$t('列锁的起始列数')"
        :labelCol="labelCol"
        :wrapperCol="wrapperCol"
      >
