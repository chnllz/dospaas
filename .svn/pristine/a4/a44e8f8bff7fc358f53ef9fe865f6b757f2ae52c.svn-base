<template>
  <!-- 标签页布局 -->
  <div v-if="record.type === 'tabs'" :class="{ 'lr-tabs': ['left', 'right'].includes(record.options.tabPosition) }">
    <a-tabs
      v-if="['left', 'top'].includes(record.options.tabPosition)"
    ...
    >
      ...
      ></a-tab-pane>
      <!-- a-tab-pane 中 设置页签高度 -->
      <!-- :style="{ minHeight: record.height ? record.height + 'px' : '100%' }" -->
    </a-tabs>
    <div
      v-for="(tabItem, index) in record.columns.filter((item) => {
        if (item.label === '流程日志' || item.label === '催办日志') {
      ...
      :key="index"
      :style="{ flex: 1, overflow: 'hidden', minHeight: record.height ? record.height + 'px' : 'auto' }"
    >
      <buildBlocks
    ...
    <a-tabs
      v-if="['right', 'bottom'].includes(record.options.tabPosition)"
      :default-active-key="0"
      :tabBarGutter="Number(record.options.tabBarGutter)"
      :type="record.options.type"
      :size="record.options.size"
  ...
  <div v-else>
        ...
        <div style="display: flex; justify-content: center; align-items: center; margin-top: 4px">
          ...
          >
            <template slot="prefix">
              ...
              <a-icon
                v-else-if="
                  record.field.setting.form.prefixType === 'icon' &&
                  record.field.setting.form.prefixIcon.indexOf('fa-') === -1
                "
                :type="record.field.setting.form.prefixIcon"
                theme="filled"
              />
              <font-awesome-icon
                v-else-if="
                  ...
                  !(record.field.setting.form.prefixIcon.indexOf('fa-') === -1)
                "
                :icon="record.field.setting.form.prefixIcon || []"
    ...
    >
      ...
      <a-input
        ...
        v-decorator="[
          'record' +
            ...
            (record.field.beforeAlias ? '[' + record.field.beforeAlias + ']' : '[' + record.field.alias + ']'),
          {
            initialValue: record.field.option ? record.field.option[0].fullName : '',
            rules: [
              {
                required: record.field.setting.attribute.required && record.fieldRule !== 'hidden',
      ...
      />
      <a-input-group compact style="display: flex; justify-content: flex-start; align-items: center">
        <address-select
          v-if="!record.field.setting.form.coordinateShow"
        ...
        <div v-else style="flex: 1">
          <AddressSearcher
            ...
            :alias="
              ...
              (record.field.beforeAlias ? '[' + record.field.beforeAlias + ']' : '[' + record.field.alias + ']')
            "
            :option="record.field.option"
            @send="getAddress"
          />
        </div>
        <a-form-item v-if="record.field.setting.form.detailsShow === '1'" style="flex: 1">
          <a-input
            v-decorator="[
              'record' +
                (record.aliasKey ? '[product_' + record.aliasKey + ']' : '') +
                '[' +
                record.field.setting.form.details +
                ']',
              {
        ...
        <a-form-item v-if="record.field.setting.form.coordinateShow">
          <a-input
            v-show="false"
            v-decorator="[
              'record' +
                (record.aliasKey ? '[product_' + record.aliasKey + ']' : '') +
                '[' +
                record.field.setting.form.coordinate +
                ']',
              {
                initialValue: record.field.addressCoordinate
              }
            ]"
            :placeholder="record.fieldRule === 'readonly' ? '' : $t('请输入详细地址')"
            :disabled="record.fieldRule === 'readonly'"
    ...
    <!-- 子表 -->
    <a-form-item
      v-else-if="record.field && record.field.formType === 'web_sub_data_window' && record.fieldRule !== 'disabled'"
    ...
    >
      ...
      <div v-show="record.fieldRule !== 'hidden'">
        <user-table-subform-list
          ref="subformList"
          :params="{ templateId: record.field.setting.form.dataView }"
    ...
    <a-form-item
      v-if="
        ...
        (record.field &&
          ...
          record.type !== 'portrait' &&
          record.fieldRule !== 'disabled' &&
          ((record.field.alias === 'accessLevel' && userInfo.superAdmin) || record.field.alias !== 'accessLevel'))
      "
      v-show="record.fieldRule !== 'hidden'"
      :labelCol="
        record.labelShow === '0'
    ...
    >
      ...
      <!-- 多行文本 -->
      <a-textarea
        v-if="record.field.formType === 'textarea' || record.field.formType === 'json'"
        v-decorator="[
          'record' +
            (record.aliasKey ? '[product_' + record.aliasKey + ']' : '') +
            (record.field.beforeAlias ? '[' + record.field.beforeAlias + ']' : '[' + record.field.alias + ']'),
          {
            initialValue:
              record.field.formType === 'textarea'
                ? record.field.value
                : record.field.value
                ? JSON.stringify(record.field.value, null, 4)
                : '',
            rules: [
              {
                required: record.fieldRule !== 'hidden' && record.field.setting.attribute.required,
        ...
        >
          <template>
            <a-select-option
              ...
              :disabled="
                myitem.disabled ||
                (record.field.setting.form.optionalRange &&
                  record.field.setting.form.optionalRange.length > 0 &&
                  !record.field.setting.form.optionalRange.includes(myitem.value))
              "
              @click="viewThis.num++"
            >
              {{ $t(myitem.label) }}
      ...
      <div v-else-if="record.field.formType === 'cascader'">
        ...
        <tabs-select
          ...
          :fieldRule="record.fieldRule"
          :optionalRange="record.field.optionalRange"
          :notOptionalRange="record.field.notOptionalRange"
          @send="
            (val, alias, name, state, allValue) => {
      ...
      <!-- 开关 -->
      <template v-else-if="record.field.formType === 'switch'">
        <a-radio-group
          v-if="record.field.setting.form.displayMode === 'radio'"
          v-decorator="[
            'record' +
              (record.aliasKey ? '[product_' + record.aliasKey + ']' : '') +
              (record.field.beforeAlias ? '[' + record.field.beforeAlias + ']' : '[' + record.field.alias + ']'),
          ...
          "
        >
          <a-radio v-for="(item, index) in record.field.setting.form.word.value" :key="index" :value="Number(index)">
            {{ $t(item) }}
          </a-radio>
        </a-radio-group>
        <a-radio-group
          v-else-if="record.field.setting.form.displayMode === 'button'"
          v-decorator="[
            'record' +
              (record.aliasKey ? '[product_' + record.aliasKey + ']' : '') +
              (record.field.beforeAlias ? '[' + record.field.beforeAlias + ']' : '[' + record.field.alias + ']'),
        ...
        >
          <a-radio-button
            v-for="(item, index) in record.field.setting.form.word.value"
            :key="index"
            :value="Number(index)"
          >
            {{ $t(item) }}
        ...
        <a-select
          v-else-if="record.field.setting.form.displayMode === 'combobox'"
          v-decorator="[
            'record' +
              (record.aliasKey ? '[product_' + record.aliasKey + ']' : '') +
              (record.field.beforeAlias ? '[' + record.field.beforeAlias + ']' : '[' + record.field.alias + ']'),
        ...
        <div v-else style="position: relative; width: 100%; margin-top: -1px">
          <template v-if="record.field.setting.form.mode === 'combobox'">
            <a-input
              ...
              @click="
                () => {
                  if (record.fieldRule === 'readonly') {
                    return
                  }
                  handleLink(record, record.field.setting.form.mode)
                }
              "
            />
            <a-icon
      ...
      <template v-else-if="record.field && record.field.formType === 'treeselect' && record.fieldRule !== 'disabled'">
        <data-picker
          ...
          "
          :checkable="record.field.setting.attribute.inherit === '1'"
          :required="record.fieldRule !== 'hidden' && record.field.setting.attribute.required"
      ...
      <div v-else-if="record.field && record.field.formType === 'tag' && record.fieldRule !== 'disabled'">
        ...
        <tag-data-picker
          ...
          :allowClear="false"
          :tagDeafultData="record.field.tagData"
          :rnumber="record.field.setting.form.tagSetting.rnumber"
    ...
    >
      ...
      <a-upload
        ...
        "
      >
        <div v-if="isShowLoad(record)" style="height: 100%">
    ...
    <!-- 子表 -->
    <template v-else-if="record.type === 'web_sub_form_view'">
      <sub-table
        :currentRecord="record"
        :formThis="formThis"
        :viewThis="viewThis"
        @change="
          (val, subThis, changeType) => {
            viewThis.sendSubTableChange(record, val, subThis, changeType)
          }
        "
      />
    ...
    <template
      v-else-if="
        record.type === 'web_sub_data_window' &&
        record.relationTable &&
        record.relationTable.type === 'tableCardWindow' &&
        record.relationTable.enable === '1' &&
        record.relationTable.access
      "
    >
      <user-table-list ref="subDataWindow" :params="record.relationTable" :formThis="formThis" :viewThis="viewThis" />
    </template>
    <template
      v-else-if="
        record.type === 'web_sub_data_window' &&
      ...
      "
    >
      <user-table-list ref="subDataWindow" :params="record.relationTable" :formThis="formThis" />
    </template>
    <!-- 颜色选择器 -->
    <template v-else-if="record.field && record.field.formType === 'colorpicker' && record.fieldRule !== 'disabled'">
      <a-form-item
        v-show="record.fieldRule !== 'hidden'"
        :labelCol="
          record.labelShow === '0'
      ...
      >
        ...
        ></a-input>
        <a-tag
          :color="record.field.value ? record.field.value : '#ffffff'"
          style="width: 64px; height: 24px; position: relative; top: 6px; cursor: pointer; border: 1px solid #d9d9d9"
          @click.stop="
            () => {
              showColor = !showColor
            }
      ...
      <div
        v-if="showColor"
        ref="colorSketchShow"
        :style="{ position: 'absolute', zIndex: 9999, left: record.labelShow === '0' ? 0 : '36px' }"
      >
        <sketch-picker
          class="sketch-picker"
          :value="record.field.value ? record.field.value : '#cdcdcd'"
          :presetColors="['#F5222D', '#FA541C', '#FAAD14', '#13C2C2', '#52C41A', '#1890FF', '#2F54EB', '#722ED1']"
          @input="
            (val) => {
              $set(record.field, 'value', val.hex8)
              const obj = {}
              obj['record' + '[' + record.field.alias + ']'] = val.hex8
              $emit('give', obj)
            }
          "
        />
    ...
    <div v-else-if="record.type === 'flowlog'">
      <template v-if="record.showType === 'card'">
        <a-row>
          ...
          <a-col :span="12" style="text-align: right">
            <div style="display: flex; align-items: center">
              <a-input-search
                v-model="queryParamLog.handleRemarks"
                placeholder="请输入备注内容"
                @search="
                  () => {
                    queryParamLog.pageNo = 1
                ...
                <template slot="content">
                  <a-form style="width: 600px" :labelCol="{ span: 4 }" :wrapperCol="{ span: 20 }" :colon="false">
                    ...
                    <a-form-item :label="$t('办理备注')">
                      <a-input v-model="queryParamLog.handleRemarks" />
                    </a-form-item>
        ...
        <a-timeline style="width: 100%; margin-top: 8px" :pending="flowLogLoading ? $t('加载中') : undefined">
          <a-timeline-item
            ...
            :key="flowItem.id"
            :color="flowItem.handleWay === 'finish' ? 'red' : flowItem.handleWay === 'current' ? 'green' : 'gray'"
          >
            <a-row :gutter="[0, 8]">
              <a-col :span="12">
                <a-tag
                  :color="flowItem.handleWay === 'finish' ? 'red' : flowItem.handleWay === 'current' ? 'green' : ''"
                >
                  {{
              ...
              <a-col :span="24">
                <a-card v-if="flowItem.handleWay === 'finish'" size="small">
                ...
                <a-card v-else-if="flowItem.handleWay === 'current'" size="small">
                  <a-descriptions :column="2">
                    <a-descriptions-item :label="$t('当前任务名称')">{{ flowItem.transitionName }}</a-descriptions-item>
                ...
                <a-card v-else size="small">
                  <a-descriptions :column="2">
                    <a-descriptions-item :label="$t('任务名称')">{{ flowItem.transitionName }}</a-descriptions-item>
                    <a-descriptions-item :label="$t('办理人')">{{ flowItem.inputUser || '--' }}</a-descriptions-item>
                    <a-descriptions-item :label="$t('办理方式')">
                      {{
                        flowItem.operation && flowItem.operation === 'handle'
                          ? ''
                          : `${operationMap[flowItem.operation] || '--'} / `
                      }}{{ flowItem.handleWay }}
                    </a-descriptions-item>
                    <a-descriptions-item :label="$t('办理部门')">{{ flowItem.inputDepartment }}</a-descriptions-item>
                    <a-descriptions-item :label="$t('办理备注')" :span="2">
                      {{ flowItem.handleRemarks || '--' }}
                    </a-descriptions-item>
                  </a-descriptions>
                  <a-row>
                    <a-col
      ...
      <template v-else>
        ...
        >
          ...
          <div slot="handleWay" slot-scope="text, obj">
            <div>
              {{ !obj.operation || obj.operation === 'handle' ? '' : `${operationMap[obj.operation] || '--'} / ` }}
              {{ text || '--' }}
            </div>
          </div>
          <div slot="handleRemarks" slot-scope="text" v-dompurify-html="text || '--'"></div>
        </s-table>
      </template>
    </div>
      ...
      <div v-if="parentParams.action === 'edit'">
        <a-timeline v-if="traceData.length > 0">
          <a-timeline-item v-for="option in traceData" :key="option.id">
            <h3>{{ option.updateTime }} {{ option.updateUser }}({{ option.realName }})</h3>
            <a-table
              size="small"
              :pagination="false"
              rowKey="fieldAlias"
    ...
    <a-form-item v-else-if="record.type === 'portrait'" style="padding-right: 20px">
      <a-row type="flex" align="middle">
        ...
        <a-col>
          <a-tag color="red">{{ $t('多次投诉') }}</a-tag>
          <a-tag color="red">{{ $t('多产品') }}</a-tag>
          <a-tag color="red">{{ $t('VIP客户') }}</a-tag>
    ...
    >
      <a-textarea
        ...
        :autoSize="{
          minRows: params.remarksMinRows || 1,
          maxRows: params.remarksMaxRows || params.remarksMinRows || 4
        }"
      />
    </a-form-item>
    <!-- 关联数据 -->
...
export default {
  ...
  components: {
    ...
    AddressSearcher: () => import('../Field/AddressSearcher'),
    ...
    LocationSetMap: () => import('@/views/admin/Field/LocationSetMap'),
    ...
    return {
      ...
      columnsMark: [{
        ...
        customRender: (text, record, index) => {
          return this.renderFormmarkCol(text, record, index, 'before')
        }
      }, {
        title: this.$t('变更后'),
        dataIndex: 'after',
        customRender: (text, record, index) => {
          return this.renderFormmarkCol(text, record, index, 'after')
        }
      }],
      typeActions: new Map([[
        '', value => value
        ...
        'json', (value) => value.target ? value.target.value : value
      ], [
      ...
      actions: new Map([[
        ...
        'bw', function (value, inputValue) {
          return inputValue.indexOf(value) === 0
        }], [
        'ew', function (value, inputValue) {
          const len = value.length
          return inputValue.slice(-len) === value
        }], [
        'em', function (value, inputValue) {
          return inputValue === '' || inputValue === null || inputValue === undefined || (Object.prototype.toString.call(inputValue) === '[object Array]' && inputValue.length === 0)
        }], [
        'nem', function (value, inputValue) {
      ...
      columnsLog: [{
        ...
        title: this.$t('办理方式'),
        dataIndex: 'handleWay',
        scopedSlots: { customRender: 'handleWay' }
      }, {
        title: this.$t('办理备注'),
        dataIndex: 'handleRemarks',
        ellipsis: false,
        width: 280,
        scopedSlots: { customRender: 'handleRemarks' }
      }],
      advancedUrge: false,
      // 催办日志表头
      columnsUrge: [{
  ...
  watch: {
    record: {
      handler (newVal) {
        ...
        } else if (newVal.field && newVal.field.formType === 'combobox') {
          if (newVal.field.setting.attribute.dataSource === 'linkData' && !newVal.field.setting.form.defaultValueLink.currentFieldCause && !newVal.field.setting.form.defaultValueLink.sourceFieldCause) {
            this.axios({
              url: '/admin/general/getLinkDataOptions',
              data: {
                dataSource: true,
                field: newVal.field.setting.form.defaultValueLink.sourceFieldEffect,
                tableId: newVal.field.setting.form.defaultValueLink.dataSource[1]
              }
            }).then(res => {
              this.$emit('sentLinkData', newVal.field.alias, res.result)
            })
          }
        }
      },
  ...
  created () {
    ...
    if (this.record.field && this.record.formType === 'organization') {
      ...
      this.axios({
        url: '/admin/general/getFieldTraceData',
      ...
      this.record.field.value = this.record.field.value.map(valItem => {
        const obj = valItem
        obj.url = this.$store.state.env.VUE_APP_API_BASE_URL + 'admin/api/download/?filePath=' + obj.url
        return obj
      })
  ...
  methods: {
    renderFormmarkCol (text, record, index, type) {
      if (record.fromType === 'image') {
        const imgList = record[type] ? JSON.parse(record[type]) : []
        if (imgList.length > 0) {
          return (
            <div>
              {imgList.map(item => {
                return (
                  <img src={this.$store.state.env.VUE_APP_API_BASE_URL + 'admin/api/download/?filePath=' + item.filePath}
                    style="width: 64px; height: 64px; cursor: pointer; margin-right: 2px"
                  />
                )
              })}
            </div>
      ...
      } else if (record.fromType === 'file') {
        const fileList = record[type] ? JSON.parse(record[type]) : []
        if (fileList.length > 0) {
          return (
            fileList.map(item => {
              return (
                <a href={this.$store.state.env.VUE_APP_API_BASE_URL + 'admin/api/download/?filePath=' + item.filePath} target={'_blank'} style={'margin:0 10px'}>{item.fileName}</a>
              )
            })
          )
        } else {
      ...
      } else if (record.fromType === 'treeselect') {
        const tagList = record[type] ? record[type] : [] // record[type] 没有值为 空字符串
        if (tagList.length > 0) {
          return (
            <div>
              {tagList.map(item => {
      ...
      } else if (['checkbox', 'combobox', 'radio', 'organization', 'tag'].includes(record.fromType)) {
        const result = record[type] ? record[type] : [] // record[type] 没有值为 空字符串
        if (result.length > 0) {
          return (
            <div>
              {result.map(item => {
      ...
      } else if (record.fromType === 'score') {
        return (
          <a-rate value={Number(record[type])} disabled allow-half />
        )
      } else if (record.fromType === 'colorpicker') {
        const style = `width:24px;height:24px;border: 1px solid gray;background-color:${record[type]}`
        return (
          <div style={style}></div>
        )
      } else {
        if (record[type]) {
          return (
            <div v-dompurify-html={record[type]}></div>
          )
        } else {
          return (
            <div>--</div>
    ...
    normFile (e) {
      ...
      // 如果beforeUpload返回false, 这里的status,response 为undifined
      // 过滤被beforeUpload拦截的文件
      if (e.fileList.length > 0 && e.fileList.some(item => !item.response && !item.status)) {
        e.fileList = e.fileList.filter(item => item.status)
      }
      // 过滤得到上传成功的图片
    ...
    getLinkList (item) {
      ...
      const templateId = item.field.setting.autofill.sourceLinkView
      this.viewThis.pageLoading = true
      this.axios({
        url: '/admin/general/getLinkRecord',
    ...
    loadDataLog (parameter) {
      ...
      return this.axios({
        ...
        if (parameter.pageNo === 1) {
          ...
          this.processSubStatusDict = res.result.processSubStatusDict ? res.result.processSubStatusDict : []
          this.operationMap = res.result.operationMap
          if (res.result.currentLog) {
            res.result.currentLog.id = '--'
            res.result.data.unshift(res.result.currentLog)
          } else if (res.result.finishLog) {
            res.result.finishLog.id = '--'
    ...
    getFlowLog (type) {
      ...
      this.axios({
        url: '/admin/centerflow/workflowLog',
        data: Object.assign(queryParam, { caseId: this.parentParams.caseId || this.parentParams.record?.caseId })
      }).then(res => {
        res.result.data.forEach(item => {
        ...
        if (this.queryParamLog.pageNo === 1) {
          ...
          this.operationMap = res.result.operationMap
          if (res.result.currentLog) {
            res.result.currentLog.handleWay = 'current'
            this.flowLogList.unshift(res.result.currentLog)
          } else if (res.result.finishLog) {
            res.result.finishLog.handleWay = 'finish'
    ...
    // 上传图片按钮限制
    isShowLoad (record) {
      const values = this.viewThis.form.getFieldsValue()['record'] || {}
      const value = values[record.field.alias] || ''
      return record.field.value.length < record.field.setting.attribute.maxFiles && value.length < record.field.setting.attribute.maxFiles
    },
    // 图片预览
    ...
    /**
    * beforeUpload 返回false并不能阻碍图片添加到上传文件列表
    * beforeUpload 返回Promise 对象可以阻碍图片添加到上传文件列表
    * 这里如果返回Promise会造成图片上传多张,发起多次请求,却只上传最后一张图片
    */
    // 附件限制条件
    beforeUpload (file, fileList, item) {
      ...
      if (!isLt2M) {
        const message = this.$t('上传文件大小超过{0}', { 0: item.field.setting.form.size + 'MB' })
    ...
    onCheckAll (e, item, index) {
      ...
      const checkedList = e.target.checked ? item.optionValue : []
      // 全选情况下，需排除禁选项
      if (item.field.setting.form.optionalRange && item.field.setting.form.optionalRange.length > 0) {
        item.checkedList = []
        checkedList.forEach(checkedItem => {
          if (item.field.setting.form.optionalRange.includes(checkedItem)) {
            item.checkedList.push(checkedItem)
          }
        })
      } else {
      ...
      if (item.field.setting.attribute.dataSource !== 'custom') {
        if (value) {
          ...
          item.timeoutId = setTimeout(() => {
            this.axios({
              url: '/admin/general/getAutocompleteData/',
    ...
    getAddress (name, number, alias, allValue, details, coordinate) {
      ...
      if (details) {
        const detailsObject = {}
        const name = `record[${this.record.field.setting.form.details}]`
        detailsObject[name] = details
        this.viewThis.form.setFieldsValue(detailsObject)
      }
      if (coordinate) {
        const coordinateObject = {}
        const name = `record[${this.record.field.setting.form.coordinate}]`
        coordinateObject[name] = coordinate
        this.viewThis.form.setFieldsValue(coordinateObject)
      }
    ...
    // 打开地图
    openMap (record, action) {
      if (record.fieldrule === 'readonly' && !record.field.value) {
        this.$message.info('位置未设置')
        return false
      }
      let adds = ''
      const values = this.viewThis.form.getFieldsValue()['record'] || {}
      const addressValue = values[record.field.alias] || {}
      const details = addressValue.details || values[record.field.setting.form.details]
      if (addressValue && addressValue.address) {
        const addArr = addressValue.address ? addressValue.address.split('/') : ['', '', '']
        adds = addArr[0] + addArr[1] + addArr[2]
        if (details) {
          adds = adds + details
        }
      }

      const coordinateAlias = record.field.setting.form.coordinate
      this.$refs['locationSet' + record.field.alias] && this.$refs['locationSet' + record.field.alias].show({
        title: '地图选点',
        action: 'set',
        data: values[coordinateAlias],
        search: adds
      })
    },
    // 选择地址
    getCoordinate (data) {
      const obj = {}
      const name = `record[${this.record.field.alias}][coordinate]`
      obj[name] = data
    ...
    getcascaderValue (val, alias, name, state, allValue) {
      ...
      this.viewThis.cascadeAllvalue[alias] = allValue
      this.$emit('give', obj)
      ...
      if (item.aliasKey) {
              ...
              array.forEach((temItem, index) => {
                if (temItem.columns) {
                  ...
                  if (temItem.field && temItem.field.alias === (this.linkData[item.field.beforeAlias].currentFieldEffect + '_' + item.aliasKey)) {
                    this.$set(temItem.field, 'option', res.result)
                  }
                }
              })
            }
        ...
        if (inputValue) {
          if (this.linkData[alias] && !this.linkData[alias].sourceFieldEffect) {
            ...
            const defaultEffect = this.viewThis.defaultLinkData[alias] && this.viewThis.defaultLinkData[alias].currentFieldEffect
            if (flag && !formData[defaultEffect]) {
              ...
              this.axios({
                ...
                const effectIndex = this.templateObj[effect] ? this.templateObj[effect].index : -1
                if (effectIndex !== -1) {
                  this.$emit('sentLinkData', effect, res.result)
                }
              })
            }
          }
    ...
    getWhere (alias, value, type) {
      ...
      function getRes (alias) {
        ...
        const value = type === 'change' ? that.viewThis.form.getFieldValue('record' + '[' + causeAlias + ']') : that.templateObj[causeAlias].field.value
    ...
    handleSimple (include, value, inputValue, allValue, all) {
      ...
      return action && action(value, inputValue, allValue, all)
    },
    // 选择办理方式
    handleWayChange (e) {
    ...
    handleWayProRule (value) {
      const handleFormRules = this.params.handleFormRules
      const wayRuleAlias = []
      this.handleWayData.forEach(item => {
        if (item.id === value) {
          item.resetTem = []
          handleFormRules.forEach(wayItem => {
            ...
            if (wayTrue && !currentTrue) {
              ...
              wayItem.simpleCondition.forEach(simItem => {
                if (simItem.fieldUser === 'way') {
                  ...
                  simItem.condition.forEach(myitem => {
                    ...
                    const allValue = this.viewThis.cascadeAllvalue[`${this.tableName}[${simItem.field}]`]
              ...
              if (wayItem.flag) {
                ...
                if (wayItem.event.some(someItem => someItem.type !== 'way')) {
                  // item.resetTem = rest
                  item.resetTem = [...item.resetTem, ...rest]
                }
              }
            }
          })
    ...
    onChange (item, type, options1, options2, allValue) {
      ...
      const handleFormRules = this.params.handleFormRules
      if (handleFormRules && handleFormRules.length) {
        handleFormRules.forEach(dataItem => {
          const simpleFlagArr = []
          dataItem.simpleCondition.forEach(simItem => {
            const conditionFlagArr = []
          ...
          if (dataItem.flag) {
            ...
            handleFormRules.forEach(ruleItem => {
              if (!ruleItem.flag) {
                this.handleWayData.forEach(handleItem => { // !handleItem.change
    ...
    getEnable () {
      if (this.record.action === 'edit') {
        this.axios({
          ...
          data: {
            action: 'edit',
            listTemplateId: '069d7670919b354b9576cd619604a96f',
            record: this.record.data,
            templateId: '414e1bd4120df8786b035a9d160d0f87'
          }
        }).then(res => {
    ...
    handleCall (record) {
      const field = 'record' +
        (record.aliasKey ? '[product_' + record.aliasKey + ']' : '') +
        (record.field.beforeAlias ? '[' + record.field.beforeAlias + ']' : '[' + record.field.alias + ']')
      const extensionDst = this.formThis.$refs.userTableFormView.form.getFieldValue(field)
      if (!this.userInfo.extension) {
        this.$message.info(this.$t('您的账号未配置分机号码'))
        return
      }
      if (String(extensionDst) && /^\d+$/.test(extensionDst)) {
        this.axios({
          url: '/callcenter/api/call',
          data: {
            extension: this.userInfo.extension,
            extensionDst
          }
        })
        return
      }
      this.$message.info(this.$t('被叫号码不符合规范'))
    },
    handleSend (record) {
      const field = 'record' +
        (record.aliasKey ? '[product_' + record.aliasKey + ']' : '') +
        (record.field.beforeAlias ? '[' + record.field.beforeAlias + ']' : '[' + record.field.alias + ']')
      const phone = this.formThis.$refs.userTableFormView.form.getFieldValue(field)
      if (String(phone) && /^\d+$/.test(phone)) {
        this.formThis.sendMessage({})
        return
      }
      this.$message.info(this.$t('电话号码不符合规范'))
    },
    // 组织结构options转换
          ...
          this.axios({
            ...
            if (!res.code) {
              const imageObj = {
                ...
                url: this.$store.state.env.VUE_APP_API_BASE_URL + 'admin/api/download/?filePath=' + res.result.filePath
              }
              const obj = {}
      ...
      this.$refs.portraitFormView.form.validateFields((err, values) => {
        if (!err) {
          ...
          const getTemplate = (array) => {
            array.forEach((item, index) => {
              if (item.columns) {
                ...
                if (item.formType === 'file' || item.formType === 'image') {
                ...
                } else if (item.formType === 'web_sub_data_window') {
                  ...
                  const address = 'record' + '[' + item.field.alias + ']'
                  values['record'][item.field.alias] = this.$refs.portraitFormView.addressValue[address] || ''
          ...
          this.axios({
            ...
            data: Object.assign(portraitValue, { action: 'submit', id: this.record.data ? this.record.data.id : 0, templateId: '414e1bd4120df8786b035a9d160d0f87' })
          }).then(res => {
            if (res.code === 0) {
              ...
              this.axios({
                ...
                data: {
                  ...
                  templateId: '069d7670919b354b9576cd619604a96f'
                }
              }).then(res => {
    ...
    handleSearch (val, item, index) {
      ...
      if (info.form.mode !== 'dialog') {
        const associatedSearches = [info.form.linkField, info.form.searchField_1, info.form.searchField_2, info.form.searchField_3]
        // const associatedSearch = {}
        // associatedSearch.conditions = []
        // associatedSearch.logic = 'or'
        // // associatedSearch.conditions.push(this.getCondition(info.form.linkField, val))
        // if (info.form.searchField_1) {
        //   associatedSearch.conditions.push(this.getCondition(info.form.searchField_1, val))
        // }
        // if (info.form.searchField_2) {
        //   associatedSearch.conditions.push(this.getCondition(info.form.searchField_2, val))
        // }
        // if (info.form.searchField_3) {
        //   associatedSearch.conditions.push(this.getCondition(info.form.searchField_3, val))
        // }
        const parameter = {
          pageNo: 1,
          pageSize: 20,
        ...
        const associatedFieldFilterConditions = []
        for (const i in this.formdata) {
          for (const j in info.form.associated) {
            if (i === info.form.associated[j].field) {
              associatedFieldFilterConditions[j] = {}
              associatedFieldFilterConditions[j].value = this.viewThis.form.getFieldsValue()['record'][info.form.associated[j].field]
              associatedFieldFilterConditions[j].comparison = info.form.associated[j].comparison
              associatedFieldFilterConditions[j].field = info.form.associated[j].sourceField
              if (!associatedFieldFilterConditions[j].value) {
                associatedFieldFilterConditions[j].value = ''
              }
            }
          }
        }
        ...
        this.timeout = setTimeout(() => {
          this.axios({
            url: '/admin/general/associated/query',
            data: Object.assign(parameter, { tableId: info.form.linkList[1], fieldId: item.field.fieldId }, { associatedSearches }, { associatedFieldFilterConditions }, { value: val })
          }).then(res => {
            const selectList = []
    ...
    handleChoice (item1, item, index) {
      this.axios({
        ...
        if (res.code === 0) {
          let result = []
          if (res.result.fields && res.result.fields.length > 0 && res.result.data && Object.keys(res.result.data).length > 0) {
            result = res.result.fields.map(fieldItem => {
              fieldItem.value = res.result.data[fieldItem.alias] || ''
              // 图片，附件做特殊处理
              if (fieldItem.formType === 'file' || fieldItem.formType === 'image') {
                fieldItem.value = fieldItem.value ? JSON.parse(fieldItem.value) : []
              }
              return fieldItem
            })
          }
          this.getVal(result, item.field.alias, item.field.setting)
        }
      })
    ...
    handleWinlist (item) {
      ...
      for (const i in formData) {
        for (const j in info.form.associated) {
          if (i === info.form.associated[j].field) {
            filtersData[j] = {}
            filtersData[j].value = this.viewThis.form.getFieldsValue()['record'][info.form.associated[j].field]
            filtersData[j].comparison = info.form.associated[j].comparison
            filtersData[j].field = info.form.associated[j].sourceField
            if (!filtersData[j].value) {
              filtersData[j].value = ''
            }
          }
        }
    ...
    // 获取关联数据
    getVal (fieldMapping, alias, info, record, screenData) {
      ...
      fieldMapping.forEach(item => {
        ...
        fieldMappingObj[item.alias] = item
      })
      if (screenData && screenData.aliasKey) {
          ...
          info.form.conditions.forEach(item => {
            ...
            if (temItem) {
              ...
              if (temItem.formType === 'switch') {
                const index = parseInt(valObj[item.sourceField])
                ...
                const address = 'record' + '[product_' + screenData.aliasKey + ']' + '[' + item.field + ']'
        ...
        if (info.form.conditions && info.form.conditions.length > 0) {
          info.form.conditions.forEach(item => {
            if (this.templateObj[item.field]) {
              const obj = {}
              ...
              if (temItem.formType === 'switch') {
                ...
                const address = 'record' + '[' + item.field + ']'
                let addressCode = ''
                const getTemplate = (array) => {
                  array.forEach((getItem, index) => {
                    if (getItem.columns) {
                      ...
                      if (getItem.field && temItem.field.alias === getItem.field.alias) {
                        ...
                        getItem.field.option = fieldMappingObj.option
                        addressCode = valObj[item.sourceField]
                        // 回写详细地址
                        const detailAlias = getItem.field.setting.form?.details
                        if (detailAlias) {
                          let detailSourceField = ''
                          info.form.conditions.forEach(im => {
                            if (im.field === detailAlias) { detailSourceField = im.sourceField }
                          })
                          const details = 'record' + '[' + detailAlias + ']'
                          obj[details] = valObj[detailSourceField]
                        }
                      }
                    }
                  })
                ...
                this.$emit('give', obj, addressCode, address)
                this.$set(temItem.field, 'value', addressCode)
              } else if (temItem.formType === 'cascader') {
                this.$set(temItem.field, 'value', valObj[item.sourceField])
              } else if (temItem.formType === 'file' || temItem.formType === 'image') {
                const name = 'record' + '[' + item.field + ']'
                const files = valObj[item.sourceField]
                obj[name] = files.map(im => {
                  return {
                    name: im.fileName,
                    response: {
                      code: 0,
                    ...
                    status: 'done',
                    uid: new Date().valueOf(),
                    url: this.$store.state.env.VUE_APP_API_BASE_URL + 'admin/api/download/?filePath=' + im.filePath
                  }
                })
                this.$emit('give', obj)
              } else if (temItem.formType === 'tag') {
                const tagDataList = fieldMapping.find(itemField => itemField.alias === temItem.field.alias)
                temItem.field.tagData = tagDataList ? tagDataList.tagData : []
                const name = 'record' + '[' + item.field + ']'
                const arr = valObj[item.sourceField] ? valObj[item.sourceField].split(',') : []
                temItem.field.value = arr
                obj[name] = arr
                this.$emit('give', obj)
              } else {
                const name = 'record' + '[' + item.field + ']'
