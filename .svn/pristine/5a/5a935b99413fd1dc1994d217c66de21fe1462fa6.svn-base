<template>
  ...
  <div v-if="record.type === 'tabs'" :class="{ 'lr-tabs': ['left', 'right'].includes(record.options.tabPosition) }">
    ...
    <div
      ...
      :key="index"
      class="content-tabtab"
      :style="{
        flex: 1,
        overflow: 'auto',
        minHeight: record.height ? record.height + 'px' : '500px',
        maxHeight: '760px'
      }"
    >
      <div v-if="activeKeyList.includes(index)" style="height: 100%">
        <buildBlocks
  ...
  <div v-else :style="record.type === 'web_sub_data_window' ? { height: '100%' } : {}">
    <!-- 单行文本 -->
    <a-form-item
      v-if="record.field && record.field.formType === 'text' && record.fieldRule !== 'disabled'"
      v-show="record.fieldRule !== 'hidden'"
      :help="`${$t(record.field.setting.attribute.help)}`"
      :labelCol="
        record.labelShow === '0'
    ...
    >
      ...
      <a-input-number
        ...
        style="width: 100%"
        :min="
          String(record.field.setting.attribute.minValue) === '0'
            ? 0
            : record.field.setting.attribute.minValue
            ? Number(record.field.setting.attribute.minValue)
            : -Infinity
        "
        :max="
          String(record.field.setting.attribute.maxValue) === '0'
            ? 0
            : record.field.setting.attribute.maxValue
            ? Number(record.field.setting.attribute.maxValue)
            : Infinity
        "
      ...
      <a-switch
        ...
        v-decorator="[
          'record' +
            ...
            (record.field.beforeAlias ? '[' + record.field.beforeAlias + ']' : '[' + record.field.alias + ']'),
          {
            initialValue: Number(record.field.value) === 1,
...
import UserTableComponents from '../General/UserTableComponents'
...
export default {
  ...
  components: {
    UserTableComponents,
    UserTableList: () => import('../General/UserTableList'),
    UserTableSubformList: () => import('../General/UserTableSubformList'),
    UserTableWinList: () => import('../General/UserTableWinList'),
    UserTableForm: () => import('../General/UserTableForm'),
    ...
    UserTableFormView: () => import('../General/UserTableFormView'),
  ...
  created () {
    ...
    // this.record.field.fieldId = this.record.field.fieldId ? this.record.field.fieldId : null
      ...
      if (screenData && screenData.aliasKey) {
        ...
        if (info.form.conditions && info.form.conditions.length > 0) {
          info.form.conditions.forEach(item => {
            ...
            if (item.formType !== 'web_sub_data_window' && temItem) {
              const obj = {}
              // todo
        ...
        if (info.form.conditions && info.form.conditions.length > 0) {
          info.form.conditions.forEach(item => {
            if (item.formType !== 'web_sub_data_window' && this.templateObj[item.field]) {
              const obj = {}
              const temItem = this.templateObj[item.field]
              if (temItem.formType === 'switch') {
                // todo
