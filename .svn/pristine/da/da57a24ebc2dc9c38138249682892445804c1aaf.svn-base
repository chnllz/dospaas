    ...
    <a-spin :spinning="loading">
      <a-tabs v-model="currentTab" :animated="false" @change="handleTabChange">
        <a-tab-pane v-for="parentItem in tabList" :key="parentItem.key" :tab="parentItem.title">
          <a-form :labelCol="{ span: 6 }" :wrapperCol="{ span: 18 }" class="search">
            <a-card size="small" :title="$t('搜索')">
              ...
              <a-row class="form" :class="advanced ? 'advanced' : 'normal'">
                <a-col :span="6">
                  <a-form-item :label="$t('会话开始时间')" :labelCol="{ span: 8 }" :wrapperCol="{ span: 16 }">
                    <a-range-picker
                ...
                <a-col :span="6">
                  <a-form-item :label="$t('客服名称')" :labelCol="{ span: 8 }" :wrapperCol="{ span: 16 }">
                    <a-tree-select
                ...
                <a-col :span="6">
                  <a-form-item :label="$t('首响合格时长')" :labelCol="{ span: 8 }" :wrapperCol="{ span: 16 }">
                ...
                <a-col :span="6">
                  <a-form-item :label="$t('平均响应合格时长')" :labelCol="{ span: 8 }" :wrapperCol="{ span: 16 }">
                    <div style="display: flex; margin-top: 4px">
                      <a-input-number
          ...
          <a-table
            ref="table"
            :scroll="{ x: true, y: windowHeight - 385 }"
            class="table-fill"
            size="small"
            rowKey="serviceId"
            :columns="columns"
...
export default {
  ...
  mounted () {
    ...
    this.axios({
      ...
      strArr.forEach(item => {
        this.$set(this.showObj, ['responseTime' + item], res.result.info.firstResponseQualifiedTime || 30)
        this.$set(this.showObj, ['replyTime' + item], res.result.info.averageResponseQualifiedTime || 30)
        this.$set(this.showObj, ['serviceId' + item], undefined)
      })
    })
    this.axios({
  ...
  methods: {
    getDataList () {
      ...
      if (this.queryParam.replyTime > 60) {
        this.$message.warning(this.$t('平均响应合格时长不能超过60'))
        return false
      }
      if (this.queryParam.responseTime > 60) {
        this.$message.warning(this.$t('首响合格时长不能超过60'))
    ...
    handleTabChange (activeKey) {
      ...
      this.content = activeKey === 'month'
        ? `${this.$t('会话量：访客被客服接起的数量。')}<br />
                ${this.$t('月均会话量：会话量/查询月数。')}<br />
                ${this.$t('平均会话时长：总会话时长/总会话量。')}<br />
                ${this.$t('访客平均排队时长：总排队时长/总会话量（排队时长为访客开始排队到访客被客服接起的时间间隔）。')}<br />
                ${this.$t('平均首次响应时长：首次响应总时长/总会话量（排除没有产生首次响应的会话）。首次响应时长：访客被客服接起，访客发送首条消息后，客服第一次回复的时间间隔。')}<br />
                ${this.$t('首次响应合格率：首次响应时长≤统计设置下{首次响应合格时长}的会话量/总会话量（排除没有产生首次响应的会话）。')}<br />
