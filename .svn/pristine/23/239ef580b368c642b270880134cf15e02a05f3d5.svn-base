        ...
        <a-tabs v-model="activeid">
          <a-tab-pane key="1" :tab="$t('基础设置')">
            ...
            <a-form-item :label="$t('创建模式')" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <a-select
                v-decorator="['info[workflowSetting][createType]', { initialValue: setting.createType || 'handle' }]"
              >
            ...
            <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol">
              ...
              <a-select
                v-decorator="['info[workflowSetting][callbackName]', { initialValue: setting.callbackName || '' }]"
                size="small"
                show-search
                :allowClear="true"
                option-filter-prop="children"
                @change="
                  (value) => {
                    $set(setting, 'callbackName', value)
                  }
                "
              >
                <a-select-option v-for="(item, index) in callbackClassOptions" :key="index" :value="item.value">
                  {{ item.text }}
                </a-select-option>
              </a-select>
            </a-form-item>
            <a-form-item :label="$t('工作日历')" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <a-select
                v-decorator="['info[workflowSetting][workCalendar]', { initialValue: setting.workCalendar || '' }]"
              >
            ...
            <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol">
              ...
              <a-textarea
                v-decorator="[
                  'info[workflowSetting][caseSummary]',
                  {
            ...
            <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol">
              ...
              <a-input
                v-decorator="[
                  'info[workflowSetting][templateAction]',
                  {
            ...
            <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol">
              ...
              <a-textarea
                v-decorator="[
                  'info[workflowSetting][templateDescription]',
                  {
          ...
          <a-tab-pane key="2" :tab="$t('查看视图')">
            <a-divider orientation="left">{{ $t('web端') }}</a-divider>
            ...
            <a-divider orientation="left">{{ $t('移动端') }}</a-divider>
            <a-tabs defaultActiveKey="3">
              <a-tab-pane key="3" :tab="$t('表单应用')">
                <a-alert
                  :message="$t('查看表单视图默认会设置为全部只读，如需要编辑功能，需自定义设置')"
                ...
                <FormApply
                  ref="appformapply"
                  :params="{ formViewMappings: appFormViewMappings, tableId: data.tableId }"
                  page="workflowForm"
                  :formViewData="appBindingFormViews"
                />
              </a-tab-pane>
              <a-tab-pane key="4" :tab="$t('扩展按钮')">
                <ExtendButton ref="appextendbutton" :extendBarMenuData="formViewButtons" flowType="workflow" />
              </a-tab-pane>
            </a-tabs>
          </a-tab-pane>
          ...
          <a-tab-pane key="6" :tab="$t('流程状态设置')">
            <dict-data-list
              ...
              :config="{
                action: 'list',
                record: {
                  dictCategoryNumber: 'process-status-' + (config && config.record ? config.record.workflowId : '')
                },
                type: 'son'
              }"
          ...
          <a-tab-pane key="7" :tab="$t('流程子状态设置')">
            <dict-data-list
              ...
              :config="{
                action: 'list',
                record: {
                  dictCategoryNumber: 'process-sub-status-' + (config && config.record ? config.record.workflowId : '')
                },
                type: 'son'
              }"
            />
            ...
            >
              ...
              >
                <a-select
                  ...
                  "
                >
                  <a-select-option v-for="pro in workflowFields" :key="pro.alias" :value="pro.alias">
                    {{ pro.name }}
                  </a-select-option>
                </a-select>
              </a-form-item>
              ...
              >
                <a-select
                  ...
                  "
                >
                  <a-select-option v-for="(value, key) in tableFields" :key="key" :value="value.alias">
                    {{ value.name }}
                  </a-select-option>
                </a-select>
              </a-form-item>
              ...
              >
                <a-select
                  ...
                  :disabled="!userInfo.superAdmin && record.required === '1'"
                  @change="
                    (e) => {
          ...
          <a-tab-pane key="10" :tab="$t('流程操作')">
            <workflowOperation
              ref="workflowOperation"
              :workflowList="workflowList"
              :workflowOperations="setting.workflowOperations"
              :tableFields="tableFields"
              :callbackName="setting.callbackName"
              type="global"
            />
          </a-tab-pane>
        </a-tabs>
...
export default {
  ...
  components: {
    ...
    WorkflowOperation: () => import('./WorkflowOperation')
  },
  data () {
    return {
      config: {},
      ...
      writeBackMappings: [{
        ...
        processField: 'workflowId',
        serviceField: 'workflowId',
        writebackMode: '1',
        required: '1',
        remark: this.$t('32位编码，用于标识是哪一个工作流')
      }, {
        processField: 'caseStatus',
        serviceField: 'caseStatus',
        writebackMode: '1',
        required: '1',
        remark: this.$t('工作流实例的状态，如：op、cl、ca、su')
      }, {
        processField: 'inputTime',
        serviceField: 'inputTime',
        writebackMode: '1',
        required: '1',
        remark: this.$t('流程开始时间')
      }, {
        processField: 'endTime',
        serviceField: 'endTime',
        writebackMode: '1',
        required: '1',
        remark: this.$t('流程结束时间')
      }, {
        processField: 'inputUser',
        serviceField: 'inputUser',
        writebackMode: '1',
        required: '1',
        remark: this.$t('流程创建人')
      }, {
        processField: 'updateUser',
        serviceField: 'updateUser',
        writebackMode: '1',
        required: '1',
        remark: this.$t('流程最后更新人')
      }, {
        processField: 'updateTime',
        serviceField: 'updateTime',
        writebackMode: '1',
        required: '1',
        remark: this.$t('流程最后更新时间')
      }, {
        processField: 'processCompleteUsers',
        serviceField: 'processCompleteUsers',
        writebackMode: '1',
        required: '1',
        remark: this.$t('流程历史已办人员')
      }, {
        processField: 'processStatus',
        serviceField: 'processStatus',
        writebackMode: '1',
        required: '1',
        remark: this.$t('流程状态，业务上使用')
      }, {
        processField: 'processSubStatus',
        serviceField: 'processSubStatus',
        writebackMode: '1',
        required: '1',
        remark: this.$t('流程状态，业务上使用')
      }, {
        processField: 'processCurrentUsers',
        serviceField: 'processCurrentUsers',
        writebackMode: '1',
        required: '1',
        remark: this.$t('流程当前待办事项的办理人')
      }, {
        processField: 'processDuration',
        serviceField: 'processDuration',
        writebackMode: '1',
        required: '1',
        ...
        processField: 'processCcUsers',
        serviceField: 'processCcUsers',
        writebackMode: '1',
        required: '1',
        remark: ''
      }, {
        processField: 'processSubscribeUsers',
        serviceField: 'processSubscribeUsers',
        writebackMode: '1',
        required: '1',
        remark: this.$t('关注人')
      }],
  ...
  methods: {
    show (config) {
      ...
      this.axios({
        ...
        this.workflowFields = res.result.workflowFields
        ...
        this.tableFields = res.result.tableFields
        this.userData = res.result.userData
        this.setting = res.result.workflowSetting
        ...
        if (res.result.workflowSetting.writeBackMappings) {
          const writeBackMappings = JSON.parse(JSON.stringify(this.writeBackMappings))
          this.writeBackMappings = res.result.workflowSetting.writeBackMappings
        ...
        this.bindingFormViews = res.result.workflowSetting.bindingFormViews || []
        this.appBindingFormViews = res.result.workflowSetting.appBindingFormViews || []
        this.formViewMappings = res.result.formViewMappings
        this.appFormViewMappings = res.result.appFormViewMappings
      ...
      this.axios({
        url: 'admin/workflow/getCallbackClassOptions'
      }).then(res => {
        this.callbackClassOptions = res.result || []
      })
      this.axios({
        url: '/admin/dict/initData',
        data: {
          dictCategoryNumber: 'process-status-' + config.record.workflowId
        }
      }).then(res => {
      ...
      this.axios({
        url: '/admin/dict/initData',
        data: {
          dictCategoryNumber: 'process-sub-status-' + config.record.workflowId
        }
      }).then(res => {
    ...
    handleSubmit () {
      ...
      validateFields((errors, values) => {
        if (!errors) {
          ...
          values.info.workflowSetting.bindingFormViews = this.$refs.formapply ? this.$refs.formapply.bindingFormViews : this.setting.bindingFormViews
          values.info.workflowSetting.appBindingFormViews = this.$refs.appformapply ? this.$refs.appformapply.bindingFormViews : this.setting.appBindingFormViews
          values.info.workflowSetting.formViewButtons = this.$refs.extendbutton ? this.$refs.extendbutton.formViewButtons : this.formViewButtons
          values.info.workflowSetting.appFormViewButtons = this.$refs.appextendbutton ? this.$refs.appextendbutton.formViewButtons : this.formViewButtons
          values.info.workflowSetting.terminateReasons = this.terminateReasons
          values.info.workflowSetting.writeBackMappings = this.writeBackMappings
          values.info.workflowSetting.cancelPermissions = this.cancelPermissions
          values.info.workflowSetting.urgeReasons = this.urgeReasons
          values.info.workflowSetting.slaData = this.slaData
          values.info.workflowSetting.notice = this.$refs.workflowFormNotice && this.$refs.workflowFormNotice.form ? this.$refs.workflowFormNotice.form.getFieldsValue().info : {}
          values.info.workflowSetting.icon = this.iconName
          ...
          values.info.workflowSetting.limitedTime = this.timeLimitData
          values.info.workflowSetting.dataWindowButtons = dataWindowButtons
          values.info.workflowSetting.workflowOperations = this.$refs.workflowOperation ? this.$refs.workflowOperation.handleSubmit() : this.setting.workflowOperations
          values.info.accessLevel = 0
          // 判断必须回写字段，对应数据表是否存在
          const writeStutas = this.writeBackMappings.map(item => {
            return this.tableFields.some(findItem => findItem.alias === item.serviceField)
          })
          const submit = (bool) => {
            const that = this
            that.loading = true
