            ...
            <a-input
              v-decorator="[
                'blacklistTips',
                {
                  ...
                  rules: [
                    { required: true, pattern: new RegExp(/^(\s*\S+\s*)+$/), message: $t('请输入黑名单提示语') },
                    { min: 10, message: $t('请输入至少10个字符') }
                  ]
                }
              ]"
              :maxLength="200"
          ...
          >
            <span slot="label">
              ...
              <a-tooltip
                :title="
                  $t(
                    '关闭后，视为24小时提供服务。开启后，转人工服务时会判断技能组的“工作时间设置”，非工作时间发送{非人工服务时间提示语}。技能组没有设置工作时间的，视为关闭人工服务。'
                  )
                "
              >
                <a-icon type="question-circle" />
