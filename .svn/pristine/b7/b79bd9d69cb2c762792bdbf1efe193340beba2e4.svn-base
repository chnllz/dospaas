<template>
  <a-drawer height="100%" placement="top" :closable="false" :destroyOnClose="true" :visible="visible" @close="onClose">
                ...
                <div class="all_box">
                  <div class="content_box">
                    ...
                    <div class="header" style="margin: 20px 0px">
                      ...
                      <span style="color: blue; cursor: pointer" @click="createLink(detail.article.id)">
                        {{ $t('生成链接') }}
                      </span>
                    </div>
                    ...
                    <div class="footer">
                      ...
                      >
                        ...
                        <span slot="operateName" slot-scope="text">{{ $t(text) }}</span>
                      </a-table>
                    </div>
                    <div class="comment">
                      <div class="header">
    ...
    <div v-viewer="{ movable: true }" class="images">
      <div v-for="(item, index) of src" :key="index">
        <img :src="item" style="width: 0; height: 0" />
      </div>
    </div>
  </a-drawer>
</template>
...
export default {
  ...
  methods: {
    ...
    createLink (id) {
      this.axios({
        url: '/knowledge/index/api/generatorKnowledgeSign?id=' + id,
        data: {}
      }).then(res => {
        try {
          const input = document.createElement('input')
          document.body.appendChild(input)
          input.value = `${location.origin + process.env.VUE_APP_BASE_URL}LoadPage/?view=knowledge/pageView&timestamp=${res.result.timestamp}&id=${res.result.id}&sign=${res.result.sign}`
          input.select()
          if (document.execCommand('copy', false)) {
            this.$message.success(this.$t('知识库链接已经生成并复制到剪贴板，请在浏览器中打开访问'))
          } else {
