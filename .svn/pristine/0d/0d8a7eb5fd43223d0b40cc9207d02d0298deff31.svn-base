<template>
  <a-spin :spinning="loading" style="height: 100%">
    <div class="page" style="height: 100%">
      ...
      <a-space>
        ...
        <a-dropdown>
          ...
          <a-menu slot="overlay" @click="handMenu">
            ...
            <a-menu-item v-if="userInfo.superAdmin" key="updateSort">
              <a-icon type="sync" />
              {{ $t('更新物理表字段排序顺序') }}
            </a-menu-item>
          </a-menu>
      ...
      <field-sync ref="fieldsync" :key="fieldSyncKey" @ok="handleOk"></field-sync>
...
export default {
  ...
  components: {
    ...
    FieldSync: () => import('@/views/admin/Field/FieldSync')
  },
  props: {
    tableId: {
      type: [String, Number],
    ...
    handMenu (e) {
      ...
      switch (key) {
        case 'plus':
          ...
          break
        case 'sync':
          this.fieldSyncKey = this.fieldSyncKey === 'fieldSyncKey_1' ? 'fieldSyncKey_2' : 'fieldSyncKey_1'
          this.$nextTick(() => {
            this.$refs.fieldsync.show({
              tableId: that.tableId
            })
          })
          break
        case 'updateSort':
          this.$confirm({
            content: this.$t('此操作将会严重影响系统性能，请在非工作时间内操作，您确认要"更新物理表字段排序顺序"吗？'),
            okText: that.$t('确定'),
