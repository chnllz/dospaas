  ...
  >
    <template slot="title">
      <a-row>
        ...
        <a-col v-if="helpText" :span="3" style="text-align: right">
    ...
    <send-message ref="sendMessage" :key="sendMessageKey" />
  </a-drawer>
</template>
<script>
...
export default {
  ...
  components: {
    ...
    UserTableComponents,
    UserTableWorkflowRemarks: () => import('./General/UserTableWorkflowRemarks'),
    userTableWorkflowTerminate: () => import('./General/UserTableWorkflowTerminate'),
  ...
  data () {
    return {
      ...
      templateId: '',
      sendMessageKey: 'sendMessageKey_1'
    }
  },
  computed: {
  ...
  methods: {
    ...
    sendMessage (data) {
      this.sendMessageKey = this.sendMessageKey === 'sendMessageKey_1' ? 'sendMessageKey_2' : 'sendMessageKey_1'
      this.$nextTick(() => {
    ...
    // 仅保存
    handleOnlySave (data) {
      this.$refs.userTableFormView.handleSubmit(data, this.parentPage, this.params.jointable, 'onlySave')
    },
    // 仅保存并关闭
    handleOnlySaveClose (data) {
      this.$refs.userTableFormView.handleSubmit(data, this.parentPage, this.params.jointable, 'onlySaveClose')
    },
    // 刷新日志
    refresh () {
