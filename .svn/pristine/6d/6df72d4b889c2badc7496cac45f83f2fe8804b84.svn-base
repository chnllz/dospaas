  ...
  <a-config-provider :locale="locale">
    <div class="form-designer-container-9136076486841527">
      ...
      >
        ...
        <!-- 中间面板区域 start -->
        <section :style="{ height: tableType ? 'calc(100% - 262px)' : 'calc(100% - 202px)' }">
          <!-- 操作区域 start -->
          <operatingArea
            v-if="!toolbarsTop"
            :showToolbarsText="showToolbarsText"
        ...
        <!-- 右侧控件属性区域 start -->
        <aside class="right" :style="{ height: tableType ? 'calc(100% - 262px)' : 'calc(100% - 202px)' }">
          <formItemProperties
      ...
      >
        <a-spin :spinning="loading">
          <a-form v-if="!loading" :form="helpForm" :labelCol="{ span: 4 }" :wrapperCol="{ span: 20 }">
      ...
      >
        <a-form :form="settingForm" :labelCol="{ span: 4 }" :wrapperCol="{ span: 20 }">
          ...
          <a-form-item v-if="userInfo.superAdmin">
            <span slot="label">
              {{ $t('访问级别') }}
              <a-tooltip placement="top">
                <template slot="title">
            ...
            <a-select v-decorator="['accessLevel', { initialValue: formdata.accessLevel || 0 }]">
    ...
    myTemplate (newValue) {
      if (!this.tableType) {
        ...
        this.fieldColumns.forEach(fieldItem => {
          fieldColumnsObj[fieldItem.alias] = fieldItem
        })
        getTemplate(this.myTemplate, fieldColumnsObj)
      ...
      function getTemplate (array, fieldColumnsObj) {
        array.forEach((item, index) => {
          item.key = item.key ?? (item.alias || item.type) + '_' + new Date().getTime() + item
          if (item.columns) {
            getTemplate(item.columns, fieldColumnsObj)
          } else if (item.trs) {
            getTemplate(item.trs, fieldColumnsObj)
          } else if (item.tds) {
            getTemplate(item.tds, fieldColumnsObj)
          } else if (item.list) {
            getTemplate(item.list, fieldColumnsObj)
          } else {
            if (fieldColumnsObj[item.value]) {
              const fieldItem = fieldColumnsObj[item.value]
              item.name = fieldItem.name
