<template>
  <div>
    <a-drawer :destroyOnClose="true" :width="1200" :visible="visible" @close="onClose">
      <template slot="title">
        <a-row>
          <a-col :span="20">{{ config.title }}</a-col>
          <a-col :span="4" style="text-align: right">
            <HelpPanel
              :number="'22041616464807'"
        ...
        <div>
          <a-tabs default-active-key="1">
                    ...
                    <a-collapse-panel v-for="(item, index) in data" :key="String(item.id)">
                                          ...
                                          if (item1.alias === value) {
                                            ...
                                            if (item1.formType === 'treeselect') {
                                              ...
                                              (item1.formType === 'combobox' ||
                                                item1.formType === 'radio' ||
                                                item1.formType === 'checkbox') &&
                                            ...
                                            if (['text', 'textarea', 'serialnumber'].indexOf(item1.formType) !== -1) {
                                              ...
                                              ['radio', 'switch', 'combobox', 'score'].indexOf(item1.formType) !== -1
                                            ) {
                                              simpleItem.conditionArr = [
                                                { enName: 'equal', cnName: $t('等于') },
                                                { enName: 'ne', cnName: $t('不等于') },
                      ...
                      <a-form-item v-for="(eventItem, key) in item.event" :key="key" :wrapper-col="{ span: 24 }">
                        <a-row type="flex" align="middle" :gutter="10">
                            ...
                            <a-row :gutter="10">
                              ...
                              <a-col :span="12">
                                ...
                                >
                                  ...
                                  <template v-if="eventItem.isHaveOption">
                                    <a-select-option value="optionalRange">
                                      {{ $t('设置字段可选范围') }}
                                    </a-select-option>
                                    <a-select-option value="notOptionalRange">
                                      {{ $t('设置字段不可选范围') }}
                                    </a-select-option>
                                  </template>
                                </a-select>
                          ...
                          <a-col
                            v-if="eventItem.status === 'optionalRange' || eventItem.status === 'notOptionalRange'"
                            :span="4"
                          >
                            <a-select
                              v-if="eventItem.dictType === 'plane'"
                              size="small"
                              :defaultValue="eventItem.optionValue"
                              mode="tags"
                              @change="
                                (value) => {
                                  eventItem.optionValue = value
                                  eventItem.choiceOption = eventItem.option.filter((im) =>
                                    value.includes(im.dictDataNumber)
                                  )
                                  eventItem.choiceOption = eventItem.choiceOption.map((im) => {
                                    const { dictDataNumber, dictDataName } = im
                                    return { value: dictDataNumber, label: dictDataName }
                                  })
                                }
                              "
                            >
                              <a-select-option
                                v-for="(optionItem, optionIndex) in eventItem.option"
                                :key="optionIndex"
                                :value="optionItem.dictDataNumber"
                              >
                                {{ optionItem.dictDataName }}
                              </a-select-option>
                            </a-select>
                            <data-picker
                              v-else
                              ...
                              name="dataOption"
                              :parentId="
                                eventItem.fieldSetting.attribute.dataSource === 'addressBook'
                                  ? 'parentNumber'
                                  : 'parentDictDataNumber'
                              "
                              :parameter="
                                eventItem.fieldSetting.attribute.dataSource === 'addressBook'
                                  ? {}
                                  : { dictCategoryNumber: eventItem.fieldSetting.form.src, filterDisabled: true }
                              "
                              displayType="tree"
                              :url="
                                eventItem.fieldSetting.attribute.dataSource === 'addressBook'
                              ...
                              :optionsConversion="
                                (list, displayType, searchType) =>
                                  optionsConversionTreeselect(
                                    { dataSource: eventItem.fieldSetting.attribute.dataSource },
                                    list,
                                    displayType,
                                    searchType
                                  )
                              "
                              :value="eventItem.choiceOption"
                              @select="
                                (value, list) => {
                                  eventItem.optionValue = value
                                  eventItem.choiceOption = list
                                }
                              "
                            />
                          </a-col>
            ...
            <a-tab-pane key="2" :tab="$t('表单加载后脚本')" force-render style="height: calc(100vh - 175px)">
              <editor ref="afterLoadingEditor" :params="afterLoading" />
            </a-tab-pane>
            <a-tab-pane key="3" :tab="$t('表单提交前脚本')" force-render style="height: calc(100vh - 175px)">
              <editor ref="beforeSubmitEditor" :params="beforeSubmit" />
            </a-tab-pane>
          </a-tabs>
  ...
  methods: {
    show (config) {
      ...
      if (config.page === 'viewForm') {
        this.afterLoading.value = config.valueinit
        this.beforeSubmit.value = config.value
        const formViewData = JSON.parse(JSON.stringify(this.theme.viewData))
        this.tableId = formViewData.tableId || formViewData.data.tableId
        if (formViewData.setting.fieldRule) {
          this.data = formViewData.setting.fieldRule.length !== 0 ? formViewData.setting.fieldRule : []
        } else {
          this.data = []
        }
      } else if (config.page === 'column') {
        this.afterLoading.value = config.afterInit
        this.beforeSubmit.value = config.beforeSubmit
        this.tableId = config.tableId
        this.data = config.fieldRule
      }
        ...
        if (this.data.length !== 0) {
          this.data.forEach((dataItem, dataIndex) => {
            ...
            dataItem.event.forEach((eventItem) => {
              const field = this.fieldColumns.find(item => item.alias === eventItem.field) || {}
    ...
    choiceField (index, key, template, val) {
      if (template === 'so') {
        ...
        const field = this.fieldColumns.find(item => item.alias === val) || {}
        arr.event[key].formType = field.formType
        arr.event[key].status = ''
        arr.event[key].mandatory = ''
        arr.event[key].optionValue = []
        if (['radio', 'checkbox', 'combobox', 'cascader', 'treeselect'].includes(field.formType)) {
    ...
    handleSubmit () {
      ...
      if (this.config.page === 'viewForm') {
        this.$emit('func', afterInit, 'init')
        this.$emit('func', beforeSubmit, 'verif')
      }
      ...
      data.forEach((item, index) => {
        ...
        item.event.forEach(eventItem => {
          if (eventItem.option) {
            delete eventItem.option
          }
          if (eventItem.fieldSetting) {
            delete eventItem.fieldSetting
          }
        })
      })
      ...
      } else if (this.config.page === 'column') {
        this.$emit('ok', {
          fieldRule: data,
          afterInit,
          beforeSubmit
