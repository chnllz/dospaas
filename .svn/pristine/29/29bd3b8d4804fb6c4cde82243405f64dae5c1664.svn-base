  ...
  <div>
    <a-tabs @change="tabChange">
              ...
              <a-row>
                <a-col :span="7">
                  <a-select v-model="triggerTimeType" @change="getTriggerTime">
                    ...
                    <a-select-option value="computed">{{ $t('通过业务数据计算') }}</a-select-option>
                  </a-select>
                </a-col>
          ...
          <template v-else-if="currentSelect.type === 'user_transition'">
            ...
            >
              ...
              <span slot="alias" slot-scope="text, record">
                <a-input
                  ...
                  :disabled="record.aliasEditabled"
                  size="small"
                />
              ...
              <span slot="checked" slot-scope="text, record">
                <a-radio-group
                  ...
                  :disabled="!record.editabled"
                  @change="
                    (e) => {
                      if (e.target.value === '1') {
                        handleMethods.forEach((item) => {
                          item.checked = '0'
                        })
                      }
                      record.checked = e.target.value
                    }
                  "
                >
                ...
                <a
                  ...
                  @click="
                    () => {
                      ...
                      $set(record, 'aliasEditabled', true)
                    }
                  "
                >
                  {{ $t('编辑') }}
            ...
            >
              <a-row>
                <a-col :span="4">
                  <a-form-item>
                    <a-input-number
                      v-model="remarksMinRows"
                      style="width: 100%"
                      :min="1"
                      @change="
                        (num) => {
                          remarksMinRows = num
                          currentSelect.remarksMinRows = num
                        }
                      "
                    />
                  </a-form-item>
                ...
                <a-col :span="4">
                  <a-form-item>
                    <a-input-number
                      v-model="remarksMaxRows"
                      style="width: 100%"
                      :min="parseInt(remarksMinRows) || 1"
                      @change="
                        (num) => {
                          currentSelect.remarksMaxRows = num
                        }
                      "
                    />
                  </a-form-item>
            ...
            >
              ...
              <span slot="visible" slot-scope="text">
                <a-badge v-if="text" status="success" :text="$t('是')" />
      ...
      <a-tab-pane v-if="currentSelect.type === 'user_transition'" key="participation" :tab="$t('办理人')">
        <a-form :form="form">
          <a-form-item label="办理人备注" :labelCol="{ span: 2 }" :wrapperCol="{ span: 22 }">
            <a-textarea v-model="transitionUserRemarks" :autoSize="{ minRows: 1, maxRows: 6 }" />
          </a-form-item>
              ...
              @change="
                (e) => {
                  ...
                  if (e.target.value === 'user') {
                    ...
                    program = 'fixedDepartment'
                  } else if (e.target.value === 'role') {
                    program = 'fixedRole'
                  }
                  changeAssociatedList(handleData.type, 'change', program, 'noRemove')
                  $forceUpdate()
                }
              "
            >
          ...
          <a-form-item label="查找方案" :labelCol="{ span: 2 }" :wrapperCol="{ span: 22 }">
            ...
            <div v-if="handleData.program === 'customHandle'">
              <a-row style="margin-top: 4px">
                ...
                <div
                  style="position: absolute; top: -6px; right: 15px; cursor: pointer; z-index: 10"
                  size="small"
                  @click="
                    () => {
      ...
      <a-tab-pane v-if="currentSelect.type === 'user_transition'" key="tplview" :tab="$t('表单应用')">
        <a-form>
          <a-form-item :label="$t('pc端表单视图')" :labelCol="{ span: 2 }" :wrapperCol="{ span: 22 }">
            <a-select v-model="currentSelect.templateId" :allowClear="true" show-search option-filter-prop="children">
              <a-select-option
                v-for="(v, i) in tplviewData.filter((item) => item.type === 'webFormView')"
                :key="i"
                :value="v.templateId"
              >
                {{ v.name }}
          ...
          <a-form-item :label="$t('移动端表单视图')" :labelCol="{ span: 2 }" :wrapperCol="{ span: 22 }">
            <a-select
              v-model="currentSelect.appTemplateId"
              :allowClear="true"
              show-search
              option-filter-prop="children"
            >
              <a-select-option
                v-for="(v, i) in tplviewData.filter((item) => item.type === 'appFormView')"
                :key="i"
                :value="v.templateId"
              >
                {{ v.name }}
                      ...
                      <a-row type="flex" align="middle" :gutter="10">
                        <a-col :span="20">
                          ...
                          >
                            ...
                            <a-col v-if="simpleItem.type === 'field' && includeItem.include !== 'em'" :span="15">
                              <template v-if="!includeItem.type">
                                <!-- 日期 -->
                                <a-date-picker
      ...
      <a-tab-pane key="WorkflowOperation" tab="变迁操作">
        <WorkflowOperation
          ref="workflowOperation"
          :workflowList="processStatusData"
          :workflowOperations="currentSelect.transitionOperations"
          :tableFields="thisTableField"
          :callbackName="workflowSetting.callbackName"
          type="transition"
        />
      </a-tab-pane>
    </a-tabs>
    ...
    <flow-attr-transition-form-rule
      ...
      :key="refreshKey"
      :params="{ handleMethods: handleMethods }"
      @ok="getFormRule"
    />
    <!-- 办理按钮 -->
    ...
    <!-- 办理方式 -->
    <flow-attr-transition-way-form
      ref="flowAttrTransitionWayForm"
      :params="{ handleMethods: handleMethods }"
      @ok="getWay"
    />
    <!-- 办理人 -->
...
export default {
  ...
  components: {
    ...
    WorkflowOperation: () => import('@/views/admin/WorkflowOperation'),
  ...
  data () {
    return {
      ...
      fieldArrHandle: [],
      associatedList: [
        { label: '流程创建人', value: 'creator' },
        { label: '流程创建人的部门主管', value: 'creatorDepartment' }
      ],
      ...
      handler (val) {
        ...
        if (this.flowData.params && this.flowData.params.flow) {
          ...
          nodeList.forEach(item => {
            if (item.handleMethods) {
              this.allWayData[item.id] = item.handleMethods
            }
          })
        }
        this.remarksRule = val.currentSelect.remarksRule || 'show_no_allow'
        this.currentSelect.remarksMinRows = this.remarksMinRows = val.currentSelect.remarksMinRows || 1
        this.currentSelect.remarksMaxRows = this.remarksMaxRows = val.currentSelect.remarksMaxRows || 4
        ...
        if (!val.currentSelect.formViewButtons) {
          this.formViewButtons = menuAllData
        } else {
          this.formViewButtons = val.currentSelect.formViewButtons
        }
        // 兼容后续添加的按钮 start
        const viewBtns = this.formViewButtons.map(item => item.usage)
        const barBtn = ['bar_save', 'bar_save_close'].filter(item => !viewBtns.includes(item))
        if (barBtn.length > 0) {
          const needAddBtn = menuAllData.filter(item => barBtn.includes(item.usage))
          needAddBtn.forEach(item => {
            this.formViewButtons.push(JSON.parse(JSON.stringify(item)))
          })
        }
        // 兼容后续添加的按钮 end
        this.handleMethods = val.currentSelect.handleMethods ? val.currentSelect.handleMethods : []
        this.handleMethods.forEach(item => {
          if (!item.alias) {
            this.getName(item, item.way)
          }
        })
        this.handleMethods.sort(this.compare('sortId'))
        this.fieldPrivData = val.currentSelect.fieldPrivData ? val.currentSelect.fieldPrivData : []
        this.transitionUserRemarks = val.currentSelect.transitionUserRemarks
        this.mydata = {
        ...
        if (val.currentSelect.handleFormRules) {
          this.handleFormRules = val.currentSelect.handleFormRules
        }
        if (val.currentSelect.limitedTime && val.currentSelect.limitedTime instanceof Array) {
          this.timeLimitData = val.currentSelect.limitedTime
  ...
  methods: {
    ...
    getFromTableField () {
      this.axios({
        ...
        this.fieldArrHandle = res.result.data.filter(item => {
          return ['text', 'textarea', 'radio', 'switch', 'checkbox', 'combobox', 'number', 'datetime', 'cascader', 'associated', 'address', 'treeselect', 'tag', 'organization'].indexOf(item.formType) !== -1
        })
      })
    },
    getTableFieldsHandleList () {
    ...
    changeAssociatedList (type, methods, program, action) {
      if (methods === 'change' && action !== 'noRemove') {
        this.handleData.associateValue = undefined
        this.handleData.customCode = undefined
        this.handleData.specifyAssociated = undefined
      }
      const handleDataProgram = program || this.handleData.program
      if (type === 'user') {
        switch (handleDataProgram) {
          case 'fixedUser':
        ...
        switch (handleDataProgram) {
          ...
          case 'flowAttrUser':
            this.associatedList = [
              { label: '流程创建人所在部门', value: 'creatorDepartment' },
              { label: '流程创建人所在的上级部门', value: 'creatorUpDepartment' }
            ]
            break
          case 'taskUp':
            this.associatedList = [
        ...
        switch (handleDataProgram) {
          ...
          case 'flowAttrUser':
            this.associatedList = [
              { label: '流程创建人所属角色', value: 'creatorRole' }
            ]
            break
          case 'taskUp':
            this.associatedList = [
    ...
    tabChange (e) {
      if (e === 'timeLimit') {
        this.axios({
          ...
          if (this.currentSelect.limitedTime && !this.currentSelect.limitedTime.workflowRestrict) {
            // 遍历规则
            this.currentSelect.limitedTime.forEach((dataItem, dataIndex) => {
            ...
            this.timeLimitData = this.currentSelect.limitedTime
          }
        })
        // 请求所有的部门，角色，用户列表
        this.axios({
        ...
        this.axios({
          ...
          data: { tableId: this.flowData.params.tableId, type: ['webFormView', 'appFormView'] }
        }).then((res) => {
          this.tplviewData = res.result.templates
        })
      }
    },
    ...
    handleChangeName (e, record) {
      ...
      if (!record.aliasEditabled) {
        record.alias = val.join('')
      }
    },
    addTimeLimit (index) {
    ...
    handleDefaultMenu () {
      this.$refs.toolButtonChoose.show({
        ...
        unCheckBar: menuAllData.filter(item => this.formViewButtons.findIndex(barItem => barItem.usage === item.usage) === -1)
      })
    },
    getButtonData (data) {
      this.formViewButtons = [...this.formViewButtons, ...data]
      this.handleOk()
    },
    ...
    getUser (data, index, index2, mode) {
      ...
      this.$set(this.formViewButtons[index], 'bookScope', obj)
      this.handleOk()
    },
    handleDeleteMenu (record) {
      const dataSource = [...this.formViewButtons]
      this.formViewButtons = dataSource.filter(item => item.id !== record.id)
      this.handleOk()
    },
    handleAddWay () {
      this.handleMethods.push({
        id: (new Date()).valueOf(),
        listOrder: this.handleMethods.length + 1,
        way: '',
        checked: '0',
        rule: '',
        alias: '',
    ...
    handleSort () {
      this.changeData = 'handleMethods'
      this.$refs.dragSort.show({
        title: this.$t('排序'),
        placement: 'right',
        width: 600,
        sortName: 'way',
        sortData: this.handleMethods
      })
    },
    // 排序
    handleTimeSort () {
    ...
    handleMenuSort () {
      this.changeData = 'formViewButtons'
      this.$refs.dragSort.show({
        title: this.$t('排序'),
        placement: 'right',
        width: 600,
        sortData: this.formViewButtons
      })
    },
    // 选择关系
    choiceInclude (value, includeItem) {
    ...
    wayDataSubmit (record) {
      ...
      } else if (!reg.test(record.alias)) {
        this.$message.warning('请输入办理方式系统名为字母或数字')
      } else {
    ...
    handleEditRule (record, index) {
      ...
      this.$nextTick(() => {
        this.$refs.flowAttrTransitionFormRule.show({
          ...
          handleFormRules: this.handleFormRules || []
        }, this.handleMethods)
      })
    },
    handleDeleteWay (index) {
      this.handleMethods.sort(this.compare('listOrder'))
      this.handleMethods.splice(index, 1)
      this.handleOk()
    },
    handleOk () {
      this.currentSelect.formViewButtons = this.formViewButtons
      this.currentSelect.handleMethods = this.handleMethods
      this.currentSelect.fieldPrivData = this.fieldPrivData
    },
    getFormRule (data) {
      this.handleFormRules = data
      this.currentSelect.handleFormRules = data
    },
    getMenu (action, values, index) {
      if (action !== 'edit') {
        this.formViewButtons.push(values)
      } else {
        this.formViewButtons.splice(index, 1, values)
      }
      this.handleOk()
    },
    compare (listOrder) {
    ...
    getWay (action, values, index) {
      ...
      this.handleMethods.sort(this.compare('sortId'))
      if (action !== 'edit') {
        this.handleMethods.push(values)
      } else {
        this.handleMethods.splice(index, 1, values)
      }
      this.handleOk()
    },
    ...
    handleSubmit () {
      ...
      validateFields((errors, values) => {
        if (!errors) {
          const arr = this.handleMethods.map(item => item.editabled)
          ...
          this.currentSelect.transitionUserRemarks = this.transitionUserRemarks || this.currentSelect.transitionUserRemarks
          this.currentSelect.transitionOperations = this.$refs.workflowOperation ? this.$refs.workflowOperation.handleSubmit() : this.currentSelect.transitionOperations
          this.currentSelect.slaData = this.slaData
          this.currentSelect.handleFormRules = this.handleFormRules
          ...
          this.currentSelect.limitedTime = this.timeLimitData
          // 新的提交数据方式
          this.params.currentSelect = { ...this.currentSelect }
          this.axios({
            url: '/admin/workflow/saveNode',
            data: { nodeSetting: this.currentSelect, nodeId: this.currentSelect.id, workflowId: this.workflowId, nodeType: this.currentSelect.type }
          }).then(res => {
            if (!res.code) {
