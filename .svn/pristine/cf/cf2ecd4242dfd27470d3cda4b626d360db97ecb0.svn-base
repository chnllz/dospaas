<template>
  ...
  >
    <HelpPanel
      v-if="['place', 'arc', 'transition'].indexOf(activeKey) !== -1"
      :number="number"
        ...
        <a-form :form="form" style="margin-top: 16px">
          <a-row>
            ...
            <a-col :span="12">
              ...
              <a-form-item :label="$t('连线系统名称')" :labelCol="{ span: 4 }" :wrapperCol="{ span: 20 }">
                <a-input
                  v-decorator="[
                    'arcNumber',
                    {
                      initialValue: currentSelect.arcNumber || null
                    }
                  ]"
                />
              </a-form-item>
          ...
          <a-tabs default-active-key="condition">
            <a-tab-pane key="condition" :tab="$t('条件设置')">
              ...
              <a-row>
                ...
                <a-col v-if="!currentSelect.matching" :span="8">
                  <a-form-item :labelCol="{ span: 6 }" :wrapperCol="{ span: 18 }">
                    <span slot="label">
                      {{ $t('优先级') }}
                      <a-tooltip :title="$t('有效值0-9999，数字小的将优先参与判断')">
                        <a-icon type="question-circle" />
                      </a-tooltip>
                    </span>
                    <a-input-number v-model="currentSelect.priority" :min="0" :max="9999" />
                  </a-form-item>
                </a-col>
              </a-row>
    ...
    <template v-else-if="activeKey === 'gateway'">
      ...
      >
        <a-form :form="gatewayForm">
          ...
          <a-form-item :label="$t('路由选择事件')" :labelCol="labelCol" :wrapperCol="wrapperCol">
              ...
              <a-form-item>
                <a-switch
                  v-decorator="[
                    'info[setRouter]',
                    {
                      initialValue: currentSelect.router.setRouter || false,
                      valuePropName: 'checked'
                    }
                  ]"
                  @change="
                    (value) => {
                      $set(currentSelect.router, 'setRouter', value)
                    }
                  "
                />
              </a-form-item>
            </div>
            <div v-if="currentSelect.router.setRouter" class="line-col">
              <div class="line marginTop12">
                <a-select
                  v-model="currentSelect.router.routerId"
                  style="width: 320px"
                  placeholder="请选择事件"
                  :allowClear="true"
                  show-search
                ...
                >
                  <a-select-option v-for="(myitem, myindex) in routerEvents" :key="myindex" :value="myitem.value">
                    {{ $t(myitem.text) }}
                  </a-select-option>
                </a-select>
              </div>
              <a-textarea
                v-model="currentSelect.router.routerRemarks"
                style="width: 320px"
                placeholder="自定义事件备注"
                :autoSize="{
                  minRows: 2,
...
export default {
  ...
  data () {
    return {
      ...
      thisTableField: [],
      workflowSetting: {},
      routerEvents: [] // 路由选择事件列表
    }
  },
  computed: {
    number () {
      if (this.activeKey === 'place') {
        return '22041616485312'
      } else if (this.activeKey === 'transition') {
        return '22041616484211'
      } else if (this.activeKey === 'arc') {
        return '22041616483210'
      } else {
        return ''
      }
    }
    ...
    currentSelect: {
      handler (val) {
        if (['start', 'end', 'event'].includes(this.currentSelect.type)) {
          ...
          if (!this.currentSelect.router) {
            this.$set(this.currentSelect, 'router', {})
          }
        } else {
          // 流程属性
          this.activeKey = 'flow'
  ...
  methods: {
    show (config) {
      this.visible = true
      this.currentSelect = config.val || {}
      this.workflowSetting = config.workflowSetting || {}
    ...
    getRouterEvents () {
      if (!this.workflowSetting.callbackName) return false
      this.axios({
        url: 'admin/workflow/getCallbackOptions',
        params: {
          name: this.workflowSetting.callbackName,
          type: 'router'
        }
      }).then(res => {
        this.routerEvents = res.result || []
      })
    },
    linkLabelChange (e) {
      const that = this
    ...
    handleSubmit () {
      if (this.activeKey === 'gateway') {
        ...
        this.form.validateFields((errors, values) => {
          if (!errors) {
            if (this.$refs.flowAttrVisualized && this.$refs.flowAttrVisualized.data) {
              const data = this.$refs.flowAttrVisualized && this.$refs.flowAttrVisualized.data
              data.simpleCondition.forEach(item => {
                ...
                if (item.condition && item.condition.length > 0) {
                  item.condition.forEach(item2 => {
                    if (item2.type === 'empty') {
                      item2.include = item2.include === 'em' ? 'em' : 'ne'
                    }
            ...
            this.$refs.arcEvents.form.validateFields((errors1, values1) => {
              if (!errors1) {
                ...
                this.currentSelect.arcNumber = values.arcNumber || null
                const ifBoolean = []
                ...
                const dictDataName = this.processSubStatusData.find(item => {
                  return item.dictDataNumber === this.currentSelect.processSubStatus
                }).dictDataName
                this.$emit('ok', Object.assign({ dictDataName }, this.currentSelect))
                this.visible = false
              } else {
...
.dospass-modal /deep/ .ant-modal-close-x {
  width: 30px;
  margin-right: 12px;
}
</style>
