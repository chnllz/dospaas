  ...
  data () {
    return {
      ...
      setting: {
        ...
        // 字体列表
        font_formats: `${this.$t('微软雅黑')}=Microsoft YaHei,Helvetica Neue,PingFang SC,sans-serif;${this.$t('苹果苹方')}= PingFang SC, Microsoft YaHei, sans- serif; ${this.$t('宋体')}=simsun, serif; ${this.$t('仿宋体')}=FangSong, serif; ${this.$t('黑体')}=SimHei, sans- serif; Arial = arial, helvetica, sans - serif;Arial Black = arial black, avant garde;Book Antiqua = book antiqua, palatino; `,
        // 快捷操作列表
          ...
          this.$nextTick(() => {
            this.axios({
              ...
              if (!res.code) {
                ...
                const img = location.origin + this.$store.state.env.VUE_APP_API_BASE_URL + 'admin/api/download/?filePath=' + path
                success(img)
              } else {
        ...
        file_picker_callback: (callback, value, meta) => {
          ...
          input.onchange = function () {
            ...
            that.axios({
              ...
              if (!res.code) {
                const path = encodeURIComponent(res.result.filePath)
                const filePath = location.origin + that.$store.state.env.VUE_APP_API_BASE_URL + 'admin/api/download/?filePath=' + path
