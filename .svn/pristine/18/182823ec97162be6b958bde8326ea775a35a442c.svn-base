      ...
      <a-form-item>
        <a-form-item>
          <a-button
            ...
            @click="handleDataPlus(data.searchConditions, data.searchConditions.length - 1)"
          >
            {{ $t('添加') }}
          </a-button>
          {{ $t('以下条件组合方式') }}：
                ...
                <template v-if="!conItem.type">
                  ...
                  <template v-else-if="item.formType === 'organization'">
                    <data-picker
                      ...
                      "
                      :name="`dataPicker${conItem.random}`"
                      :parameter="
                        item.settings && item.settings.form.optionType === 'user' ? { sortField: 'username' } : {}
                      "
                      parentId="parentDepartmentId"
                      :displayType="item.settings && item.settings.form.optionType === 'department' ? 'tree' : 'list'"
                      :url="
                        item.settings && item.settings.form.optionType === 'department'
                  ...
                  <a-select
                    v-else-if="
                      ...
                      item.settings &&
                      (item.settings.form.loadMode === 'dynamic' || item.settings.attribute.dataSource === 'otherTable')
                    "
                    v-model="conItem.value"
                    show-search
                    option-filter-prop="children"
...
export default {
  ...
  methods: {
    show (config) {
      ...
      this.groupSearchItem = config.groupSearchItem
    ...
    handleSave () {
      const data = JSON.parse(JSON.stringify(this.data))
      data.searchConditions.forEach(items => {
        items.searchCondition.forEach(item => {
          delete item.random
        })
      })
      ...
      // this.$emit('ok', data)
      this.groupSearchItem.searchConditionSet = data
      this.visible = false
    }
  }
}
