          ...
          <a-col :span="config.mode === 'multiple' ? 14 : 24">
            <a-row type="flex">
              <a-col :span="12">
                ...
                >
                  <a-input v-model="queryParam.username" placeholder="请输入账号搜索" />
                </a-form-item>
              </a-col>
              <a-col :span="12">
            ...
            <div v-if="config.mode === 'multiple'" style="display: flex">
              <div>
                <div style="margin: 10px 0; display: flex; flex-direction: column">
                  <a-button style="margin: 60px 20px 20px 20px" @click="handleSelect">添加</a-button>
                  <a-button style="margin: 0 20px" @click="deleteAll">清空</a-button>
                </div>
              </div>
              <div style="flex: 1; margin-top: 10px">
                <a-card size="small" title="已选中的用户" :bordered="false">
  ...
  methods: {
    ...
    loadDataTable (parameter) {
      return this.axios({
        ...
        data: Object.assign(parameter, this.queryParam, { init: !this.department.length, fieldId: this.fieldId })
      }).then(res => {
