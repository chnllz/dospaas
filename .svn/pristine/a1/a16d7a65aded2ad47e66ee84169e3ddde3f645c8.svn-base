  ...
  <div class="page">
    <a-form :labelCol="{ span: 6 }" :wrapperCol="{ span: 18 }" :colon="false" class="search">
      <a-card size="small" :title="$t('搜索')">
        <a-space slot="extra">
          <HelpPanel :number="'22041616491715'" />
    ...
    >
      <span slot="name" slot-scope="text, record">
        {{ text + (record.childCount > 0 ? '(' + record.childCount + ')' : '') }}
      </span>
      ...
      <div slot="action" slot-scope="text, record">
        ...
        <a-divider type="vertical" />
        <a :disabled="record.dictType !== 'basic'" @click="handleView(record)">{{ $t('查看') }}</a>
      </div>
    </s-table>
    ...
    <dict-view ref="dictview"></dict-view>
...
export default {
  ...
  data () {
    return {
      ...
      dictType: {
        basic: this.$t('基础'),
        general_table: this.$t('普通数据表'),
        grouping_table: this.$t('分组数据表'),
        sql: 'SQL',
        callback: this.$t('自定义')
      }
    }
  ...
  methods: {
    ...
    handleView (record) {
      this.$refs.dictview.show({
        title: '查看',
        dictCategoryNumber: record.dictCategoryNumber
      })
    },
    changeAdvanced (tag) {
      if (tag) {
