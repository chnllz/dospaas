...
export default {
  ...
  created () {
    this.lang = JSON.parse(JSON.stringify(this.lang))
    ...
    handleLangChange () {
      if (this.showLanguage) {
        ...
        this.axios({
          url: '/admin/data/get',
          data: {
            tableName: 'admin_system_library',
            where: {
              fieldName: 'langZhCn',
              fieldValue: this.$parent.value
            }
          }
        }).then((res) => {
          if (!res.code) {
            ...
            this.languageInit = res.result.data
          } else {
        ...
        if (submit) {
          ...
          if (this.language.id) {
            ...
            handleData.langZhCn = this.$parent.value
