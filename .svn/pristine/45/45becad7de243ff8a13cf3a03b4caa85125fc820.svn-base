...
export default {
  ...
  methods: {
    ...
    requestData () {
      ...
      const _this = this
      const templateId = this.query?.templateId ? this.query.templateId : this.$route.query.templateId
          ...
          array.forEach((item, index) => {
            if (item.columns) {
              ...
              if (item.type === 'component') {
                ...
                // eslint-disable-next-line no-new-func
                const func = new Function(funcStr)
                const obj = func()(_this, this.params.record)
                if (obj.template) {
                  obj.template = obj.template.replace(/[\r\n]/g, '')
                }
                item.component = obj
        ...
        array.forEach((item, index) => {
          if (item.columns) {
            ...
            if (item.field && item.field.alias === val.alias) {
              ...
              const name = `record[${item.field.alias}]`
