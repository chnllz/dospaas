...
<template>
  <div class="form-panel">
    <p v-show="data.list.length === 0" class="hint-text">{{ $t('从左侧选择控件添加') }}</p>
    <a-form
    ...
    >
      <div class="app-design-box">
        <div v-if="entranceType === 'appDataDesign'" class="app-header">
          <div class="app-header-item app-header-left">
            <div v-if="headerOptions.returnBtn">
              <a-icon type="left" />
              {{ $t('返回') }}
            </div>
          </div>
          <div v-if="headerOptions.showTitle" class="app-header-item app-header-title">
            {{ $t(headerOptions.title) }}
          </div>
          <div class="app-header-item app-header-set">
            <a-icon type="setting" theme="filled" @click="headerModalVisible = true" />
          </div>
        </div>
        <div v-if="entranceType === 'appDataDesign'" class="app-search">
          <a-row>
            <a-col span="20"><a-input :placeholder="$t('请输入关键词')" /></a-col>
            <a-col span="4" style="text-align: center; line-height: 28px">{{ $t('搜索') }}</a-col>
          </a-row>
          <div style="position: absolute; right: 10px; top: 6px" class="app-search-set">
            <a-icon
              type="setting"
              theme="filled"
              @click="
                () => {
                  searchPanelKey = searchPanelKey === 'searchPanelKey_1' ? 'searchPanelKey_2' : 'searchPanelKey_1'
                  $nextTick(() => {
                    $refs.searchPanel.show()
                  })
                }
              "
            />
    ...
    <div v-show="showRightMenu" :style="{ top: menuTop + 'px', left: menuLeft + 'px' }" class="right-menu">
      <ul>
        ...
        <li @click="handleAddCol">
          ...
          {{ $t('增加') }}
          <span @click.stop="getTable"><a-input-number v-model="tableCol" :min="1" /></span>
          {{ $t('列') }}
        </li>
        <li @click="handleAddRow">
          <a-icon type="border-verticle" />
          {{ $t('增加') }}
          <span @click.stop="getTable"><a-input-number v-model="tableRow" :min="1" /></span>
          {{ $t('行') }}
        </li>
        <li @click="copyRow">
          <a-icon type="copy" />
        ...
        <li v-if="rowData.tds" @click="pasteRowUp">
          <a-icon type="arrow-up" />
          {{ $t('粘贴至上方') }}
        </li>
        <li v-if="rowData.tds" @click="pasteRowDown">
          <a-icon type="arrow-down" />
          {{ $t('粘贴至下方') }}
        </li>
        <!-- <li @click="handleDelCol"><a-icon type="up-square" />{{ $t('删除一列') }}</li> -->
        <!-- <li @click="handleDelRow"><a-icon type="left-square" />{{ $t('删除一行') }}</li> -->
      </ul>
    </div>
    <a-modal
      v-if="entranceType === 'appDataDesign'"
      :title="$t('标题页面设计')"
      :visible="headerModalVisible"
      :destroyOnClose="true"
      @cancel="
        () => {
          headerOptions = JSON.parse(JSON.stringify(headerOptionsProps))
          headerModalVisible = false
        }
      "
    >
      <a-row>
        <a-col :span="5">{{ $t('页面标题') }}</a-col>
        <a-col :span="12">
          <a-switch
            v-model="headerOptions.showTitle"
            @change="
              () => {
                headerOptions.title = ''
              }
      ...
      <a-row v-if="headerOptions.showTitle" style="margin-top: 10px">
        <a-input v-model="headerOptions.title"></a-input>
      </a-row>
      <a-row style="margin-top: 10px">
        <a-col :span="5">{{ $t('返回按钮') }}</a-col>
        <a-col :span="12">
          <a-switch v-model="headerOptions.returnBtn"></a-switch>
        </a-col>
      </a-row>
      <template slot="footer">
        <a-button
          ...
          @click="
            () => {
              $emit('setHeaderOptions', headerOptions)
              headerModalVisible = false
            }
          "
        >
    ...
    <setSearchPanel
      ...
      ref="searchPanel"
      :key="searchPanelKey"
      :conditionSetProps="conditionSetProps"
      :fieldColumns="fieldColumns"
      :fieldCategory="fieldCategory"
      @setCondition="
        (conditionSet) => {
          $emit('setCondition', conditionSet)
        }
      "
    />
  </div>
...
export default {
  ...
  components: {
    ...
    setSearchPanel: () => import('./setSearchPanel')
  },
  props: {
    noModel: {
      type: Array,
  ...
  data () {
    return {
      ...
      headerOptions: {},
      searchPanelKey: 'searchPanelKey_1'
    }
  },
  watch: {
    // 监听接口返回数据，更新headerOptions
    headerOptionsProps: {
      handler (val) {
        if (val && Object.keys(val).length > 0) {
          this.headerOptions = JSON.parse(JSON.stringify(val))
        }
      }
    }
  },
  mounted () {
    // 初始化headerOptions
    this.headerOptions = JSON.parse(JSON.stringify(this.headerOptionsProps))
    // 添加监听取消右键菜单
  ...
  methods: {
    ...
    handlePaste (e, tabKey = 0) {
      ...
      if (localstorageTableId !== this.params.tableId) {
        this.$message.warning(this.$t('不支持跨表复制粘贴，请检查'))
        return
      }
      if (!localstorageData || localstorageData.length === 0) {
        this.$message.warning(this.$t('暂无复制数据，请先复制数据再粘贴'))
        return
      }
    ...
    handleDownMerge () {
      ...
      if (this.rightMenuSelectValue.trs.length - this.rightMenuSelectValue.trs[this.trIndex].tds[this.tdIndex].rowspan <= this.trIndex) {
        this.$message.error(this.$t('当前是最后一行，无法向下合并'))
        return false
      }
      // 获取当前单元格的rowspan
      ...
      if (Number(this.rightMenuSelectValue.trs[this.trIndex].tds[this.tdIndex].colspan) !== Number(this.rightMenuSelectValue.trs[this.trIndex + currentRowspan].tds[this.tdIndex].colspan)) {
        this.$message.error(this.$t('当前表格无法向下合并'))
        return false
      }
      // 获取下一列单元格的rowspan
    ...
    handleRightMerge () {
      ...
      if (sumCols - this.rightMenuSelectValue.trs[this.trIndex].tds[this.tdIndex].colspan <= this.tdIndex) {
        this.$message.error(this.$t('当前是最后一列，无法向右合并'))
        return false
      }
      ...
      if (Number(this.rightMenuSelectValue.trs[this.trIndex].tds[this.tdIndex].rowspan) !== Number(this.rightMenuSelectValue.trs[this.trIndex].tds[this.tdIndex + currentColspan].rowspan)) {
        this.$message.error(this.$t('当前表格无法向右合并'))
        return false
      }
