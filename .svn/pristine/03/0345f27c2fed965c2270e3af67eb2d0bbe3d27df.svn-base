...
export default {
  ...
  methods: {
    ...
    treeCheck (checkedKeys, e) {
      ...
      const queueArr = e.halfCheckedKeys.filter(item => item !== '0-0')
      if (queueArr.length > 0) {
        queueArr.forEach(item => {
          const obj = this.treeData[0].children.find(itemChild => itemChild.key === item)
          if (this.searchData.queue.length === 0 || this.searchData.queue.every(itemSearch => itemSearch.split('###')[0] !== obj.id)) {
            const newName = obj.id + '###' + obj.name
            this.searchData.queue.push(newName)
          }
        })
      }
