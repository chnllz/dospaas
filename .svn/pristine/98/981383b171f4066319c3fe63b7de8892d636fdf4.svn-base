        ...
        <a-card size="small">
          ...
          >
            <a-textarea
              ...
              :autoSize="{
                minRows: params.remarksMinRows || 4,
                maxRows: params.remarksMaxRows || params.remarksMinRows
              }"
            />
          </a-form-item>
        </a-card>
...
export default {
  ...
  components: {
    buildBlocks: () => import('./ConfigFormBuild'),
    UserTableComponents
  },
  props: {
    params: {
        ...
        const getTemplate = (array) => {
          array.forEach((item, index) => {
            if (item.columns) {
              ...
              if (item.field && item.field.formType === 'web_sub_data_window' && this.num < 2) {
                const templateObj = {}
  ...
  methods: {
    ...
    handleSubmit (e) {
      e.preventDefault()
      const result = this.getValueData()
      if (result) {
        this.$setLoading({ spinning: true, tip: this.$t('保存中……') })
        this.axios({
          url: 'admin/config/saveItemValue',
          data: {
            templateId: this.$route.query.templateId,
            data: result.valueData.record
          },
          tips: false
        }).then(res => {
          ...
          const getTemplate = (array) => {
            ...
            array.forEach((item, index) => {
              if (item.columns) {
                ...
                } else if (item.field && item.field.formType === 'web_sub_data_window') {
                  let value = []
                ...
                } else if (item.field && item.field.formType === 'json') {
                  if (values.record[item.field.alias]) {
                    try {
