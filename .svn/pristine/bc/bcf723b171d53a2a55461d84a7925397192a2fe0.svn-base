    ...
    loadDataTable (parameter) {
      if (this.item.tableId) {
        return this.axios({
          ...
          data: Object.assign(parameter, this.queryParam, { tableId: this.item.tableId, type: ['webFormView', 'appFormView'] })
        }).then(res => {
          return res.result
        })
      }
              ...
              this.axios({
                ...
                if (!res.code) {
                  const data = {
                    ...
                    } : Object.assign(this.settingData.data, { id: Number(res.result.id), templateId: res.result.templateId }),
                    tableId: this.item.tableId,
                    type: values.type,
                    module: this.item.module,
                    alias: this.item.data ? this.item.data.alias : '',
