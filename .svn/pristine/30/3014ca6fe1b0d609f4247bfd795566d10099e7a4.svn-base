          ...
          <a-col :span="config.mode === 'multiple' ? 14 : 24">
            <a-row type="flex">
              <a-col :span="12">
                ...
                >
                  <a-input v-model="queryParam.username" :placeholder="$t('请输入账号搜索')" />
                </a-form-item>
              </a-col>
              <a-col :span="12">
                <a-form-item :label="$t('角色')" :labelCol="{ span: 4 }" :wrapperCol="{ span: 20 }">
                  <a-select
                  ...
                  >
                    <a-select-option v-for="(item, key) in roleList" :key="key" :value="key">
                      {{ item }}
                    </a-select-option>
                  </a-select>
                </a-form-item>
            ...
            <a-row type="flex" justify="space-between">
              <a-col :span="18" :style="config.mode === 'multiple' ? { marginLeft: '-8px' } : { marginLeft: '-28px' }">
                <a-form-item :label="$t('部门')" :labelCol="{ span: 3 }" :wrapperCol="{ span: 21 }">
                  <a-tree-select
              ...
              <a-col v-if="config.mode !== 'multiple'" :span="2">
                <a-form-item :wrapperCol="{ span: 24 }">
                  <a-button htmlType="submit" @click="$refs.table.clearSelected()">{{ $t('清空') }}</a-button>
                </a-form-item>
              </a-col>
            </a-row>
            <s-table
            ...
            <div v-if="config.mode === 'multiple'" style="display: flex">
              <div>
                <div style="margin: 10px 0; display: flex; flex-direction: column">
                  <a-button style="margin: 60px 20px 20px 20px" @click="handleSelect">{{ $t('添加') }}</a-button>
                  <a-button style="margin: 0 20px" @click="deleteAll">{{ $t('清空') }}</a-button>
                </div>
              </div>
              <div style="flex: 1; margin-top: 10px">
                <a-card :title="$t('已选中的用户')" :bordered="false" size="small">
  ...
  methods: {
    ...
    loadDataTable (parameter) {
      return this.axios({
        ...
        data: Object.assign(parameter, this.queryParam, { init: this.department ? !this.department.length : true, fieldId: this.fieldId })
      }).then(res => {
