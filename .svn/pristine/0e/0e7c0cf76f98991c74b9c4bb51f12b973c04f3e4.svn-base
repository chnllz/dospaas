          ...
          <a-card size="small" style="flex: 1; margin-bottom: 10px" :headStyle="{ height: '10px' }">
                    ...
                    >
                      <a-col :span="4">
                        ...
                        >
                          ...
                          <a-select-option value="way">{{ $t('办理方式') }}</a-select-option>
                        </a-select>
                      </a-col>
                      ...
                      <a-col
                        v-else-if="simpleItem.fieldUser === 'way'"
                        :span="2"
                      ...
                      <a-col v-if="simpleItem.fieldUser === 'way'" :span="16">
                        <a-select v-model="simpleItem.way" size="small">
                          <a-select-option v-for="wayItem in wayName" :key="wayItem.alias" :value="wayItem.alias">
                            {{ wayItem.way }}
                          </a-select-option>
                        </a-select>
                      </a-col>
                                ...
                                <template v-if="includeItem.include !== 'em' && includeItem.include !== 'nem'">
                                  ...
                                  <div v-else-if="simpleItem.formType === 'cascader'">
                                    ...
                                    <tabs-select
                                      ...
                                      :value="
                                        simpleItem.option
                                          ? simpleItem.option.filter(
                                              (item) => item.dictDataNumber === includeItem.value
                                            )
                                          : []
                                      "
                                      @send="
            ...
            <a-form-item v-for="(eventItem, key) in item.event" :key="key" :wrapper-col="{ span: 24 }">
              <a-row type="flex" align="middle" :gutter="10">
                ...
                <a-col v-if="eventItem.type === 'field'" :span="8">
                  <a-row :gutter="10">
                    <a-col :span="12">
                      <a-select v-model="eventItem.field" size="small" show-search optionFilterProp="children">
                        <a-select-option
                    ...
                    <a-col :span="12">
                      <a-select
                        size="small"
                        :defaultValue="
                          eventItem.mandatory ? eventItem.status + '_' + eventItem.mandatory : eventItem.status
                        "
                        @change="
                          (e) => {
                        ...
                        "
                      >
                        <a-select-option value="show_true" @click="eventItem.titleField = false">
                          {{ $t('允许必填') }}
                        </a-select-option>
                        <a-select-option value="show_false" @click="eventItem.titleField = false">
                          {{ $t('允许不必填') }}
                        </a-select-option>
                        <a-select-option value="readonly_true" @click="eventItem.titleField = false">
                          {{ $t('只读必填') }}
                        </a-select-option>
                        <a-select-option value="readonly_false" @click="eventItem.titleField = false">
                          {{ $t('只读不必填') }}
                        </a-select-option>
                        <a-select-option value="hidden" @click="eventItem.titleField = false">
                          {{ $t('不可见可提交') }}
                        </a-select-option>
                ...
                <a-col v-if="eventItem.type === 'way'" :span="8">
                  <a-row :gutter="10">
                    <a-col :span="12">
                    ...
                    <a-col :span="12">
                      <a-select v-model="eventItem.status" size="small">
                        <a-select-option value="allow">{{ $t('显示') }}</a-select-option>
                ...
                <a-col flex="70px" style="display: flex; align-items: center; justify-content: center">
                  <a-icon
                    ...
                    type="plus-square"
                    theme="filled"
                    @click="item.event.splice(key + 1, 0, { type: 'field', field: '', status: '', mandatory: '' })"
                  />
                  <a-icon
                    :style="{
                      fontSize: '24px',
                    ...
                    type="minus-square"
                    theme="filled"
                    @click="item.event.length === 1 ? '' : item.event.splice(key, 1)"
                  />
                </a-col>
              </a-row>
            </a-form-item>
...
export default {
  ...
  methods: {
    show (config, data) {
      this.visible = true
      this.config = config
      this.tableId = config.flowData.params.tableId
      this.data = config.handleFormRules
      this.wayName = []
      ...
      this.axios({
        url: `/admin/field/getAliasFieldMapping?tableId=${config.flowData.params.tableId}`
      }).then((res) => {
        var arr = []
        for (var k in res.result) {
          arr.push(res.result[k])
        }
        this.fieldColumns = arr.filter(item => {
          // return ['image', 'editor', 'file'].indexOf(item.formType) === -1
          return ['editor'].indexOf(item.formType) === -1
        })
      })
    ...
    choiceField (index, key, fieldIndex, template) {
      if (template === 'so') {
        ...
        arr.event[key].field = this.fieldColumns[fieldIndex].alias
        ...
        obj.selectUser = this.fieldColumns[fieldIndex].alias
      }
    },
    // 选择关系
    choiceInclude (value, includeItem) {
