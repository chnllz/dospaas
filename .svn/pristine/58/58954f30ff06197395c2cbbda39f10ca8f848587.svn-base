<template>
  <div>
    ...
    >
      ...
      >
        ...
        <a-radio-group v-else-if="element.type === 'workflowFilter'" :disabled="element.fieldRule === 'readonly'">
          <a-radio-button
            v-for="workflow in element.workflowFilters.filter((filterItem) => filterItem.enable)"
            :key="workflow.value"
    ...
    <querier-control ref="querierControl" :fieldColumns="controlFieldsarr" :template="data" @func="addOther" />
...
export default {
  ...
  computed: {
    plainOptions: function () {
      return this.fieldColumns.map((item) => (item.name))
    }
  },
  watch: {
  ...
  created () {
    ...
    this.checkedList = this.fieldColumns
  ...
  methods: {
    ...
    workflowFilterSetting (data) {
      if (this.flowgroupBtnConfig.type !== 'webProcessCenterDataWindow' && !this.tabThis.form.getFieldValue('setting[workflowId]')) {
        this.$message.warning('请先选择一个与当前数据表关联的流程')
        return
      }
      let obj = data
      if (!obj) {
      ...
      this.$refs.tplviewDataQuerierWorkflowFilter.show({
        ...
        record: obj || { showWorkflowFilters: true }
      })
    },
      ...
      switch (e) {
        ...
        case 2:
          this.$refs.columnCheck.show({
            action: 'edit',
            title: this.$t('选择字段'),
            data: this.fieldMappings.filter(item => !['image', 'file', 'editor'].includes(item.formType))
          })
          break
        case 3:
          this.data = []
    ...
    // 保存字段
    saveField (choiceFields) {
      this.addField = false
      const arr = choiceFields.map(mapItem => {
        const obj = this.fieldColumns.find(findItem => findItem.alias === mapItem.alias)
        return {
          type: 'field',
          name: obj.name,
          fieldId: obj.fieldId,
      ...
      arr.forEach(item => {
        const index = this.data.findIndex(findItem => findItem.value === item.value)
        if (index !== -1) {
      ...
      this.data.forEach((item, index) => {
        if (item.type === 'field' && arr.every(findItem => findItem.value !== item.value)) {
          this.data.splice(index, 1)
        }
      })
    },
