...
export class FormRule {
    ...
    runRule (record, field) {
        ...
        for (let i = 0; i < fieldRule.length; i++) {
            ...
            // 当表单规则的if事件为false，且change事件字段在if事件中，则还原表单规则
    ...
    handleEvent (event, index) {
        event.forEach(item => {
            ...
            } else if (item.status === 'optionalRange') {
                const aliasItem = this.config[item.field]
                // 单选，下拉，多选，使用
                if (['radio', 'checkbox', 'combobox'].includes(aliasItem.formType)) {
                    aliasItem.option = aliasItem.option.map(im => {
                        let choiceOption = item.choiceOption || []
                        choiceOption = item.choiceOption.map(choiceItem => choiceItem.value)
                        im.disabled = !choiceOption.includes(im.value)
                        return im
                    })
                } else {
                    aliasItem.optionalRange = item.choiceOption
                }
            } else if (item.status === 'notOptionalRange') {
                const aliasItem = this.config[item.field]
                if (['radio', 'checkbox', 'combobox'].includes(aliasItem.formType)) {
                    aliasItem.option = aliasItem.option.filter(im => {
                        let choiceOption = item.choiceOption || []
                        choiceOption = item.choiceOption.map(choiceItem => choiceItem.value)
                        im.disabled = choiceOption.includes(im.value)
                        return im
                    ...
                    aliasItem.notOptionalRange = item.choiceOption
                }
            } else {
                const aliasItem = this.config[item.field]
    ...
    recoverConfig (event) {
        event.forEach(item => {
            ...
            aliasItem.option = oldItem.option
            if (aliasItem.optionalRange) {
                aliasItem.optionalRange = []
            }
            if (aliasItem.notOptionalRange) {
                aliasItem.notOptionalRange = []
            }
        })
    }
}
