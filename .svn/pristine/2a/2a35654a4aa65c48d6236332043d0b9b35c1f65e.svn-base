        ...
        <a-form :form="form">
            ...
            <a-col :span="12">
              <a-form-item :labelCol="{ span: 8 }" :wrapperCol="{ span: 16 }">
                ...
                <a-switch
                  v-decorator="[
                    'info[virtualField]',
                    { initialValue: data.virtualField == 1, valuePropName: 'checked' }
                  ]"
              ...
              <a-form-item :labelCol="{ span: 8 }" :wrapperCol="{ span: 16 }">
                ...
                <a-radio-group
                  v-decorator="[
                    ...
                    {
                      initialValue:
                        config.action === 'add' ? 1 : String(data.status) === 'undefined' ? undefined : data.status
                    }
                  ]"
                >
          ...
          <template v-if="data.virtualField !== 1">
            ...
            <template v-if="data.formType !== 'switch'">
              ...
              <template
                v-else-if="
                  data.fieldType === 'TINYINT' || data.fieldType === 'SMALLINT' || data.fieldType === 'MEDIUMINT'
                "
              >
                <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol">
                  <span slot="label">
                  ...
                  <a-input-number
                    v-decorator="[
                      'info[fieldLength]',
                      {
                        initialValue: data.fieldLength || 11,
                        rules: [{ required: true, message: $t('请输入字段长度') }]
                      }
                    ]"
                    :precision="0"
            ...
            <template v-else-if="data.formType === 'number'">
              <field-number
                ...
                @changeDefaultNumber="changeDefaultNumber"
              />
            </template>
            <!-- 级联 -->
            <template v-else-if="data.formType === 'cascader'">
              ...
              <a-form-item :label="$t('后置图标')" :labelCol="labelCol" :wrapperCol="wrapperCol">
                <a-row>
                  ...
                  <a-col :span="12">
                    ...
                    >
                      <a-badge v-if="suffixVal.length" status="success" />
              ...
              <a-form-item :label="$t('可选范围')" :labelCol="labelCol" :wrapperCol="wrapperCol">
                <a-row type="flex" align="middle">
                  ...
                  <a-col v-if="optionValue === '0'" :span="20">
                    <data-picker
                      :key="cascaderRangeUpdate"
                      ref="dataPicker"
                      name="setting[form][optionalRange]"
                      :parentId="'parentDictDataNumber'"
                      :parameter="{ dictCategoryNumber: cascaderValue, filterDisabled: true }"
                      displayType="tree"
                      :url="'/admin/dict/initData'"
                      :searchUrl="'/admin/search/dictSearch'"
                      :multiple="true"
                      :value="setting.form.optionalRange"
                      :optionsConversion="
                        (list, displayType, searchType) => optionsConversionTreeselect(list, displayType, searchType)
                      "
                      @select="
                        (data, options) => {
                          setting.form.optionalRange = options
                        }
                      "
                    />
                  </a-col>
            ...
            <template v-else-if="data.formType === 'switch'">
              <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol">
                ...
                <a-radio-group
                  :defaultValue="setting.form.displayMode ? setting.form.displayMode : 'switch'"
                  style="margin-top: 10px"
                  @change="
                    (e) => {
                      setting.form.displayMode = e.target.value
                    }
                  "
                >
                  <a-radio value="switch">{{ $t('开关') }}</a-radio>
                  <br />
                  <a-radio value="radio">{{ $t('单选框') }}</a-radio>
                  <br />
                  <a-radio value="button">{{ $t('单选按钮组') }}</a-radio>
                  <br />
                  <a-radio value="combobox">{{ $t('下拉框') }}</a-radio>
                </a-radio-group>
              </a-form-item>
              <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol">
                <span slot="label">
            ...
            <template v-else-if="data.formType === 'tag'">
              <a-form-item :label="$t('标签设置')" :labelCol="labelCol" :wrapperCol="wrapperCol">
                <!-- 该input只是为了给label前添加红星以示必填项， setting[form][tagSetting]中的数据在提交时会被重新覆盖-->
                <a-input
                  v-decorator="[
                    'setting[form][tagSetting]',
                    {
                      initialValue:
                        ...
                        setting.form.tagSetting.tagRule.length > 0
                          ? JSON.stringify(setting.form.tagSetting)
                          : undefined,
                      rules: [{ required: true, message: $t('请设置标签') }]
                    }
                  ]"
                ...
                <field-tags-setting
                  ...
                  @ok="
                    (data) => {
                      $set(setting.form, 'tagSetting', data)
                      if (data.tagRule.length > 0) {
                        form.setFieldsValue({
                          'setting[form][tagSetting]': JSON.stringify(data)
                        })
                      }
                    }
                  "
              ...
              >
                ...
                <a-row type="flex" align="middle">
                  ...
                  <div
                    style="position: absolute; right: 4px; cursor: pointer; z-index: 10"
                      ...
                      <template v-else-if="defaultType === 'value'">
                        ...
                        <template v-else-if="data.formType === 'cascader'">
                          ...
                          <tabs-select
                            ...
                            :writeBack="writeBack"
                            fieldType="field"
                            :value="cascaderDefaultValOptions"
                        ...
                        <a-col :span="24">
                          <querier-codemirror-input
                            ...
                            @setFormFields="
                              (data) => {
                                form.setFieldsValue({
                                  defaultValueCheck: data.value
                                })
                                form.validateFields(['defaultValueCheck'], (errors, values) => {})
                              }
                            "
                          />
                        </a-col>
                  ...
                  <a-col :span="24">
                    <querier-codemirror-input
                      ...
                      @setFormFields="
                        (data) => {
                          form.setFieldsValue({
                            forcedDefaultValue: data.value
                          })
                          form.validateFields(['forcedDefaultValue'], (errors, values) => {})
                        }
                      "
                    />
                  </a-col>
...
export default {
  ...
  data () {
    ...
    return {
      ...
      formtypes: {
        ...
        number: {
          ...
          fieldtypes: [
            ...
            { value: 'BIGINT', text: this.$t('BIGINT-长整数') },
      ...
      numMinVal: 0,
      numMaxVal: 0,
      cascaderRangeUpdate: 0 // 用于更新级联选择组件==>可选范围下的data-picker组件
    }
  },
  computed: {
    ...mapGetters(['userInfo'])
  ...
  methods: {
    ...
    show (config) {
      ...
      this.axios({
        ...
        const fieldtypes = this.aliasDisabled ? this.formtypes[this.data.formType]?.fieldtypes : []
        this.fieldtypes = JSON.parse(JSON.stringify(fieldtypes))
        ...
        if (this.data.formType === 'datetime') {
          ...
          if (this.setting.form.defaultTemplate === 'value') {
            this.axios({
              url: '/admin/dict/getCascader',
              data: { dictCategoryNumber: this.setting.form.src, dictDataNumber: this.setting.form.defaultValue }
            }).then(res => {
              const valOptionsList = res.result
              const loopSetDefaultOptions = (list) => {
                list.forEach(optionItem => {
                  if (optionItem.number === this.setting.form.defaultValue) {
                    this.cascaderDefaultValOptions = [optionItem]
                  }
                  if (optionItem.children && optionItem.children.length > 0) {
                    loopSetDefaultOptions(optionItem.children)
                  }
                })
              }
              loopSetDefaultOptions(valOptionsList)
            })
          }
        } else if (this.data.formType === 'number') {
          this.numMinVal = String(this.data.setting.attribute.minValue) === '0' ? 0 : this.data.setting.attribute.minValue
          this.numMaxVal = String(this.data.setting.attribute.maxValue) === '0' ? 0 : this.data.setting.attribute.maxValue
        }
    ...
    getCascader (value) {
      ...
      this.setting.form.optionalRange = []
      this.cascaderRangeUpdate++
      this.$nextTick(() => {
    ...
    getUploadNum (value, type) {
      ...
      if (!this.minImg && this.minImg !== 0) {
        ...
        this.imgHelp = this.$t('请输入最小上传数量')
      } else if (!this.maxImg && this.maxImg !== 0) {
        this.imageStatus = 'error'
        this.imgHelp = this.$t('请输入最大上传数量')
      } else {
        this.imgHelp = ''
    ...
    handleComponentChange (value) {
      ...
      const fieldtypes = this.formtypes[value].fieldtypes
      this.fieldtypes = JSON.parse(JSON.stringify(fieldtypes))
      const fieldType = this.formtypes[value].fieldtypes[0].value
      const { setFieldsValue } = this.form
    ...
    // 数字组件，"允许输入数值范围" 和 "默认值" 的联动
    changeDefaultNumber (payload) {
      let defaultNumber
      if (this.defaultType) {
        defaultNumber = this.form.getFieldValue('setting[form][defaultValue]')
      }
      if (payload.type === 'numMaxVal' && ((payload.val < defaultNumber && typeof defaultNumber !== 'undefined') || typeof defaultNumber === 'undefined')) {
        this.numMaxVal = payload.val
        if (this.defaultType) {
          this.form.setFieldsValue({
            'setting[form][defaultValue]': payload.val
          })
        }
      } else if (payload.type === 'numMinVal' && ((payload.val > defaultNumber && typeof defaultNumber !== 'undefined') || typeof defaultNumber === 'undefined')) {
        this.numMinVal = payload.val
        if (this.defaultType) {
          this.form.setFieldsValue({
            'setting[form][defaultValue]': payload.val
          })
    ...
    getDataForm (value, uid) {
      this.axios({
        ...
        data: { tableId: this.tableId, type: ['webSubformDataWindow'] }
      }).then(res => {
        if (res.code === 0) {
    ...
    setFormFields (data) {
      ...
      this.form.validateFields(['forcedDefaultValue'], (errors, values) => { })
    },
    ...
    getCondition (data) {
      if (this.forcedType === 'forced') {
        ...
        this.form.validateFields(['defaultValueCheck'], (errors, values) => { })
      }
    },
    // 组织结构options转换
    ...
    toSubmit (values, visible) {
      ...
      switch (values.info.formType) {
        case 'switch':
          ...
          values.setting.form.displayMode = this.setting.form.displayMode || 'switch'
          if (this.setting.form.word.type === 'custom') {
            const value = this.setting.form.word.value
            if (!value[0] || !value[1]) {
              this.$message.error('请填写自定义的显示内容')
              this.loading = false
              return
            }
          }
        ...
        case 'organization':
          ...
          values.info.optionType = values.setting.form.optionType
          values.info.optionSource = values.setting.form.optionType
          break
        case 'associated':
          ...
          values.info.optionType = val.setting.attribute.dataSource // 树形字典
          values.info.optionSource = val.setting.form.src
          break
        case 'text':
        ...
        case 'number':
          ...
          values.setting.attribute = Object.assign(values.setting.attribute, val.setting.attribute)
          values.setting.form = Object.assign(values.setting.form, val.setting.form)
          if (values.info.fieldType !== 'DECIMAL') {
            values.info.fieldDecimal = null
          }
          if (values.info.fieldType === 'INT') {
            values.info.fieldLength = 11
          }
          if (values.info.fieldType === 'BIGINT') {
            values.info.fieldLength = 20
          }
          break
        case 'datetime':
        ...
        case 'checkbox':
          ...
          if (String(values.setting.form.optionRange) === '0') {
            values.setting.form.optionalRange = val.setting.form.optionalRange
          }
          values.setting.attribute = Object.assign(values.setting.attribute, val.setting.attribute)
          values.setting.attribute.checkAll = values.setting.attribute.checkAll ? '1' : '0'
          values.setting.form = values.setting.form ? values.setting.form : {}
          values.setting.form = Object.assign(values.setting.form, val.setting.form)
          values.info.optionType = val.setting.attribute.dataSource
          values.info.optionSource = val.setting.form.src
          break
        case 'file':
        ...
        case 'cascader':
          ...
          if (String(values.setting.form.optionRange) === '0') {
            values.setting.form.optionalRange = this.setting.form.optionalRange
          }
          break
        case 'address':
          ...
          values.info.optionType = 'addressBook'
          values.info.optionSource = 'addressBook'
          break
        case 'treeselect':
          ...
          values.info.optionType = values.setting.attribute.dataSource
          values.info.optionSource = values.setting.attribute.dataSource === 'addressBook' ? 'addressBook' : values.setting.form.src
          if (String(values.setting.form.optionRange) === '0') {
            values.setting.form.optionalRange = this.setting.form.optionalRange
          }
          break
        ...
        case 'tag':
          ...
          values.info.optionType = 'tag'
          values.info.optionSource = values.setting.form.tagSetting.rnumber.join(',')
          break
        default:
          break
      }
      ...
      // 虚拟字段默认设置为单行文本，VARCHAR，1
      if (values.info.virtualField) {
