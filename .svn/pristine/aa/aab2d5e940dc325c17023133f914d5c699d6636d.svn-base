  ...
  <div class="knowSelectBox">
    <a-form :form="form" style="display: flex; padding-left: 8px">
      ...
      <a-form-item>
        ...
        <a-select
          v-else-if="params.type === 'questions'"
          :open="false"
          :show-arrow="false"
          default-value="1"
          :value="theme"
      ...
      <a-form-item v-if="theme === '1'" style="flex: 1; padding-right: 8px; margin-top: 1px">
        <a-input
          ...
          allowClear
          :placeholder="params.type !== 'questions' ? $t('请输入搜索内容') : $t('请输入标题搜索')"
          @pressEnter="search"
        />
      </a-form-item>
  ...
  watch: {
    params: {
      handler (newValue) {
        ...
        if (!newValue.status) {
          if (newValue.type === 'myCreated') {
            this.status = '5'
            this.$emit('ok', this.sort, this.words, this.theme, this.status)
          } else if (newValue.type === 'myAudited') {
            this.status = '1'
            this.$emit('ok', this.sort, this.words, this.theme, this.status)
          } else if (newValue.type === 'dealBug') {
            this.status = '8'
            this.$emit('ok', this.sort, this.words, this.theme, this.status)
          }
        }
  ...
  methods: {
    ...
    search () {
      this.form.validateFields((err, values) => {
        if (!err) {
          this.words = this.form.getFieldsValue().words
