  ...
  <div class="page">
    <a-form :labelCol="{ span: 6 }" :wrapperCol="{ span: 18 }" class="search" :colon="false">
      <a-card size="small" :title="$t('搜索')">
        <a-space slot="extra">
          <a-button @click="taskSearch" @keyup.enter="taskSearch">{{ $t('搜索') }}</a-button>
          <a-button
            @click="
              () => {
                queryParam = {}
    ...
    >
      <div slot="action" slot-scope="text, record">
        <a @click="showPage(record)">{{ $t('明细') }}</a>
        <a-divider type="vertical" />
        <a @click="assignPage(record)">{{ $t('分配') }}</a>
        <a-divider type="vertical" />
        <a @click="editPage(record)">{{ $t('编辑') }}</a>
        <a-divider type="vertical" />
        <a @click="taskDelete(record)">{{ $t('删除') }}</a>
      </div>
    </s-table>
    <a-drawer :title="config.title" :width="600" :destroyOnClose="true" :visible="visible" @close="visible = !visible">
      <a-spin :spinning="loading">
        <a-form :form="form" :label-col="{ span: 6 }" @submit="handleSubmit">
          <a-form-item :label="$t('任务名称')" :wrapper-col="{ span: 16 }">
            <a-input
              v-decorator="[
                'info[taskname]',
          ...
          <a-form-item :label="$t('选择问卷')" :wrapper-col="{ span: 16 }">
            <a-select
              v-decorator="[
                'info[nairename]',
          ...
          <a-form-item :label="$t('任务备注')" :wrapper-col="{ span: 16 }">
            <a-textarea
...
export default {
  ...
  data () {
    return {
      ...
      // 搜索参数
      queryParam: {},
      nairenamelist: [this.$t('调查问卷一'), this.$t('调查问卷二')],
      nairename: [this.$t('调查问卷一'), this.$t('调查问卷二')],
      config: {
        data: {}
      },
        ...
        if (!err) {
          if (this.config.action === 'edit') {
            ...
            self.axios({
              ...
              this.visible = false
              table.refresh()
              self.$message.success(this.$t('修改成功'))
            })
          } else {
            ...
            self.axios({
              ...
              this.visible = false
              table.refresh()
              self.$message.success(this.$t('添加成功'))
            })
          }
        }
      })
